...
> **2026-08-24 CODEX/DEPENDABOT — Alpaca-trade-api v4 migration BLOCKED (PR #3077, `dependabot/npm_and_yarn/alpacahq/alpaca-trade-api-4.0.1`).**  Dependabot major bump `@alpacahq/alpaca-trade-api` 3.1.3 → 4.0.1; v4 is a full SDK rewrite (named export only, flat methods → `trading.*`/`marketData.*` namespaces, camelCase responses), so `src/lib/alpaca.ts` (v3 flat surface) breaks the build (`tsc` TS2613 at line 1).  Codex P1 threads on quote/order/account/position parsing are all symptoms of the one migration.  **Scope/next action:** owner decision — (a) migrate the adapter + test fixtures to v4, or (b) revert the bump and retain v3.  Autofix does not guess at the real-money gateway.  Rollout: `docs/rollouts/2026-08-24-alpaca-v4-dependabot-blocked.md`.
...
> **2026-08-22 CURSOR — Alpaca Paper Approve greyed after Retry Red Team (`cursor/alpaca-paper-approve-greyed-8ba8`).**  Paper is Exit-only.  Retry no longer greys Approve.  Retry applies the Red verdict.  Owner Approve of an opening in Exit-only is the override; the agent still will not open on its own.  Halted and liquidating still block openings.  Do not HOTFIX during this change.  Rollout: `docs/rollouts/2026-08-22-alpaca-paper-approve-greyed.md`.
