...
> **2026-08-21 CURSOR — Activity layout redesign (`cursor/activity-layout-redesign-085b`, PR #3031).**  Five Title Case Activity tabs (Alerts Center first).  All → Audit Log.  Plain-English run failures.  APNs-first delivery + admin system-alert fanout.  Alpaca session-close vs Delayed Quote.  iOS chips/rail parity with additive `?tab=` deep links.  Rematched onto `origin/main` `ffb4ea1b`; dispatching `verify` + `ios-build` because cloud-proxy pushes skip `pull_request`.  Rollout: `docs/rollouts/2026-08-21-activity-layout-redesign.md`.
