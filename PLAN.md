...
> **2026-09-01 CLAUDE - PR #3139 plist comment: actually fix the XML `--` bug — MERGED (`fix/plist-comment`, PR #3139, squash `fe9944e81`).**  The PR's original fix (relocating the comment below `<!DOCTYPE>`) did not fix `plistlib.loads` raising `ExpatError` on `scripts/com.jay.provider-knob-sync.plist` because XML comments cannot contain `--` anywhere, and the comment still had two literal `--apply` mentions.  Reworded both to "the apply flag"; verified `plistlib.load` now parses clean (it raised before).  Also adds the rollout-doc update AGENTS.md requires for script changes.  Both bot threads owner-resolved; hosted `verify` gate went green and pre-armed auto-merge landed it on its own.  Rollout: `docs/rollouts/2026-09-01-pr-3139-plist-comment-fix.md`.
