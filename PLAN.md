...
> **2026-08-20 CURSOR — Kimi fleet audit D/E/F (`cursor/kimi-audit-def`).** Operator Robinhood stays for CT peer **intraday** only (`operatorPeerRead` after ingest token; stored `local` token, not env bypass). Quotes unchanged. `npm ci` on verify/e2e; SHA-pinned gh CLI; Dependabot github-actions; SHA pin + required `check-pin`. Rollout: `docs/rollouts/2026-08-20-kimi-audit-def.md`.
