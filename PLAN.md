...
- **congress.trade — return-path + analytics ownership reply** (2026-06-24,
  ...
  **pull/pull** transport (no aggregate pushing either way); specified the inbound
  return-path contract App A is waiting on. Two scoped follow-up PRs — **both BUILT
  2026-06-25** (`docs/rollouts/2026-06-25-app-b-securities-import-fundamentals-price-targets.md`),
  additive + default-OFF:
  (1) `feat/securities-import-receiver` — `POST /api/admin/securities/import`
  (bearer `APP_B_INGEST_TOKEN`, default-closed) + a local EOD cache table wired as an
  opt-in, density-guarded `fetchDailyOHLC` tier, to land App A's price/spx/ref gap-fills;
  (2) extended `congress-share.ts` with `fundamentals[]`/`analyst[]` for App A's PR #46
  import slots. Numeric price targets are now ALSO fillable via the opt-in FMP
  `price-target-consensus` provider (`FMP_PRICE_TARGETS_ENABLED`) — they no longer ride
  null. Operator sets the tokens/flags to activate; cross-app token handoff is out-of-band.
