...
> **2026-08-25 CURSOR — CI autofix: vitest exclude for #3091 node:test (`cursor/ci-autofix-automation-bcfd`).**  `publish-ios-versions.test.mjs` stays on `node --test` in `ci.yml`.  Vitest must not glob `scripts/**/*.test.mjs` or `verify-hosted` dies after the rest of the suite.  Rollout: `docs/rollouts/2026-08-25-vitest-exclude-node-test-ios-versions.md`.
