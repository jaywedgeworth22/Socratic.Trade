...
> **2026-09-01 CLAUDE - L1 boundary-trim hardening + Congress.Trade wedge (`claude/l1-trim-hardening`).**  Closes the three guard gaps #3140 accepted but left open.  Repo copy resynced byte-identical to the hardened host tool (sha256 `71c14e0a...668c`, 8018 bytes, mode 755).  Adds `--app {socratic,congress,usage-monitor}` with a bucket/prefix table; a relative truncation guard (abort under 50% of the previous snapshot - the 100 MB absolute floor cannot reject a truncated ~4.5 GB snapshot); full kept-chain contiguity (twins allowed) with its own exit code, since L2 stays wedged on non-contiguous input; `--max-snapshot-age-hours` (default 48, scheduled units pass 6); an abort when nothing is superseded while L1 exceeds 200 objects; and `--b2-hard-delete` on every delete, because a plain delete writes only a hide marker and the bytes stay BILLED for ~a day (measured: 199.59 GB billed vs 126.49 GB logical, ~73 GB overhang).  Exit codes 0-6 documented.  **CT had never been trimmed:** 2,413 L1 objects, 2,362 (24.6 GB) superseded, 51 (12 MB) kept, L2 at ZERO - same wedge as ST.  Six TRANSIENT one-shots armed 2026-09-02 00:04/00:20/00:40 UTC (`l1trim-st-*`, `l1trim-ct-*`), replacing the stopped `l1-boundary-trim-oneshot`.  **Do NOT convert these into a permanent nightly timer** - transient units expire on reboot by design; trimming below a snapshot costs sub-daily PITR granularity and the 168h snapshot retention stays authoritative.  **Open blocker:** the CT `--apply` run removed zero objects in its first ~7 minutes; the delete loop is O(n^2) (~8s/object) and hides failure detail, so prove one manual CT run actually removes objects before trusting the CT timers.  Rollout: `docs/rollouts/2026-09-01-l1-trim-hardening.md`.
