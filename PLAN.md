...
In progress on `codex/model-catalog-account-labels` (board `0d775009`, PR #3196).  Refreshed provider models, MiniMax API support, and concise account labels.  Browser fixture QA passed; hosted lint, typecheck, and tests passed on aec6c040a, build pending.  Follow-up review fixes align chat availability and usage budgets with actual tenant credential routing, preserve MiniMax schema prompts through OpenRouter, block Infisical runtime-key writes, and correct learning-review model identity.  These follow-up changes require a new hosted gate.  Local dependencies remain incomplete after registry ETIMEDOUT; no local full-gate or deployment claim.  Rollout: `docs/rollouts/2026-09-09-model-catalog-account-labels.md`.
