...
| 10 | Stronger signals, learning & UI (v2 plan) | `docs/phase-10-signals-learning-ui-v2.md` | Forward plan (executing on branch `phase-10`): positioning/smart-money deterministic sub-score so scraped data affects ranking (Codex P2 remaining half); sector + factor learning dimensions; full EvidenceDigest for chosen+skipped + counterfactual learning; new free sources (SEC 8-K, market breadth, Kenneth French, options/put-call, analyst revisions, XBRL facts); adaptive prompt compaction + prompt caching + raw-document digests; symbol drilldown drawer + learning-matrix UI; exposed scoring thresholds; de-risk-in-Crisis guardrail |
| 11 | Multi-user & API-key management (plan) | `docs/phase-11-multi-user.md` | Forward plan (not started): per-user analysis/proposals from each user's own preferences + API keys. Near-term: an API-keys Settings section (single default user, builds on the existing `user_api_keys` table) + route providers through `resolveApiKey(service,userId)`. Later: per-user policy/data isolation, concurrent per-user scheduler execution, then identity/auth. Single-user (default `local`) for testing now |
