...
> **2026-08-18 CURSOR — rag-embed DeepInfra batch-window (`cursor/rag-embed-batch-window-54d7`, #2840).** Live 32-text embed POSTs 400 at 8193 tokens on baai/bge-m3 (batch-sum, not one unchunked 10-K).  Keep hybrid condense-first; token-pack after that step so a count-32 batch stays under ~7500 approxTokens.  Isolate one over-limit condensed text; do not fail the companions.  Do not invent a second filing chunker.  Do not flip write-class or prune.  Do not re-clamp the #2800 fuse.  Leave the #2812 health 503 gate alone.  Rollout: `docs/rollouts/2026-08-18-rag-embed-batch-window.md`.
