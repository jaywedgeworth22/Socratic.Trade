...
| SEC/RAG | 10-K, 10-Q, 8-K and fundamentals chunks; SEC XBRL | Point-in-time retrieval uses one run `asOf`; retrieved text is contained, budgeted, hashed, and shared with both teams | Undated or instruction-like text could enter prompts without one run-wide receipt; a nonexistent transcript type was requested | Corpus breadth and occurrence-level provenance still depend on the separate staged SEC backfill program; no earnings-transcript producer exists yet |
...
3. The SEC corpus expansion, immutable artifact archive, and occurrence-level filing provenance are
   ...
   completeness.
4. Earnings transcripts are not requested because no producer is currently wired. Add a licensed,
   point-in-time producer before adding that document type back to retrieval.
