...
## Provenance / rescue note

This feature was originally committed (as `d014842`) bundled into a throwaway
`safety-quick-wins` branch alongside unrelated safety quick-wins, a UI/UX
overhaul, doc deletions, and CI-workflow moves — and on a `main` base that
predated the proposal-revalidation merge, so it did not build on current `main`.
It was **re-extracted clean** onto current `main` (`feat/account-capabilities`)
by applying only the additive capabilities deltas: `types.ts`, `db.ts` (column +
migration + parse + upsert), the Alpaca/Robinhood gateways, the `policy.ts`
two-layer short gate, the `strategy.ts` context passthrough, the
connected-accounts route, the dashboard capability badges, and
`test/policy.test.ts`. Proposal-revalidation is untouched; the safety quick-wins
live in PR #13; the CI moves and UI overhaul were excluded. Because it was
extracted rather than authored here, the owning session should review the result
before merge.

Verification: `npx tsc --noEmit` clean · `npm test` 397 passed (50 files) ·
