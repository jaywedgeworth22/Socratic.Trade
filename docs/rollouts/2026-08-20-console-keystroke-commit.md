...
- `app/console/settings/page.tsx` — `DataSourcesCard` number rows: added `numberDrafts` state, `resolveSourceFeatureNumberCommit`, `commitNumberRow`.
...
**The commit decision is extracted as a pure exported function** in each file (`resolveSourceFeatureNumberCommit`, `resolveMaxActiveCommit`) purely so the no-fallback-on-empty rule is testable without a DOM.  This is factoring out logic that was previously inline, not a new generic abstraction.
