...
```bash
PATH="/opt/homebrew/opt/node@24/bin:$PATH" npm run lint       # <fill: result>
...
PATH="/opt/homebrew/opt/node@24/bin:$PATH" npx vitest run     # <fill: result>
PATH="/opt/homebrew/opt/node@24/bin:$PATH" npm run build      # <fill: result>
```

Targeted, all green pre-full-run:
