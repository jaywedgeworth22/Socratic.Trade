...
## Context & Objective
The user noticed that the Socratic Trade iOS app on Mac Catalyst was launching upside down due to a misconfigured orientation constraint. Additionally, they requested a return to the "stacked" wordmark (SOCRATIC on top of TRADE) for the native splash screen, matching earlier designs, and wanted missing app icons on their personal site restored. 

## Changes Made
- **iOS Orientation**: Removed `UIInterfaceOrientationPortraitUpsideDown` from `ios/project.yml` and `ios/SocraticTrade/Info.plist`. This corrects the upside down window issue when launched on macOS.
