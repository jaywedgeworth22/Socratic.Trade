...
## In Progress
- **Expensive admin-operation abuse/cost controls (CODEX, branch `codex/admin-rate-limits`, MERGED PR #1409, worktree `/Users/jay/.codex/worktrees/socratic-admin-rate-limits`) — COMPLETED; SHARED v1.5.0 FOLLOW-UP PR #1426 CURRENT-MAIN GREEN 2026-07-11.** Named per-admin anti-repeat budgets and contract-aligned 429/409 responses wrap high-cost admin actions (`reindex-8k`, `reindex-10k`, `backtest-ic`, `tuning-dry-run`, Congress score-eval/share, `refresh-websource`, Robinhood probe). PR #1409 merged as `9552b648`; that merge dropped the wrappers, but later Tradier merge `e3d04221` restored all eight. Current-main reconciliation verifies `requireAdmin` still precedes every guard, so #1426 now carries only the exact released/tag-install-verified shared `v1.5.0` pin, local HTTP-adapter delegation, shared-schema assertions, and restored real Auth.js provenance coverage. Current-main Node24 gate green: focused 29/29, lint 0 errors, tsc, 330 files/3,740 tests, build. Hosted refresh and matched Congress.Trade peer pin pending. Scheduler/background convergence remains separately reserved.
- **Unify manual and scheduler single-flight at underlying provider/dataset operation boundaries (CODEX, branch `codex/provider-operation-leases`, worktree `/Users/jay/.codex/worktrees/socratic-provider-leases`, 2026-07-11) — IN PROGRESS; IMPLEMENTED / FOCUSED GREEN.** Four durable SQLite settings-KV owner-token groups now cover RAG reindex/filing ingest, Congress share, Congress refresh, and SEC 8-K refresh across manual and background entrants. Immediate acquisition, TTL heartbeat, persisted-owner revalidation/cooperative loss cancellation, and owner-checked release close process/deploy overlap; every non-forced path rechecks cadence after acquisition. The admin guard claims before rate debit and passes an opaque claim for core reuse. Typed busy results are benign for background callers (no network/marker advancement) and map through the shared-v1.5-backed 409 adapter for admin routes. `scheduler.ts` is untouched; detached `refreshEightK` embedding remains detached by explicit scope. Node 24 focused verification: 9 files / 130 tests (including the independently discovered route-harness regression); `tsc --noEmit` clean; touched-file ESLint 0 errors / 4 inherited warnings. Parent review/full ordered gate remain before commit/PR. Rollout: `docs/rollouts/2026-07-11-provider-operation-leases.md`.
- **Alpha Vantage health lane canonicalization (CODEX, branch `codex/alpha-health-lane-fix`, ready PR #1438, worktree `/Users/jay/.codex/worktrees/socratic-alpha-health`) — READY / CURRENT-MAIN GREEN 2026-07-11.** Live Alpha failures are genuine free-plan daily-cap exhaustion and correctly noncritical; no provider or secret change was made. Fixed the `EXPECTED_BACKEND_LANES` typo `alphavantage` -> `alpha-vantage` that created a phantom neutral card beside the real env lane; authenticated route coverage proves one canonical lane and no legacy-spelling entry. Reconciled through `main@da9558ac`; final Node 24 gate is green: focused 1/1, lint 0 errors (404 inherited warnings), TypeScript, full 332 files / 3,747 tests, and production build. Hosted checks and production verification remain. Separate unused dev-only `drizzle-kit` cleanup has its own lane.

- **Expensive admin-operation abuse/cost controls (CODEX, branch `codex/admin-rate-limits`, MERGED PR #1409, worktree `/Users/jay/.codex/worktrees/socratic-admin-rate-limits`) — COMPLETED; SHARED v1.5.0 FOLLOW-UP PR #1426 FINAL CURRENT-MAIN GREEN 2026-07-11.** Named per-admin anti-repeat budgets and contract-aligned 429/409 responses wrap high-cost admin actions (`reindex-8k`, `reindex-10k`, `backtest-ic`, `tuning-dry-run`, Congress score-eval/share, `refresh-websource`, Robinhood probe). PR #1409 merged as `9552b648`; that merge dropped the wrappers, but later Tradier merge `e3d04221` restored all eight. Current-main reconciliation verifies `requireAdmin` still precedes every guard, so #1426 now carries only the exact released/tag-install-verified shared `v1.5.0` pin, local HTTP-adapter delegation, shared-schema assertions, and restored real Auth.js provenance coverage. Final `main@e395e65a` Node24 gate green: focused 29/29, lint 0 errors/404 warnings, tsc, 331 files/3,746 tests, build. AG's ready Congress.Trade PR #296 exact-pins the same tag/commit with 940 tests green; its peer check is expected red until #1426 lands first. Scheduler/background convergence remains separately reserved.
...
- **Strategy lease correctness hotfix (CODEX, new follow-up after landing train) — PLANNED / RESERVED 2026-07-11.** Use unique approval owners, remove obsolete release calls, make renewal loss explicit/safe, cover wrong-owner/duplicate/default-on behavior, and sync stale env/docs. No AG branch collision; branch only after current main settles.
