...
## In Progress
- **[Socratic.Trade][GROK4] Multi-wave expert-review implementation (claimed 2026-07-20) — IN PROGRESS.** Branch `grok/multi-wave-a-onward`. **Wave A DONE** (skip≠completed, early budget admission, openrouter enforce identity, SF price, paper wording, first-run Needs-attention, circuit-breaker). **Wave C partial DONE** (mark-price add-to-loser, short labels, short synthetic trail from shortStopLossPct, bracket shortStopLossPct). Next: Wave B re-embed+RAG expansion, coach ingest, broker cover stops. Pushover already in notify.ts (env NOTIFY_PUSH_PROVIDER=pushover).
