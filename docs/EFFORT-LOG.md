...
## Completed
- **[Socratic.Trade][CLAUDE-sub] PR #1669 (SEC/RAG Phase 3) Codex-thread pickup — IN PROGRESS 2026-07-17 (owner-directed cap-reset pickup of the stalled AG lane; round 2 done).** Round 1: fixed the 6 stale unresolved Codex threads (form-aware Item titles, standalone SEC headings, valid td-only GFM tables, documented unversioned accession skip) + 3 regression tests. Round 2 (coordinator-directed, same day): fixed ALL remaining 21 unresolved threads — 2 P1s (embed/rerank provider routing; additive embedding-space isolation via model-aware embed-revision ids + `embed_model` query filter, no corpus purge/migration) and 19 P2s across sec-ingest-worker (serialize/verify/heartbeat/acceptance/FTS-after-commit), sec-filings production FTS wiring, db-learning per-occurrence FTS dedupe, search-fusion (bm25 ASC + provider-correct MMR), sec-facts (numeric booleans, aff10b5One, periodOfReport, multi-owner, transaction_code + guarded v50 migration, IFRS, error propagation), eval harness (denominator + ESM guard), and chunker overlap cap. +9 more regression tests. Gates green both rounds (round 2: tsc / 408 files / 4,690 tests / build / lint 0 errors). After reply+resolve, PR #1669 should have zero unresolved threads; armed auto-merge waits on `verify`. Correction note (in place, per board rules): the AG row below marks PR #1669 COMPLETED, but the PR is NOT merged until auto-merge fires. Rollout: `docs/rollouts/2026-07-17-pr1669-parser-thread-pickup.md` (both rounds).
