...
- **[Socratic.Trade][CURSOR] Gather must not inventory Pinecone or latch on congress 502 / Massive 429 — IN PR #2854 2026-08-20 (branch `cursor/gather-no-pinecone-inventory-befc`, rebased onto `44e9ef06`).** CONFLICTING/DIRTY vs later main was phantom (`merge-tree` clean, docs-union).  Rebase onto `ce31c367` then `44e9ef06`.  Kept gather skip + 502/429 fail-open.  Vector-db tests stub idle `getDb`.  Did not absorb other clusters.  Do not flip write-class.  Do not prune.  Do not merge.  Rollout: `docs/rollouts/2026-08-19-gather-no-pinecone-inventory.md`.
