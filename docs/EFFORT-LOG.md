...
## In Progress
- **2026-08-25 — CURSOR — IN PROGRESS — Console honesty: Guardrails Discard, approve typed-confirm, Home Coach chips.**  Branch `cursor/console-discard-approve-chips-1ffb`.  Designer discard hypothesis confirmed: Discard now runs one `discardAll` that clears `universeDraft`.  Approve cards/bulk resolve `row.executionMode ?? currentMode` and render the 409 phrase.  Home no longer toasts Approved on `busy`.  Coach chips prefill/focus the note.  Rollout: `docs/rollouts/2026-08-25-console-discard-approve-chips.md`.
