- **[Socratic.Trade][GROK] Mac TestFlight install-but-no-launch RCA + stable Xcode ship pin — IN PROGRESS 2026-08-11.** amfid Code=80 Authentication error on all TF iOS-on-Mac apps (ST/CT/UM) under macOS 27 beta; not ST-specific. Pin DEVELOPER_DIR to Xcode.app; docs + LSRequiresIPhoneOS + web-auth anchor harden. OpenRouter mgmt credit monitor landing same session.
...
- **[Socratic.Trade][GROK] OpenRouter credit monitor via management key + per-key limits — IN PROGRESS 2026-08-11 (landing now).** Redesign `/api/health` openrouterCredits: prefer OPENROUTER_ADMIN/MANAGEMENT key, check account /credits remaining AND any key limit_remaining exhausted/low; keep UptimeRobot keyword stable. Branch `grok/openrouter-mgmt-credit-monitor` rebased on main; landing this session. Redesign `/api/health` openrouterCredits: prefer OPENROUTER_ADMIN/MANAGEMENT key, check account /credits remaining AND any key limit_remaining exhausted/low; keep UptimeRobot keyword stable.
