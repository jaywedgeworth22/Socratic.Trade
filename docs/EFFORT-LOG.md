...
**[Socratic.Trade][CODEX] CI event-SHA checkout pin (branch `codex/ci-checkout-ref`, 2026-07-18) — IN PROGRESS.** Follow-up to shallow checkout recovery: lightweight jobs pin the event SHA to avoid broad ref traversal on the persistent runner. Owner: CODEX.

**[Socratic.Trade][CODEX] CI shallow-checkout recovery (branch `codex/ci-checkout-fast`, 2026-07-18) — IN PROGRESS.** Stacked follow-up to PR #1739: lightweight classify/security checks are avoiding full-history fetches that monopolize the single Coolify CI runner. Owner: CODEX.
- **[Socratic.Trade][CODEX] Coolify CI runner routing unblock (branch `codex/coolify-ci-runner-routing`, PR #1739, 2026-07-18) — LANDING.** GitHub-hosted `ubuntu-latest` jobs fail before assignment (`runner_id=0`, no steps/log blob). Coolify service `github-runner` had both Socratic containers exited; API restart recovered `socratic-deploy` and `socratic-ci` while production stayed healthy. Required PR checks and helper workflows now target only `[self-hosted, socratic-ci]`, preserving the deploy lane and serializing memory-heavy jobs; the failure/schedule-only Sentry reporter uses the separate `socratic-deploy` runner so CI-runner outages remain observable. Gitleaks' optional SARIF artifact upload is disabled because it failed after a clean scan when `/_work` was outside `/root`. TypeScript hit Node's default ~1 GiB ceiling and a 1536 MiB retry moved the failure to Next build, so the dedicated CI container is now capped at 3 GiB with a 2560 MiB Node heap while retaining low CPU shares/high OOM priority. Playwright's CI-only server-start timeout is 600 seconds after the low-CPU build compiled but exceeded the old 240-second limit; local stays 240 seconds. Codex autofix now admits bot-triggered work only for same-repo PR heads before checkout/write credentials/secrets reach the persistent runner. Coolify's `EPHEMERAL=1` runners had reused the same container filesystem under `restart: always`; both Socratic runners now clear only unmounted `/_work` before each registration, and the first fresh checkout/check-pin passed. Coolify production had drifted to branch `agent/ag-recovery-v48-migration`; restored it to `main` with auto-deploy enabled, without a manual deployment. After this lands, rerun #1728/#1733/#1735/#1736/#1737/#1738 so their existing auto-merge can fire.
