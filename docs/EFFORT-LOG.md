...
## Completed
- **Secure FMP Stable Endpoint Integration (AG, branch `agent/ag-fmp-transcripts`) — COMPLETED 2026-07-15.** Transitioned all FMP endpoints (real-time quotes, macro context, economic indicators, calendars, transcripts, news, DCF, financial scores, analyst ratings) to stable base paths. Routing goes through a central safe fetch utility with auto-scrubbing for secret key leaks on error and graceful 402/Restricted degradation. Verified that all type checks, unit tests, and Next.js production build pass cleanly.
