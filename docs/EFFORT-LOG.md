...
- **[Socratic.Trade][GROK] Mac runner TestFlight ship + iOS 17 + export compliance — IN PROGRESS 2026-08-12 (branch `grok/ios-tf-runner-fix`, PR #2656).** Phone had no ST update because (1) Mac runner only unsigned-compiled (PR #2648 still not on main) and (2) latest ASC builds 1.0.1/2 and 202608120212 were `MISSING_EXPORT_COMPLIANCE` (patched live). ST `IPHONEOS_DEPLOYMENT_TARGET` was 26.0. Adding `ios-ship.yml`, `ITSAppUsesNonExemptEncryption=false`, min iOS 17.0. Rebased onto origin/main 2026-08-12: kept Lato + LaunchBackground from #2667, unrolled glass TabView for Release archive type-check, added Sentry observer name so verify-hosted cannot fail the workflow-list fixture.
