...
## In Progress
- **Public trading-framework explainer doc + `/framework` page (CLAUDE, branch `claude/trading-framework-docs-713061`, worktree `.claude/worktrees/kind-chaum-b7bbb3`) — IN PROGRESS 2026-07-11.** Owner-requested: framework-level (not user/account-specific) explanation of the trading pipeline as a repo doc (`docs/trading-framework.md`, summary + detailed) plus a public page at `socratictrade.com/framework` with conceptual diagrams, hardened against AI/bot content extraction (robots disallow, noindex/noai headers, AI-crawler UA blocking, JS-required content render). Lands via PR; auto-deploys on merge.
