- **[Socratic.Trade][CURSOR] iOS wide-window layout + Admin tab + chrome (gear/bell) — COMPLETED (merged to `main`) 2026-08-22 (PR #3028, squash `a851a68d`).**  Adaptive regular-width layout (sidebar-adaptable TabView, larger settings/inbox sheets, native Admin sidebar).  Gear top-trailing on every tab.  Bell top-leading opens the notification inbox (price-alert composer deleted from Assets).  Admin Portal is a first-class tab for `isAdmin` only; WKWebView intercepts "Back to Console" instead of loading the website.  Merge head `04300371`: `verify` 32541767856 and `ios-build` 32541767843 succeeded.  Rematch onto #3031 kept five-section Activity + `NotificationsInboxView`; `ac9bafd5` dropped duplicate `NotificationHistoryRow`.  iOS ships only via TestFlight.  Rollout: `docs/rollouts/2026-08-21-ios-wide-layout-admin-tab.md`.
