...
- **[Socratic.Trade][CODEX] Usage telemetry v2 producer adoption (branch `codex/usage-telemetry-v2-20260721`, worktree `/Users/jay/.codex/worktrees/socratic-telemetry-v2`) — IMPLEMENTED / RECEIVER GATE CLEARED 2026-07-22.** Exact-pins immutable shared `v2.0.0` over HTTPS; fresh live events use strict v2 `schemaVersion`/`producerId`/`eventId`/`producerKeyRef` fields with typed ACK parsing. One-time HMR normalization retains buffered pre-v2 events; durable replay persists each ledger's first high-water mark before network I/O, drains that fixed pre-v2 window through the actual v1 envelope and its original receiver idempotency key across bounded passes, fails closed on unknown/corrupt cutover state, records `legacy-drained`, then advances to `v2-active` only after a newer strict-v2 row is acknowledged. Cold `GIT_SSH_COMMAND=false npm ci`, TypeScript, lint (zero errors), production build, 83 broader focused tests, final producer/replay/FMP regression set, and focused replay 12/12 pass. Usage-Monitor exact main `335723775ef0f8114ee1ca77b4716139018026dc` is committed live on Oracle; producer PR/deploy receipts follow. Rollout: `docs/rollouts/2026-07-22-usage-telemetry-v2-producer.md`.
