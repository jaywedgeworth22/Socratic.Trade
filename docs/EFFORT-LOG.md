- **[Socratic.Trade][AG] Resolve reviewer feedback on guardrail capabilities, market hours hints, unmanaged shorts, and stop fallbacks — IN PROGRESS 2026-08-26 (branch `antigravity/reviewer-fixes`).**  Pass live account capabilities into `mergeAccountCapabilities` in Guardrails page; align broker-specific `syntheticStopHoursHint` in `market-hours.ts` with executable order windows; fallback `shortTrailFallback` to `stopLossPct` when short stop loss is unconfigured; check effective stop distance configuration in `deriveUnmanagedShorts`.
