...
- **[Socratic.Trade][CLAUDE] SQLite busy_timeout headroom 30s->60s (branch `claude/sqlite-busy-timeout-headroom`) — READY FOR PR.** Root-caused Sentry SOCRATIC-TRADE-T ("Pinecone connection failed", 13 events/5 days, most recent minutes before this fix): the actual `reason` tag is `"database is locked"` (SQLITE_BUSY) hit during a RAG query's incidental local DB read, not a real Pinecone outage — Pinecone's own index status reports Ready throughout. PR #1728's 30s busy_timeout (2026-07-18) isn't absorbing contention during the current unusually heavy concurrent-write period (bulk RAG backfill/reindex + scheduler + burst ingest all writing the same prod SQLite file). One-line pragma bump to 60s; WAL already lets readers proceed during a writer, so this only affects genuinely write-contended callers. `src/lib/db.ts` only.
