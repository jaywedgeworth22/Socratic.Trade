...
- **[Socratic.Trade][GROK] UX PR-C1+C2 snapshot TTL cache + calculatePnl once — IN PR 2026-08-04 (branch `grok/ux-c-snapshot-pnl`).** ~10s in-memory TTL+singleflight for `getDashboardSnapshot` keyed by `(userId, accountNumber, connectedAccountId)`; invalidate on `setPolicy` + mobile command completion; assembly runs `calculatePnl` once per live/paper source and threads `PrefetchedFills.livePnl/paperPnl` into scorecards/tax/performance. Tests: `test/dashboard-snapshot-cache-pnl.test.ts`. Rollout: `docs/rollouts/2026-08-04-ux-c-snapshot-cache-pnl.md`.
