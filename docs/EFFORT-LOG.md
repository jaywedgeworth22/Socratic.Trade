- **[Socratic.Trade][CURSOR] Litestream restore drill (scratch only) — IN PROGRESS 2026-08-18 (branch `cursor/litestream-restore-drill-2cd9`).** Report only.  ASC B2 restore to scratch VERIFIED (4.9G, integrity ok, RPO ~48s).  Decrypt last-4 BLOCKED.  R2 weekly retain=1 NOT VERIFIED (`R2_ARCHIVE_KEEP_GENERATIONS=2`).  No product code.  No Coolify.  No rolling/zero-downtime.  Rollout: `docs/rollouts/2026-08-17-litestream-restore-drill.md`.
...
- **P2 Ops (9):** verify drawdown kill-switch (S), verify correlation gate (S), Litestream
  ...
  `docs/rollouts/2026-08-17-litestream-restore-drill.md`; decrypt last-4 still BLOCKED;
  R2 retain=1 still NOT VERIFIED), account-deletion table sync test (S), disk/WAL monitoring (S),
