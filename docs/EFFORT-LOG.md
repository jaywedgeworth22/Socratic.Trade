- **[Socratic.Trade][GROK] Review UX: approve speed + prices + Retry Red Team + agent controls + PWA off — IN PROGRESS 2026-08-16 (branch `grok/review-ux-parity`, worktree `~/apps/trading-grok-review-ux`, issue #2753).**  Full-universe scan removed from Approve.  Proposed/Now/Target/Delay on website and iOS.  Retry Red Team when critic failed.  State-aware Start/Stop.  `/mobile` redirects to `/console`.
