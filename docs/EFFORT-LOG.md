...
| 2026-07-23 | **Fix GitHub CI runner environment for cleanup-caches** — The recent retargeting of GitHub CI jobs to self-hosted runners caused `cleanup-caches.yml` to fail on `pull_request: closed` events because the Coolify runner does not have the `gh` CLI installed. Retargeted `delete-pr-caches` and `prune-stale-caches` to `ubuntu-latest`. | ANTIGRAVITY | **Completed** — Fix committed and PR ready to merge. | fix/cleanup-caches-runner |
