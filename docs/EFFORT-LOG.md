...
- **[Socratic.Trade][CURSOR] Pinecone daily-fuse deadlock + keep VIX backup — IN PROGRESS 2026-08-17 (branch `cursor/pinecone-write-deadlock-64c1`).** Owner correction: writes should still be happening; 15 WU / 1-text caps are a deadlock from local-MTD trial math, not the 2.5M fuse and not a spent trial. Do not treat HTTP 429 as a happy expected outcome. Yahoo VIX stays in the cascade; do not re-probe it while Cboe is serving. CT OpenRouter "files-endpoint prepaid minimum" is a leftover stored halt — ST credits check is ok and the owner has >$50 on OpenRouter. Do not repeat that prepaid diagnosis.
