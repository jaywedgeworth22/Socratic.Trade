...
- 2026-07-12 - **[codex-autofix] Honor HTTP-date Retry-After values in 429 handling (CLAUDE, PR #1475 `ag/troubleshoot-sentry`).** Codex review (P2) flagged that the PR's 429 handling only parses delta-seconds. Added Date.parse() fallback for the HTTP-date Retry-After format (RFC 7231 §7.1.3). Verify trio passes (349 files, 3896 tests, build clean). Thread resolved, auto-merge enabled. Rollout: docs/rollouts/2026-07-12-codex-triage-429-retry-after.md. State: **Completed 2026-07-12**.
