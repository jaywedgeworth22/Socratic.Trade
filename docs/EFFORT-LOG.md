- **[Socratic.Trade][CURSOR] Deploy freshness alert + isolate CT OCR load — IN PR #2796 2026-08-17 (branch `cursor/deploy-freshness-ocr-isolate-d4cf`, closes #2545).** 20-minute cron pages when oldest undeployed main commit is >1h (silent-freeze class).  Dry-run `docker update` CPU cap for CT OCR/scan workers; never restarts; never touches ST.  Remaining host constraint: Coolify/CT-repo limits + retry-on-255 are not settable from this repo.  Did not touch prod.
