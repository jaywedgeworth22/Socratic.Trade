...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, MERGED PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, READY PR #1429, branch `codex/strategy-lease-correctness`) — CURRENT-MAIN SCOPED GREEN / FINAL FULL GATE PENDING 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Approval executions use invocation-unique owners; sticky renewal loss and synchronous ownership proof fail closed; setup failure cannot leak the guard; completed autonomous results survive later loss; and `SCHEDULER_SINGLE_LEADER` is default-on. Review autofix `9d2ba1fb` moved the scheduler heartbeat after the leader gate but arrived from `github-actions[bot]`, so CI correctly refused the untrusted actor. CODEX merged current `main@7c01f87e` and committed trusted human follow-up `b19650ac`, tightening the autofix: broker health, tradability, initial review, and bump re-review now re-prove ownership before blocked/proposed persistence, while post-notification checks precede later side effects. A follower cannot refresh `scheduler:lastTick`; direct loss-during-tradability/review tests leave no proposal and make no broker call. Reconciliation also removed a stale `heartbeatTimer` reference from the newly merged broker-health early return so the authoritative `finally` performs cleanup. Current Node24 receipts after the latest main merge: focused 4 files/21 tests, touched lint 0 errors/33 inherited warnings, tsc clean, diff-check clean. The prior full 3,764-test/build gate predates these review changes; serialized full test/build, final push, hosted rerun, merge, and production verification remain. No CI trust-policy, provider-boundary, production-config, merge, or deploy mutation is included.
