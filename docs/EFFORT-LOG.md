- **[Socratic.Trade][GROK] UX PR-C3 scan table virtualization (TableVirtuoso) — IN PROGRESS 2026-08-04 (branch `grok/ux-c3-scan-virtuoso`).** Virtualize desktop Market Scan candidates with `react-virtuoso` TableVirtuoso; preserve sticky symbol column, sort, column chooser, mobile cards. Program §PR-C3. Rollout: `docs/rollouts/2026-08-04-ux-c3-scan-virtuoso.md`.
...
- **[Socratic.Trade] UX Wave C perceived speed (snapshot TTL cache, P&L once, scan virtuoso, React.memo) — PARTIAL — PR-C3 IN PROGRESS (GROK); remaining C slices UNASSIGNED.** Program §Wave C. Hotspot: `src/lib/dashboard.ts`.
