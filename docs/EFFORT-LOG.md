...
- **[Socratic.Trade][CURSOR] Abort paid Finnhub wave on gather timeout — IN PR #3018 2026-08-21 (branch `cursor/gather-timeout-abort-0388`).**  Leftover after #3013 `eef81eb6`: first gather-abort dropped `AbortSignal` on paid/scarce waves so Finnhub's paced queue kept running after the 8m deadline.  Rematched onto `origin/main` (#3008 / #3019); rematch left conflict markers in the rollout Verification block — resolved.  Parent owns Coolify.  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
- **[Socratic.Trade][CURSOR] Abort abandoned strategy gather + last-good tape — COMPLETED/MERGED 2026-08-21 (PR #3013, squash `eef81eb6`).**  Abort the controller, honor the signal in cascade/enrichment, reuse last completed MarketScan + 45s quote refresh so Green can start.  Follow-up leftover is #3018 (paid Finnhub wave).  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
