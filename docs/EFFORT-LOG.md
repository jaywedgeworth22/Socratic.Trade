...
- **[Socratic.Trade][CODEX] Shared-package pin-check queue unblock (replacement PR for #1780, branch `codex/check-pin-always-on-fix`) — IN PROGRESS 2026-07-22.** Removed the pull-request path filter so every PR receives a pin-check status, and added Node 24 before the comparison script. Focused workflow-safety tests, ESLint, TypeScript, YAML parsing, and diff checks pass; hosted `verify-hosted` passed on an earlier head, and subsequent main synchronizations have advanced the branch again. This row intentionally does not pin a supersedable SHA: refresh PR #1890 before acting; current-head `gitleaks`, `check-pin`, and required `verify` are queued. Auto-merge is intentionally held off until current-head verification passes and every review thread is resolved. Close #1780 only after this replacement merges.
...
- **[Socratic.Trade][CODEX] Usage telemetry v2 producer adoption (branch `codex/usage-telemetry-v2-20260721`, worktree `/Users/jay/.codex/worktrees/socratic-telemetry-v2`) — IMPLEMENTED / RECEIVER GATE CLEARED / LOCAL GATE PASS 2026-07-22.** Exact-pins immutable shared `v2.0.0` over HTTPS; fresh live events use strict v2 `schemaVersion`/`producerId`/`eventId`/`producerKeyRef` fields with typed ACK parsing. One-time HMR normalization retains buffered pre-v2 events. Durable replay now performs one synchronous `BEGIN IMMEDIATE` direct-v2 cutover for all LLM/RAG/provider ledgers before any await or producer startup, seeds cursors to current high-water, records skipped pre-v2 row counts, keeps seeded boundaries exclusive until a newer strict-v2 ACK, halts corrupt-state lanes without network/state mutation, and replaces stale v2 HMR workers with v3 direct-v2 state. No legacy sender remains. Owner-authorized tradeoff: the bounded pre-v2 remainder is not replayed, avoiding duplicate money at the cost of possible loss for any row not already live-pushed under v1. Node 24 focused push/replay/startup tests pass 52/52 with TypeScript, scoped ESLint, and diff-check clean. Usage-Monitor exact main `335723775ef0f8114ee1ca77b4716139018026dc` is committed live on Oracle; producer PR and deploy receipts follow. Rollout: `docs/rollouts/2026-07-22-usage-telemetry-v2-producer.md`.
