...
## 🚧 In Progress
- **[Socratic.Trade][CODEX] Coolify CI runner routing unblock (branch `codex/coolify-ci-runner-routing`, 2026-07-18) — LANDING.** GitHub-hosted `ubuntu-latest` jobs are failing before runner assignment on every required open-PR check (`runner_id=0`, no steps/log blob). Verified repo runners: `socratic-ci` and `socratic-deploy` are online on the Coolify Hetzner box; `trading-live-mac` is offline. Routing required PR checks and PR-visible helper workflows to `[self-hosted, socratic-ci]`, then rerun #1728/#1733/#1735/#1736/#1737/#1738 so their existing auto-merge can fire.
