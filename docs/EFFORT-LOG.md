- **[Socratic.Trade][CURSOR] Manual Run once must not wait behind ROIC/FTS — IN PR #2848 2026-08-18 (branch `cursor/getaccounts-loop-budget-befc`).** VERIFY_FAIL: `test/alpaca-mcp.test.ts:214` hung 60s under the 16s live wait + never-settling first call.  Mock a short `alpacaAccountReadBudgetMs` in that test.  Sweep: same-process stall is not "Process restarted" (Roth `b3b83913` on `4abfb7fa`).  Live-path ROIC/FTS pause kept.  Do not skip the test.  Do not merge/deploy.  Rollout: `docs/rollouts/2026-08-18-getaccounts-loop-budget.md`.
...
- **[Socratic.Trade][CURSOR] iOS Scan last-good on 503 refresh — IN PR #2850 2026-08-19 (branch `cursor/ios-scan-last-good-503-b104`).** ASC: testers on TF 1.0.68 `581467e1` already have #2830+#2831.  Live `/api/scan` still 503s without names (`4abfb7fa` path; process now `c55c2e64` #2848, scan files unchanged).  That client has no snapshot last-good and times out at 25s vs 35s budget.  Seed-first backend is the 1.0.68-compatible fix.  Do not merge/deploy/bounce/second TF.  Do not touch #2848/#2849/#2841/#2840.  Rollout: `docs/rollouts/2026-08-19-ios-scan-last-good-503.md`.
