...
- **[Socratic.Trade][KIMI] Macro feed resilience + Unknown-side regime-flip suppression (branch `agent/kimi-lane`) — Implemented 2026-07-29.** Keyless VIX cascade (Yahoo -> Cboe; honestly 2 lanes after verifier live-probes killed Stooq/Nasdaq/Yahoo-v7 tier-3 candidates) behind the shared per-lane circuit breaker (60s trip + half-open probe via api-circuit-breaker/api_health_log); checkRegimeFlip holds last-known regime on feed outage (no flip/dirty/event, once-hourly macro_feed_unavailable diagnostic, recovery announces real changes with existing semantics); throttle marker registered in account-deletion ownership registry. Finding: macro chain never used FMP — flap driver was the single Yahoo lane. 9 new tests; tsc/lint/vitest(5387)/build all green. Landed via PR from agent/kimi-lane (squash, auto-merge). Rollout: docs/rollouts/2026-07-29-macro-feed-resilience.md.
- **[Socratic.Trade][AG] Mobile PWA Account Switcher touch & layout fix — COMPLETED 2026-07-29.** Fixed account switching failure on mobile PWA by constraining `ScopeSelector` dropdown width (`w-[min(calc(100vw-48px),360px)]`), replacing backdrop `div` with a full-screen `<button>` for iOS Safari / PWA touch compatibility, and dismissing menu immediately upon switch activation.
- **[Socratic.Trade][KIMI] Playwright Smoke ENCRYPTION_KEY CI fix (branch `kimi/smoke-encryption-key`) — In Progress 2026-07-29.** Main-branch smoke failing since 2026-07-29 07:08Z: production boot guard (`assertEncryptionKeyConfiguredInProduction`) refuses `next start` without a 64-char hex ENCRYPTION_KEY; fix adds a fixed test-only key to the Playwright webServer env (throwaway smoke DB). PR open, auto-merge armed.
- **[Socratic.Trade][AG] CI PR merge blocker root-cause diagnosis & fix — COMPLETED 2026-07-28.** Fixed `gitleaks` job failure by adding `/tmp/gitleaks*` cleanup pre-step in `security.yml` and fixed `verify-hosted` mock hydration error in `test/milestone-4-challenger.test.ts`.
- **[Socratic.Trade][AG] Minimum account equity threshold strategy gate ($10) — COMPLETED 2026-07-28.** Added `MIN_STRATEGY_ACCOUNT_EQUITY = 10` gate in `runStrategyOnce()` to skip LLM strategy runs on empty/unfunded accounts (< $10 total equity/buying power/cash).
- **[Socratic.Trade][AG] Daily Notional Cap zero-balance & buying power clamping fix — COMPLETED 2026-07-28.** Updated `resolveDailyOpeningCap` and `deriveSpend` to recognize non-negative `$0` spend limits; zero-balance/buying-power accounts now resolve to `$0.00` daily notional limit while funded margin accounts use available leverage buying power.
...
- **[Socratic.Trade][KIMI] OSS-lessons program: docs/oss-lessons.md + task brain / cron journal — COMPLETED 2026-07-29 (PR #2272 merged to main; auto-deploys to prod).** Owner-directed. Unified `task_journal` SQLite ledger journaling every scheduler lane (OpenClaw Task Brain / Hivekeep cron-journal pattern, migration v62) + research doc mapping OSS lessons (freqtrade, Lean, OpenBB, Jesse, qlib, TraderHarness, nofx, OpenClaw, Hivekeep) to concrete repo changes. Local gates: tsc/eslint/37 targeted tests green; full suite+build delegated to verify CI (host load 90-600 blocked local full runs) — verify went green on the CI fleet. Branch `kimi/task-brain-cron-journal`. oss-lessons.md §5/§8/§9 status updates land via docs-only PR #2289 (the commit was pushed after GitHub auto-deleted the squash-merged branch and had to be cherry-picked).
...
## In Progress
- **[Fleet][GROK] Shut down Oracle Actions runners; all repos GitHub-hosted CI — IN PROGRESS 2026-07-29 (landing).** ST+CT workflows → `ubuntu-latest`; Oracle `oracle-runner-*` containers stopped; GH runner regs deleted (last ST runner may need job drain). Branch `agent/grok-github-hosted-ci`.
- **[Socratic.Trade][GROK] Unstick open PRs (#2256 typecheck + CI queue) — IN PROGRESS 2026-07-29 (fix pushed pending CI).** Restored `nonNegativeFinite` + policy-caps tests; PR #2256 typecheck was sole hard fail. #2257/#2259/#2265 merge-tree CLEAN, auto-merge armed, CI queue/in-progress. Rollout: `docs/rollouts/2026-07-29-pr-2256-nonnegative-finite-restore.md`.
