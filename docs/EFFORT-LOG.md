...
- **[Socratic.Trade][AG] Prevent deleted API keys from auto-reseeding on server startup (branch `agent/antigravity-key-deletion-tombstone`) — COMPLETED 2026-07-21.** Implemented `__DELETED__` tombstone row handling for `LOCAL_ENV_MIGRATION_SERVICES` in `src/lib/db-api-keys.ts` and `src/lib/st-primary-bridge-writer.ts` so deleted LLM keys (Gemini, DeepSeek) stay deleted across server reboots. Unit test added, verification green, PR #1856 created and auto-merged.
