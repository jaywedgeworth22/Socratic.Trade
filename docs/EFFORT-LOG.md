...
## 🚧 In Progress
- **[P0][Money path] Synthetic stops resolve the UI-active account instead of the stop policy's account (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — IMPLEMENTED / FOCUSED GREEN, AWAITING FULL GATE.** The owned policy account now authoritatively rebinds account id/number, broker, and execution mode before any broker work; missing/deleted/foreign targets fail closed. Multi-account, spoofed-routing, and ownership regressions pass (47/47).
- **[P1][Mobile truth] Preserve failed/unknown initial dashboard state (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — IMPLEMENTED / FOCUSED GREEN, AWAITING FULL GATE.** Initial failure renders an explicit retryable unavailable state and no fabricated market/portfolio/account emptiness.
- **[P2][Mobile commands] Preserve command input and enforce readiness (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — IMPLEMENTED / FOCUSED GREEN, AWAITING FULL GATE.** Watchlist/alert drafts clear only after accepted HTTP responses and only if unchanged; run/start require ready account+universe, close-only/proposals require an account, durable stale command rows do not cage controls, and STOP remains a protective override.
- **[P1][Mobile reliability] Sequence/single-flight snapshot refreshes and fail stale commands closed (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — CORE FIX IMPLEMENTED / FOCUSED GREEN; SSE FALLBACK POLLING REMAINS.** A 45-second abort deadline plus coalesced trailing refresh prevents overlap/out-of-order overwrite; refresh errors mark truth stale and pause new commands, while focus/visibility/online recover. Health-aware polling during an SSE outage remains unimplemented.
- **[P1][OAuth integrity] Robinhood exact state consumption and trusted callback binding (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — IMPLEMENTED / FOCUSED GREEN, AWAITING FULL GATE.** Canonical 32-byte base64url state is exact-matched and atomically consumed (no SQL LIKE); callback origin ignores forwarded host; optional binding verifies the Auth.js cookie directly. Wildcard, concurrency, origin, and session tests pass.
- **[P0][Policy correctness] Bind account-scoped edits to their originating account and guard
  scope switches (CODEX, branch `codex/app-wide-audit-20260711`, 2026-07-11) — IMPLEMENTED / ADVERSARIAL FIXED / FOCUSED GREEN, AWAITING FULL GATE.**
  Account writes carry an owned origin id; prompt+policy validation/persistence is atomic; same-tab
  card writes serialize; queued-save busy state is truthful; account editors remount; scope switching
  uses DirtyGuard and a one-event unload bypass only after activation succeeds, preventing stale-A/
  active-B split-brain. Foreign-target, delayed-switch, partial-write, and queue regressions pass.
- **Whole-app current-main issue audit and prioritized remediation (CODEX, branch
  `codex/app-wide-audit-20260711`, worktree
  `/Users/jay/.codex/worktrees/socratic-app-wide-audit`, 2026-07-11) — IN PROGRESS.**
  Recorded 34 reproducible findings (8 P0 / 18 P1 / 8 P2) across production UI/runtime, API/security,
  trading/data correctness, and stale-board truth. Five findings are implemented and one additional
  mobile reliability row has its core race fixed; combined focused Node24 result is TypeScript green,
  touched lint 0 errors/6 inherited warnings, and 6 files/85 tests. Full gate/PR remains next.
...
- **[P1][Congress ingest] Webhook/SSE delivery can acknowledge and permanently lose failed events — PLANNED 2026-07-11, unclaimed.** Persist trade data before dedupe/checkpoint, return a retryable batch failure, checkpoint SSE only after successful apply, persist the cursor, add heartbeat/idle recovery, and cover persistence-failure redelivery.
- **[P1][Public ingress] Webhooks permit memory/durable-disk denial of service and unsafe proxy-IP trust — PLANNED 2026-07-11, unclaimed.** Add streaming body limits for chunked/lying lengths, trusted-proxy address resolution, and bounded/coalesced rejection telemetry with retention for Congress and TradingView ingress.
- **[P1][Cross-app auth] App A's bearer securities-import route is unreachable behind production middleware — PLANNED 2026-07-11, unclaimed.** Add an exact self-authenticated middleware exemption, preserve handler-default-closed bearer validation, and test middleware-to-handler composition.
- **[P2][Admin auth contract] Documented token-only production admin access is blocked by middleware — PLANNED 2026-07-11, needs contract decision.** Either admit exact `x-admin-token` requests Edge-safely or retire the alternate-auth claim and explicitly require app identity plus token; add end-to-end middleware tests.
- **[P1][Health truth] Health/readiness can claim writable storage while SQLite is read-only or disk free space is exactly zero — PLANNED 2026-07-11, unclaimed.** Add a rollback-safe SQLite write canary, represent storage measurement failure separately from zero bytes, and fail readiness when active trading cannot persist.
- **[P1][Broker endpoint trust] Arbitrary Alpaca endpoints enable SSRF and forwarding of stored broker credentials — PLANNED 2026-07-11, unclaimed.** Default to canonical hosts; require explicit confirmed custom endpoints with HTTPS/public-network validation plus DNS/redirect revalidation before sending credentials.
- **[P1][Scheduler reliability] Unbounded broker I/O plus reentrant interval ticks can multiply hangs — PLANNED 2026-07-11, unclaimed.** Add whole-tick singleflight/skip-overlap and broker request deadlines so a never-settling health/order call cannot spawn duplicate work or keep the lease falsely fresh.
- **[P1][Account onboarding] Rejected multi-account Tradier onboarding can leave an active credential row — PLANNED 2026-07-11, unclaimed.** Probe/validate ambiguity before persistence or transactionally compensate failure; assert rejected connects leave no row or active-pointer mutation.
- **[P2][Notification security] Webhook URLs permit SSRF and are persisted/audited as plaintext secrets — PLANNED 2026-07-11, unclaimed.** Encrypt/redact stored endpoints, exclude secrets from policy audit payloads, and enforce HTTPS/public-network plus redirect/DNS validation at save and delivery.
- **[P2][Content security] Admin transcript Markdown auto-fetches untrusted remote images — PLANNED 2026-07-11, unclaimed.** Reuse the console's safe renderer or override generic Markdown images as non-fetching links; add a DOM regression proving model text creates no `<img>`.
- **[P2][Abuse controls] Paid test notifications have no rate or spend limit — PLANNED 2026-07-11, unclaimed.** Add per-user/IP throttles, concurrent singleflight, channel-aware debit/ceiling checks, and retry-safe test coverage.
- **[P2][Public ingress] Massive flatfile retrieval has no request, decompression, or response bound — PLANNED 2026-07-11, unclaimed.** Bound source bytes, decompressed rows/time, and response size; stream/abort oversized work and meter the route.
- **[P2][Resource bounds] Ticker-logo proxy has unbounded upstream buffering and persistent cache growth — PLANNED 2026-07-11, unclaimed.** Enforce content-type/byte/time limits and a cache quota/eviction policy with hostile-upstream tests.
- **[P1][Policy concurrency] Concurrent policy PUTs from separate tabs/processes can still lose sibling fields — PLANNED 2026-07-11, unclaimed.** Current client serialization is tab-local and server validation awaits between read/merge/write. Re-read and merge the sparse patch inside an account-keyed server critical section/transaction, or add versioned compare-and-swap with conflict retries.
- **[P0][Money path] Broker-health admission gates skip protective and reconciliation maintenance — PLANNED 2026-07-11, unclaimed.** `scheduler.ts` and `strategy.ts` return on low equity or repeated uncertainty before pending-fill/placing-intent reconciliation, stale-exit replacement, and synthetic-stop work. Run maintenance unconditionally and gate only new strategy/LLM/order generation; cover low-equity and uncertainty-deadlock cases.
- **[P0][Money path] A first partial fill permanently ends reconciliation — PLANNED 2026-07-11, unclaimed.** `strategy-execution.ts` marks `partially_filled` broker responses as fully `filled`, while `db-fills.ts` reselects only `pending_reconciliation`; later fills are lost from holdings/P&L. Preserve a reconcilable partial state and apply only fill deltas until terminal quantity is reached.
- **[P1][Money path] Red Team review of an inactive account uses the active account's doctrine/environment — PLANNED 2026-07-11, unclaimed.** `red-team.ts` accepts a policy override but independently loads the active account and unscoped prompt. Thread the evaluated account through doctrine, execution-mode, and prompt reads.
- **[P1][Money path] Post-mortems use the active account's fill source for inactive-account policies — PLANNED 2026-07-11, unclaimed.** `post-mortem.ts` receives a scoped policy but resolves fills through the active account. Make the policy account authoritative and cover cross-account runs.
- **[P0][Risk caps] Daily/hourly order caps are measured from proposal creation time, not placement time — PLANNED 2026-07-11, unclaimed.** `db-execution.ts` filters `trade_proposals.created_at`; delayed approvals/retries can evade the intended placement window. Persist/query an attempt/placement timestamp and migrate conservatively.
- **[P0][Risk caps] `placing` intents reserve neither notional nor order count — PLANNED 2026-07-11, unclaimed.** Cap queries include only placed/paper rows, so concurrent or uncertain in-flight orders can each pass the same remaining budget. Include unresolved placing intents in reservations with idempotent release/reconciliation.
- **[P0][Money path] Connected-account deletion can race an in-flight order and erase its intent/lock — PLANNED 2026-07-11, unclaimed.** The account route deletes directly and `db-api-keys.ts` purges execution state without a scheduler/order fence. Require a durable draining state, block new runs, reconcile/cancel outstanding work, then delete.
- **[P0][Money path] Nonthrowing rejected/canceled stale-exit replacements are recorded as successful — PLANNED 2026-07-11, unclaimed.** `order-replacement.ts` treats any returned replacement as pending/replaced and only handles thrown failures. Validate accepted broker status/order id before advancing replacement state and persist definitive rejection/cancellation.
- **[P1][Accounting] Bounded fill/snapshot readers return the oldest rows and eventually freeze current accounting — PLANNED 2026-07-11, unclaimed.** `db-fills.ts` applies ascending limits (100 snapshots/500 fills), while performance expects a current window. Select newest rows in a subquery then restore chronological order; add histories beyond both limits.
- **[P1][Loading/a11y] Legitimate slow dashboard loads trigger a false 15-second failure while remaining screen-reader silent — PLANNED 2026-07-11, unclaimed.** The same in-flight request can complete after the watchdog has shown an error/retrying state, and loading content is `aria-hidden`. Tie timeout/error state to request identity, cancel superseded work, and expose a polite live status.
- **[P1][Accessibility] Tooltip help is mouse-only and lacks `aria-describedby` semantics — PLANNED 2026-07-11, unclaimed.** Give triggers keyboard focus, stable ids, hover/focus parity, Escape dismissal, and programmatic association.
- **[P1][Accessibility] Command palette lacks a focus trap/return and complete combobox semantics — PLANNED 2026-07-11, unclaimed.** Trap focus inside the modal, restore it to the opener, wire listbox/option ids and active descendant, and cover keyboard-only use.
- **[P1][Data loss prevention] Browser back/forward navigation bypasses `DirtyGuard` — PLANNED 2026-07-11, unclaimed.** Intercept or safely recover popstate-driven console navigation so dirty account-scoped forms receive the same confirmation as links and account switches.
- **[P2][Accessibility] Selected-state and form-label relationships remain incomplete — PLANNED 2026-07-11, unclaimed.** Audit console/mobile segmented controls, tabs, and settings inputs for `aria-current`/`aria-selected`, `label`/`htmlFor`, names, descriptions, and error association; add targeted axe/keyboard coverage.
