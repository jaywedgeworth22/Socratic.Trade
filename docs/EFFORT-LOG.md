...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, MERGED PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, READY PR #1429, branch `codex/strategy-lease-correctness`) — CURRENT-MAIN SCOPED GREEN / FINAL FULL GATE PENDING 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Approval executions use invocation-unique owners; sticky renewal loss and synchronous ownership proof fail closed; setup failure cannot leak the guard; completed autonomous results survive later loss; and `SCHEDULER_SINGLE_LEADER` is default-on. Review autofixes `9d2ba1fb` and `3bfd3122` arrived from `github-actions[bot]`, so CI correctly refused their actor; no trust-policy change was made. CODEX merged current `main@7c01f87e` and tightened both patches: approval and autonomous paths re-prove after broker/setup scans, quotes, stale notifications, tradability, initial review, and bump re-review before blocked/proposed/snapshot/breaker writes; budget and volatility awaits re-prove before later audit/policy changes; notification-time loss cannot demote authority. Direct regressions cover approval/autonomous tradability and review loss plus setup-read loss before scan/snapshot. A follower cannot refresh `scheduler:lastTick`; #1430's obsolete `heartbeatTimer` cleanup is removed; the bot's unrelated one-line lockfile normalization was reverted. Current Node24: focused 7 files/38 tests, touched lint 0 errors/34 inherited warnings, tsc clean, diff-check clean. Hosted full verify/security/smoke were green at prior human head `ed3793e3` but predate this second reconciliation; serialized full test/build, trusted push, hosted rerun, merge, and production verification remain. No provider-boundary, production-config, merge, or deploy mutation is included.
