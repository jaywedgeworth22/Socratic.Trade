- **[Socratic.Trade][GROK] Mobile tab bar: close 80% chrome gap + continuous surface under URL — IN PROGRESS 2026-08-05 (branch `grok/mobile-tabbar-gap-and-chrome-bg`).** Measure gap above Safari floating URL chrome; shift `.con-tabbar` down 80%; solid surface underlay for remaining gap + under translucent chrome so colder `--con-bg` grey no longer flashes around the address bar. Files: `app/console/components/nav.tsx`, `app/console/console.css`.
