...
## In Progress
- **Account-relative risk + final-size/lifecycle truth follow-up (CODEX, branch `codex/account-relative-risk-review-fixes`, claimed 2026-07-13) — IN PROGRESS / HOSTILE REVIEW FIXED / CURRENT MAIN INTEGRATED.** Closes PR #1561's post-merge findings and the deeper money-path audit: all four legacy policy stores migrate once while later explicit dollar caps (including `$500` and values above `$500k`) persist; Guardrails Dollar/Percent mode follows the active account; exact Green/sizing receipts survive lifecycle writes; final applied override truth drives evidence/UI; and GPT-5.6 catalog/Responses effort invariants are tested. A successful broker-minimum bump refreshes deterministic arithmetic and reruns Red once on the exact risk-adding order; half-size is down-only, reject/unavailable/unplaceable half requires one owner decision, exits are exempt, and reason-scoped holds preserve rationale-collapse/override gates. Proposal intent + initial Socratic case commit atomically before broker submission; lifecycle transitions sync transactionally; uncertain submissions remain `placing`; same-decision vector writes serialize and re-read current SQLite truth; approval Green copy excludes appended Red prose. Fresh adversarial review found 2 P1/3 P2 issues and all five now have fixes/regressions. `origin/main@86971ec4` is integrated; Node 24 TypeScript and 8 files/68 focused tests pass; repository lint is 0 errors/458 inherited warnings. Re-review, authoritative full gate, ready PR, original-thread resolution, and exact production verification remain. No broker-protective-stop, host, secret, or corpus mutation.
