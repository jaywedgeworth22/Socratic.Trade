...
## In Progress
- **Decision-detail dissent content deduplication (CODEX, ready PR #1593, branch `codex/decision-dissent-dedup`, 2026-07-14) — COMMIT `40853f3e` LOCAL-GREEN / LAND GATE QUEUED / KEEP OUT.** The canonical Red Team card now preserves the explicit approve-at-half label and rejection status while duplicate rationale rows remain suppressed, addressing exactly `PRRT_kwDOS7mOVM6Q6zNL` and `PRRT_kwDOS7mOVM6Q6zNO`. Node 24: focused 2 files / 24 tests, lint, TypeScript, full 369 files / 4,135 tests, production build with TypeScript + 32 static pages, and diff-check all pass. Mandatory `scripts/land.sh` is queued behind the active FMP full gate; final-head hosted checks/review, replies, and thread resolution follow. Display-only; auto-merge is disabled, and production is unchanged.
