...
- **Consolidated Improvements and Codex PR #1611 Review Resolution (AG, branch `agent/ag-reconciled-improvements`, worktree `/Users/jay/apps/trading-antigravity`) — IN PROGRESS.** Reconciled outstanding branch changes off main: FMP stable endpoints, PR #1611 transcripts hardening (redacting logs, exit codes, fiscal quarters, 403 mapping, transient error caching, Pinecone write budget doubling), browser tab title removal, Strategy fallback scoping, and SEC Ingest validation. All local checks (lint, tsc, tests, and build) pass cleanly. Landing to main next.
...
- **[Socratic.Trade][CODEX] FMP coverage, market-scan reliability, and non-scan ticker-sheet enrichment (branch `codex/fmp-market-data-reliability`, worktree `/Users/jay/apps/trading-codex-fmp`, claimed 2026-07-15) — CURRENT-MAIN RECONCILED / HOSTILE REVIEW REMEDIATED / FOCUSED GREEN / LAND GATE PENDING.** Production RCA found three July 14 interactive 25-second timeouts caused by a cold 150-symbol all-provider cascade; Finnhub can enqueue 750 paced requests, while the old route did not cancel timed-out work or coalesce retries. Interactive scans now keep deep ingestion off-request, accept only 24-hour slow-only seed facts, single-flight identical refreshes, bound Nasdaq, and show an explicitly stale last strategy scan rather than blanking on outage. FMP uses stable header-auth profile + insider-search endpoints, maps the broader ratios/profile field set, and no longer duplicates Senate data owned by Congress.Trade. Out-of-scan ticker sheets have a current identity/quote floor, timestamp-aware rich arbitration, per-user/symbol coalescing, and a dynamic company-name header; exact LRCX browser QA passed with zero console errors. Reconciled `origin/main@ede902f5` while preserving PR #1586's durable FMP dispatch and default-off rights-gated transcript producer. Node 24: lint and TypeScript clean, 9 files / 201 focused tests, diff-check green; the earlier pre-main full gate passed 4,179 tests and build. Current-main full land gate, ready PR, hosted verification, protected merge, and exact production verification remain.
...
- **[Socratic.Trade][CODEX] Infisical JSON-export production compatibility (branch `codex/infisical-export-json-compat`, 2026-07-14) — CORRECTIVE IMPLEMENTED / FOCUSED GATES GREEN / FULL LAND GATE NEXT.** PR #1594 merged as `48bd191c`, but Coolify deployment `trxqzfunxctpy440ozbyt5if` failed health and rolled back cleanly to healthy prior SHA `2dabc7f8`. Pinned Infisical CLI v0.43.98 emits `export --format json` as an array of secret records; the merged parser incorrectly required a flat object. The corrective parser accepts only validated non-duplicate `{ key, value }` records and copies no metadata; malformed shapes, entries, keys, values, and NULs fail closed without raw output. Node 24 focused result: 37 tests plus scoped ESLint, standalone TypeScript, and diff-check green. Remaining: canonical full gate, ready PR, hosted checks, merge, automatic rollout, and exact production SHA/health verification.
