...
- **Framework Models card truth fixes — Proposer blank-select display + Reviewer "inherits
  ...
  Landing via `scripts/land.sh` as a standalone PR off `origin/main` (PR #1346's branch was already
...
- **Per-team reasoning levels + rotation auto-effort + usage/Learning-Review links (CLAUDE) —
  COMPLETED 2026-07-10, PR #1346 squash-merged to `main` (`c7a2fa95`).** New account-scoped
  `TradingPolicy.redTeamReasoningEffort` (mirrors `redTeamLlmModel` naming); legacy
  `llmReasoningEffort` = the PROPOSER's; reviewer falls back until explicitly set via the single
  helper `resolveReviewerReasoningEffort` (src/lib/llm-request.ts), wired at red-team.ts /
  strategy-tuning.ts / the AI-review panel. `validatePolicy` rejects a gpt-5.5+high combo on
  EITHER team, naming the team. Framework UI: per-seat reasoning selects (shown only when that
  model supports it), curated per-model advice from NEW `src/lib/model-reasoning-recommendations.ts`
  (gpt-5.5 interactive-high rule surfaced BEFORE save; High disabled in-select), reviewer
  "Same as proposer (…)" inherit option (for reasoning EFFORT — a real fallback); rotating seats
  ...
  Gate green (tsc / lint 0-err / 3383 tests / build) + live browser smoke of all four items. Rollout:
  `docs/rollouts/2026-07-10-per-team-reasoning.md`. Two pre-existing UI-copy follow-ups (Proposer
  blank-select display quirk + Reviewer "inherits proposer" false hint) logged and fixed separately
  — see the In Progress entry for `claude/models-card-truth` above.
