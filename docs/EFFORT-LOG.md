- **[Socratic.Trade][GROK] Strategy-run model slugs + Pinecone lease-lost mislabel — IN PROGRESS 2026-08-17 (branch `grok/strategy-run-model-slugs`, worktree `~/apps/trading-grok-model-slugs`).** Owner screenshot 8:34–8:38am CT: Green failed on OpenRouter `mistralai/mistral-medium-3.5` (invalid; wire slug is `mistral-medium-3-5`) and `gpt-5.4-nano` 400 Provider returned error (slug is correct; OpenAI endpoint status -2 / no `max_completion_tokens`, Azure is healthy). Pinecone + OpenRouter rerank "connection failed" are dispatch-lease-lost during the 13:34Z UptimeRobot blip, not vendor outages. Last-2d Pinecone otherwise is the known WU-fuse park.
- **[Socratic.Trade][GROK] Flip VECTOR_ASOF_STRICT=on — IN PROGRESS 2026-08-16 (branch `grok/asof-strict-on`, worktree `~/apps/trading-grok-overlay-poly`).** Infisical prod set `on` (was `off`, len=3). Coolify restart `fwqascvivxvc7342hkw3aizk` finished. Health 200. Live desk still omits asOf.
- **[Socratic.Trade][GROK] ROIC Individual harvest (breadth then deepen then archive) — IN PROGRESS 2026-08-16 (branch `grok/roic-harvest`, worktree `~/apps/trading-grok-roic-harvest`).**  Panel already approved rev 3 (#2760) — no second panel.  Local-first persist to `earningscalls_transcripts`; latest-first universe; 20q deepen on held/watchlist/technical; archive remaining universe locally.  Pinecone fuse no longer stops ROIC fetch.  #2750 single-flight on main.
- **[Socratic.Trade][GROK] ROIC single-flight (stop 22-min crash loop) — COMPLETED 2026-08-16 (merged #2750 `1867addd`).**  #2741 due check restacked a universe walk every 60s; 714 running journal rows; Coolify `last_restart_type=crash` every ~22 min.  Single-flight + start watermark + incremental cursor.
- **[Socratic.Trade][GROK] Rotation fail-closed + Red timeout + Alpaca penny 422 + RAG ingest unstick — IN PR #2751 2026-08-16 (branch `grok/48h-money-path-ingest`, issue #2749).**  Drop known-dead slugs on /models/user timeout.  Red uses Green capturing timeout.  Alpaca >=$1 rounds to $0.01.  ROIC single-flight + requeue budget-misclassified SEC dead letters.  Worker tick cap 5 across all jobs (unstick 2156 Aug-10 facts_extracted).  Sibling #2748 owns the daily WU fuse.
- **[Socratic.Trade][GROK] ROIC single-flight (stop 22-min crash loop) — IN PROGRESS 2026-08-16 (branch `grok/roic-singleflight`, worktree `~/apps/trading-grok-ops-roic`).**  #2741 due check restacked a universe walk every 60s; 714 running journal rows; Coolify `last_restart_type=crash` every ~22 min.  Single-flight + start watermark + incremental cursor.  L1 keep-400 shrink still running.  FilingAPI still owner Plus.
- **[Socratic.Trade][GROK] Overlay regime label/enum match hotfix — IN PROGRESS 2026-08-16 (branch `grok/overlay-regime-match`, worktree `~/apps/trading-grok-overlay-poly`).** #2743 live apply passed `determineMarketRegime` labels into an enum router, so only `any` overlays fired. Pass `classifyMarketRegime().regime`, normalize labels in the router, contain overlay text as coach, scan as untrusted.
...
- **[Socratic.Trade][GROK] iOS Proposals For Review: proposed / live / target / delay + expert-panel target debate — COMPLETED/DEPLOYED 2026-08-16 #2740 `c1db7d12`.**  Home tile title is "Proposals for Review" above the count, nothing below.  Prompt `agentic-strategy@2.10.0`.  TestFlight **1.0.36 (202608162123)** `IN_BETA_TESTING` (cron ship `31970196190` of `c1db7d12`).  Docs closeout #2754.  Rollout: `docs/rollouts/2026-08-15-ios-proposals-for-review.md`.
- **[Socratic.Trade][GROK] Ticker desk sheet — position, exits, peer-account mention — IN PR #2742 2026-08-16 (branch `grok/ticker-desk-sheet`).**  iOS + website only (PWA out of scope, owner 2026-08-16).  Lot + exit plan + last Green/Red + other-account size/direction.
- **[Socratic.Trade][GROK] ROIC single-flight (stop 22-min crash loop) — IN PROGRESS 2026-08-16 (branch `grok/roic-singleflight`, worktree `~/apps/trading-grok-ops-roic`).**  #2741 due check restacked a universe walk every 60s; 714 running journal rows; Coolify `last_restart_type=crash` every ~22 min.  Single-flight + start watermark + incremental cursor.  L1 keep-400 shrink still running.  FilingAPI still owner Plus.
- **[Socratic.Trade][GROK] Overlay library CRUD + Polymarket sector/macro + ASOF coverage — IN PROGRESS 2026-08-16 (branch `grok/overlay-poly-asof`, worktree `~/apps/trading-grok-overlay-poly`).** Owner: full overlay product; defer weekly hard-delete; no Reddit/X — deepen Polymarket; run VECTOR_ASOF_STRICT coverage. Receipt: 13076/13076 epoch'd, 0 undated.
- **[Socratic.Trade][GROK] Litestream L2/L3 + FilingAPI + ROIC earnings universe ingest — IN PROGRESS 2026-08-16 (branch `grok/litestream-filingapi-roic`, worktree `~/apps/trading-grok-ops-roic`).**  Owner: actually unstick L2/L3 (not just detect), fix FilingAPI using `FILINGAPI` in `~/.secrets/global-api-keys` (mint only if that key is broken), ingest ROIC.ai earnings calls across the trading universe including useful history, stored so the LLM can read many accurately.  Live health: L2 empty/wedged on 5945 L1 files, L3 upstream-wedged; `filingapi.ok` and `roic.ok` currently true.
- **[Socratic.Trade][GROK] Rename Apple Note pointer to `⭐️ Background Jobs Master List` — IN PR 2026-08-16 (branch `grok/note-title`).**  Owner retitled the pinned Coding note; AGENTS.md uses the exact new title.
- **2026-08-15 — GROK — IN PROGRESS — R2 cold snapshot retain 1 (branch `grok/r2-cold-retain-one`).**  Live DB is 4.2 GB; default retain 4 would be ~17 GB of a 10 GB free tier.  Code default → 1; override `R2_COLD_SNAPSHOT_RETAIN`.
- **2026-08-15 — GROK — IN PROGRESS — Weekly R2 on ST+CT + prune ST leftover LTX.**  Leftover R2 `trading-live/` LTX deleted.  Health ok on Aug 9 cold snapshot.  Week-2026-08-16 upload in progress.
- **[Socratic.Trade][GROK] 13F + ARK daily holdings + Form 4 as live idea sources — SUPERSEDED by ops-fix row 2026-08-16 (original #2736 merged; incomplete ingest).**
- **[Socratic.Trade][GROK] Stop boot-reseeding Gemini/DeepSeek user keys from Infisical env — IN PROGRESS 2026-08-15 (branch `grok/stop-ghost-native-keys`, issue #2728).**  migrateLocalEnvCredentials overwrote delete-tombstones when Coolify re-injected GEMINI_API_KEY/DEEPSEEK_API_KEY.  Failed 48h Gemini/DeepSeek calls were OpenRouter, not direct.
- **[Socratic.Trade][GROK] Account-config Title Case (Fractional Shares, Whole Shares, Regular + Extended) — COMPLETED/DEPLOYED 2026-08-15 #2727 `46d1dace` (issue #2726 closed).**  Capabilities sheet chips match Connected / Disabled.
...
- **[Socratic.Trade][GROK] Ban grepping secrets files for KEY=value lines — IN PROGRESS 2026-08-14 (branch `grok/secret-file-grep-ban`).**  `grep '^[A-Z0-9_]+='` on `~/.secrets/global-api-keys` prints values.  Rule in AGENT-SYNC + AGENTS.md + secret-safety: names only via `grep -oE`.
- **2026-08-15 — GROK — IN PROGRESS — R2 cold snapshot retain 1 (branch `grok/r2-cold-retain-one`).**  Live DB is 4.2 GB; default retain 4 would be ~17 GB of a 10 GB free tier.  Code default → 1; override `R2_COLD_SNAPSHOT_RETAIN`.
- **2026-08-15 — GROK — IN PROGRESS — Weekly R2 on ST+CT + prune ST leftover LTX.**  Leftover R2 `trading-live/` LTX deleted.  Health ok on Aug 9 cold snapshot.  Week-2026-08-16 upload in progress.
...
- **2026-08-14 — GROK — COMPLETED — Two-space rule: App Store review notes are not exempt (branch `grok/two-spaces-rule`).**  Owner: two spaces after every period everywhere, including ASC review notes.  AGENTS.md stanza now names App Review notes and IAP review notes and points at `AGENT-SYNC.md` § Two spaces.
- **[Socratic.Trade][GROK] Monet Backend-updates pickup (r4 land + r5) — IN PR #2721 2026-08-14 (branch `grok/monet-backend-r5`, worktree `~/apps/trading-grok-monet-r5`).** Resume after freeze.  Rematched `origin/main` (`cf0c8d78` incl. #2720).  2026-08-15 verify-hosted failed: `test/web-sources.test.ts` live-flow stub used `06/16/2026` disclosedAt; 60d window cutoff is exactly 2026-08-15T00:00Z.  Fixture dates now relative.  #2689 held (superseded by this stack).  #2691 independent residue.  Rollout: `docs/rollouts/2026-08-14-monet-backend-r5.md`.
- **[Socratic.Trade][GROK] Land Claude r4 leftover (benchmarks, ATR pullback, ops panel, data-age) — IN PROGRESS 2026-08-13 (branch `grok/claude-r4-pickup`, worktree `~/apps/trading-grok-r4`).** Pickup of five local `agent/claude` commits after `origin/agent/claude` vanished.  r3 already merged #2666.  Toggles yielded to Monet #2682 (migration 78).  No r4 schema bump.  Prompt version 2.5.0 (main already used 2.4.0).  Advisory-tail not in these five.  Rollout: `docs/rollouts/2026-08-13-claude-r4-pickup.md`.
- **[Socratic.Trade][GROK] Congress fixture dates aged out of 60d window — IN PROGRESS 2026-08-15.** test/web-sources.test.ts live-flow stub used 2026-06-16; after 2026-08-15T00:00Z that is >60d and `getSymbolWebSignals` drops NVDA. Relative dates. Unblocks remaining PR verify after #2720.
...
- **[Socratic.Trade][GROK] Bound per-document FTS mirror + durable resume — IN PROGRESS 2026-08-14 (branch `grok/bound-fts-mirror`, worktree `~/apps/trading-grok-bound-fts`, issue #2715).** Monet leftover (workflow `bound-fts-mirror-cost` started, zero results).  #2680 250ms yield left 933-chunk filings at 279s (`ftsMirrorBatch 279522ms`); lease expired mid-mirror; checkpoint advance failed; queue ~0.5%.  Bound = 20 chunks or 6s/tick (`20 * 279522/933 = 5991.9ms`).  Resume from FTS row count.  Heartbeat during FTS.  Do **not** re-enable `SEC_INGEST_WORKER_ENABLED` in this PR.  Rollout: `docs/rollouts/2026-08-14-bound-fts-mirror.md`.
- **[Socratic.Trade][GROK] Claude/Monet r5 residue (advisory-tail + parked owner decisions) — IN PROGRESS 2026-08-13 (branch `grok/claude-r5-residue`, worktree `~/apps/trading-grok-r5-residue`).** After #2682, leftover from Claude salvage `r4-toggles-superseded` is the risk_advisory tail reword ("nothing was blocked or changed") + merge-gate. Settings-surface sweep already found nothing. Parked: Reddit/X keys, VECTOR_ASOF_STRICT flip, r5 design slices, settings/page.tsx label until r4 PR merges. Does not touch grok/claude-r4-pickup files. Rollout: `docs/rollouts/2026-08-13-pickup-r5-residue.md`.
- **[Socratic.Trade][OWNER] Enable VECTOR_ASOF_STRICT after a fresh as_of_epoch_ms coverage proof — COMPLETED 2026-08-16 (GROK, Infisical prod `on`, restart `fwqascvivxvc7342hkw3aizk`).** Receipt 13076/13076 epoch'd. Live desk still omits asOf. Rollout: `docs/rollouts/2026-08-16-asof-strict-on.md`.
...
- **[Socratic.Trade][GROK] iOS agent build-loop policy — IN PROGRESS 2026-08-13 (branch `grok/ios-agent-rules`, worktree `~/apps/trading-grok-ios-rules`).** Docs/hooks only: `ios/CLAUDE.md`, AGENTS stanza, Claude pbxproj write-block. xcodebuild via bash is pre-approved; do not stand up Xcode MCP.
- **[Socratic.Trade][GROK] Stale ~1200s quotes + origin timeouts (Alpaca socket death) — IN PR #2720 2026-08-14 (branch `grok/prod-error-triage`, worktree `~/apps/trading-grok-prod-triage`, issue #2714).** Phantom-CONFLICTING after main moved; merge-tree clean; rematched `origin/main` (`6503a9c`); CI dispatched `31851110781`; squash auto-merge armed. RTH openings fell through to Yahoo delayed (~15-20m) after Alpaca `UND_ERR_SOCKET`; one `fetch failed` auto-halted Autopilot; `/api/health` 8s credits fetch stacked into UptimeRobot Connection Timeouts (and the paired credits-low keyword monitor). Retry dead sockets; 3-streak connectivity halt; abort=soft; 1.5s health credits budget; WU Sentry 6h dedup. Residual: filingapi 401 (owner key), Litestream L2 wedge (owner), CT latency probes.
- **[Socratic.Trade][GROK] Monet App / Issue Audit owner decisions — IN PROGRESS 2026-08-14 (branch `grok/audit-owner-decisions`, worktree `~/apps/trading-grok-audit-decisions`).** Docs-only.  Verified merge table #2680 #2681 #2682 #2684 #2685 #2687 #2709 #2712 all MERGED.  CT trial already 2 weeks; ASC intro TWO_WEEKS.  TF accept / Coolify rolling+B2 / ASC EULA+review stay owner-only.  No ASC writes.  Rollout: `docs/rollouts/2026-08-14-monet-audit-owner-decisions.md`.
- **2026-08-14 — GROK — IN PROGRESS — iOS TestFlight auto-ship once per hour (branch `grok/ios-ship-hourly`).** Standing gate is now 3600s in the shared fleet script. This PR only refreshes `scripts/ios-fleet.sha256` + `ios-ship.yml` comments so the runtime 1h default is the pinned tooling. Sibling CT change owns the script.
...
- **[Socratic.Trade][MONET] AGENTS.md: record the "local gate does not compile Swift" trap — IN PR 2026-08-14 (branch `monet/agents-swift-gate-trap`, worktree `~/apps/trading-monet-swifttrap`).** Docs-only. The four-command verify gate (lint / tsc / vitest / build) and `scripts/land.sh` compile ZERO Swift, so a fully green local run says nothing about `ios/**` — the first Swift compilation of any iOS change happens in CI. This cost a real CI cycle on 2026-08-13: merging `main` into `monet/apns-push` produced a DUPLICATE `@Binding private var pendingDeepLink` plus a second `init(pendingDeepLink:)` in `MobileControlView.swift`, and git did NOT flag a conflict — both sides had added the same declaration at slightly different offsets, so the text merge kept both copies and reported success. 6,563 vitest tests and a clean `next build` passed while the iOS app did not compile at all; the compiler's `ambiguous use of 'Preview'` error pointed at an innocent singular `#Preview` block that only became ambiguous because there were suddenly two initializers. "Verify before claiming done" now carries a CAUTION block with the exact `xcodebuild build` and `xcodebuild test` commands to run whenever a change **or a merge** touches `ios/**`. Merges are the highest-risk case precisely because you did not write the code and have no reason to suspect it.
- **[Socratic.Trade][GROK] Onboarding links + subagent/economics wording — IN PROGRESS 2026-08-14 (branch `grok/docs-onboard-links`).** AGENTS.md start-here table + stronger Delegation stanza. Docs only.
...
- **[Socratic.Trade][GROK] Autopilot accounts + rotation timeout RCA — IN PROGRESS 2026-08-13 (branch `grok/st-autopilot-accounts`, worktree `~/apps/trading-grok-st-autopilot`).** Consecutive failed runs were OpenRouter `/models/user` timeouts emptying `__rotate__`.  Fail-open + honest messages + Autopilot vs Running vocab.  Roth IRA / Alpaca Paper / Sandbox are decide+halted.  Script `scripts/set-autopilot-accounts.ts`.  Do not restart the container.
- **[Socratic.Trade][GROK] Bump Gemini defaults to 3.7 Flash — IN PROGRESS 2026-08-14 (branch `grok/gemini-3-7-flash`, worktree `~/apps/trading-grok-gemini-37`).**  Catalog alias `gemini-flash-latest` now wires to `google/gemini-3.7-flash` (native `gemini-3.7-flash`).  Dead `google/gemini-flash-latest` 404s.  Batch/offline uses `:batch`.  Isolated worktree.
- **[Socratic.Trade][GROK] Fix ST Litestream wedge + prefer Pushover over Resend — IN PR 2026-08-13 (branch `grok/litestream-fix-and-pushover`).** Operator alerts skip Resend when Pushover can deliver.  Coolify consistent names on; start period 60s.  L2/L3 still need a surgical L1 delete.
- **[Socratic.Trade][GROK] Alert email sign-off (sent by Socratic.Trade) — MERGED 2026-08-13 #2696.** Every Resend body ends with `(sent by Socratic.Trade)`.
- **[Socratic.Trade][GROK] Usage-monitor 409 retry loop + wrong health probe — IN PROGRESS 2026-08-13 (branch `grok/um-telemetry-health`).** Live flush re-queued a colliding telemetry row forever (36 repeats / 2h), painting Peer App Health Degraded while UM was up.  Drop named 409 key like replay; probe `/api/ready` then `/api/health`.
- **[Socratic.Trade][GROK] Pickup Monet+Claude quota-cap 2026-08-13 — IN PROGRESS (owner-directed, worktree `~/apps/trading-grok-pickup`).** Taking: ST #2685+#2683 phantom unstick; land `monet/ship-pipeline-fix`; land Claude r4 leftover; quote Key-Stats dashes + tappable fill/position cards; CT/UM CF leftover. Do not double-work. Rollout: `docs/rollouts/2026-08-13-pickup-monet-claude-cap.md`.
- **[Socratic.Trade][MONET] Litestream compaction visibility: make a silent backup failure loud — IN PROGRESS 2026-08-13 (branch `monet/compaction-visibility`, worktree `~/apps/trading-monet-compaction`).** Step 3 of a diagnosed fix (Steps 1/2 — Coolify Consistent Container Names + a one-time B2 delete of poison L1 objects — are the owner's). L2 deep compaction has been frozen since 2026-08-08T14:35Z (dual `litestream replicate` per rolling deploy, identical MaxTXID, permanent `ltx.IsContiguous` failure) and ran silently 5 days (`ok:true`, `litestreamDegradedReasons:[]`). Added: `validation: interval: 1h` in `litestream.coolify.yml` — corrected to TOP-LEVEL (not nested under `dbs:` as originally briefed; verified against pinned v0.5.12 Go source that `Validation` is a `Config`-level field, not `DBConfig`). A third independent detection signal needing no S3/B2 creds and no dependency on the remote-inventory pipeline (separate known bug in flight on `monet/durable-inventory-cache`, not touched): `scripts/coolify-prod-start.sh` tees litestream's combined stdout/stderr (it owns the container's real stdout via `-exec`) to a local log file via `> >(tee -a ...) 2>&1` (process substitution, preserves `$!`/SIGTERM-forwarding/exit-code contract — verified with a full local dry-run + isolated bash-idiom test before landing); `src/lib/runtime-health.ts`'s new `scanLitestreamRuntimeLogFile` (bounded 256KiB tail, never throws) scans for litestream's own `compaction failed`/`validation error detected` lines; wired into `/api/health` as `litestreamCompactionLogFailureCount` (count only, public-safe) + `alertStorageWarning`. No new `NotificationEventType`/migration — reuses `alertStorageWarning`'s existing free-text `warningType`, so unaffected by `monet/real-toggles` (#2682) merge order either way. Currently INERT in prod (the `.litestream-disabled` marker dropped today for an unrelated OOM incident short-circuits before the new line). Zero-code finding: searched for docs claiming the L2 wedge was "cleared"/"fixed" by the two earlier local resets — found none; all existing entries already correctly say the resets never touched B2 (confirmed against litestream's `DB.ResetLocalState` source). 14 new tests (11 unit + 3 integration), verified to fail without the implementation via `git stash`. Rollout: `docs/rollouts/2026-08-13-litestream-compaction-visibility.md`.
- **[Socratic.Trade][MONET] Honest server stats - delete fabricated Actions runners — IN PROGRESS 2026-08-13 (branch `monet/honest-server-stats`, worktree `~/apps/trading-monet-serverstats`).** Six hardcoded `running:healthy` runners on `ci-cpx32` (deleted 2026-07-31) served on 100% of prod requests (no GH token in ST prod). Replaced with `state: known|unavailable` + reason + detail. Also: unhealthy containers rendered green (`"unhealthy".includes("healthy")`), status label dropped the health half, Security card asserted litestream->R2 with zero observation, 4 host cards blank with no reason. AGENTS.md "stale Coolify registration" note corrected. Adversarial review caught the same class reintroduced in the adjacent Services card (empty `resources` asserted as "coolify reported no services" even when never queried or the read failed) - fixed with a matching `resourcesObservation` union + three-state panel; Security-card provenance claim and unverified CPU per-core scaling also made honest. Gates green (tsc 0, 6547 tests, build 0, lint 764 warn/0 err). UM/CT same-class findings logged as follow-ups, not fixed here.
- **[Socratic.Trade][MONET] Remove banned force-include notification pattern — IN PR 2026-08-13 ST #2682 (branch `monet/real-toggles`, worktree `~/apps/trading-monet-toggles`), auto-merge armed (squash) — merges on green `verify`.** Owner ruling 2026-08-12: "ALL toggles must be real." Removed 10 force-include-at-send-time sites across 8 files (lookahead_leak, signal_health, provider_degraded x2, storage_warning, autonomy_halted_on_boot, earningscalls_entitlement_blocked, budget_alert, risk_advisory x4) that silently overrode a user's OFF toggle forever. Replaced with a one-time versioned migration (db.ts v78 — originally 77, renumbered after `monet/apns-push` #2681 merged first and claimed 77 for `device_push_tokens`) that backfills those event types into legacy stored `enabledEvents` arrays once; toggle is genuinely real afterward. Rewrote a test that pinned the old behavior as a "regression" test; added signal_health/lookahead_leak off-toggle regression coverage + a migration test. Rollout: `docs/rollouts/2026-08-13-remove-force-include-notifications.md`.
...
## In Progress
- **[Socratic.Trade][MONET] Durable litestream remote-inventory cache (PR #2665's tier monitor never actually reported) — IN PROGRESS 2026-08-13 (branch `monet/durable-inventory-cache`, worktree `~/apps/trading-monet-inventory`).** `checks.storage.litestreamTierCoverage.remoteInventoryState` stuck permanently `"missing"` in production, forcing all 4 remote-only tiers `not-observable` forever. Root cause (task_journal evidence: collector 932 runs/24h, 0 errors — the collector worked fine): the snapshot cache was a bare module-level variable, and Next's build gives the scheduler and the API routes SEPARATE instantiations of `src/lib/litestream-remote-inventory.ts`, so the writer's assignment and the reader's lookup were never the same variable. Fix: persist through the existing `durable_state` primitive (`src/lib/db-durable-state.ts`, no migration). `lastAttemptAtMs` (30-min collection gate) deliberately stays in-memory — the writer is only ever driven by one stable `setInterval` closure for the process lifetime, proven by the same evidence (932 ticks, only 32 real collections = exact 30-min cadence). Two new tests use `vi.resetModules()` + fresh `await import(...)` to force genuinely separate module instances; both confirmed to fail against the pre-fix source. Gate: tsc clean, 566/567 files / 6569/6620 tests (pre-existing skips), build clean, lint 0 errors. Rollout: `docs/rollouts/2026-08-13-durable-inventory-cache.md`.
