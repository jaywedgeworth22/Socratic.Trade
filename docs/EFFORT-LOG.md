...
| 2026-08-19 | MONET | **Tranche-1 clusters `run-scoped-account` + `per-account-visibility` (issue #2880).** CORRECTION: originally claimed `order-provenance-guard` + `identity-fails-open`; both were already in flight under #2866 (I claimed off a truncated notification without reading the peer's full list) - YIELDED, that seat keeps them. Now taking two genuinely unclaimed account-scoping clusters: run-scoped-account (llm-02: run-scoped code re-resolves the console-active account, not the account being traded) and per-account-visibility (multi-account screens label one account's data as every account's). | **In Progress** |
