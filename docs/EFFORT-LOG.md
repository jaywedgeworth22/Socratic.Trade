...
- **[Socratic.Trade][CODEX] FMP coverage, market-scan reliability, and non-scan ticker-sheet enrichment (ready PR #1618, branch `codex/fmp-market-data-reliability`, worktree `/Users/jay/apps/trading-codex-fmp`, claimed 2026-07-15) — FINAL LAND GATE GREEN / HOSTED CHECKS RUNNING.** Production RCA found three July 14 interactive 25-second timeouts caused by a cold 150-symbol all-provider cascade; Finnhub can enqueue 750 paced requests while the old timeout leaked work. Interactive scans now keep deep ingestion off-request, accept only 24-hour slow facts, single-flight refreshes, bound Nasdaq, and show an explicitly stale strategy fallback. FMP maps broader stable profile/ratio/insider data with durable endpoint attribution; out-of-scan ticker sheets have a current identity/quote floor, timestamp arbitration, coalescing, and dynamic company-name header. Exact LRCX browser QA passed with zero console errors. PR #1616's concurrent FMP adapters are preserved on exact `main@d3efc9a6` and upgraded to verified header auth plus durable quota/outcome accounting. Final `scripts/land.sh` passed TypeScript, 381 files / 4,377 tests, and production build/32 static pages; refreshed head `8949ebd8` is pushed. Hosted smoke/verify, protected merge, and exact production verification remain.
