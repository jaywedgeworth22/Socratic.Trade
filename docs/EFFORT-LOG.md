...
- **[Socratic.Trade][CODEX] Admin console shell parity (branch `codex/admin-console-shell`, 2026-07-18) — IN PROGRESS / RECONCILED WITH MAIN.** Implementation and local gates are complete; PR #1740 is reconciled with current main at `a9171052` and awaits fresh hosted checks, auto-merge, and production verification.
...
- **[Socratic.Trade][CODEX] PR #1738 protective-stop pending-replace lifecycle review fixes (branch `codex/pr1738-p2-resolution`, worktree `/Users/jay/.codex/worktrees/socratic-pr1738-p2`, 2026-07-18) — IN PROGRESS, LOCAL COMMIT ONLY / NO PUSH (owner-directed).** Resolve four P2 findings in `src/lib/broker-protective-stops.ts`: preserve real submitted pending-replace refs through plan removal/unhalt/exit until reconciliation; reconcile terminal/filled saved refs and book fills before reuse; prevent halted right-size from loosening stop terms. Add focused durable-marker, broker-state, fill-booking, synthetic-exit, and long/short-side regressions; run proportionate gates. Based exactly on PR head `origin/claude/money-path-followups-1701@4e04bead`.
...
- **[Socratic.Trade][CLAUDE] bge-m3 reindex + backfill program (owner-directed 2026-07-18) — 3 lanes IN BUILD.** `claude/bge-m3-metering-gate` (provider-aware rag metering — fixes the live bug that arms when the next deploy flips embeddings to bge-m3 via #1669's key-presence routing: rag_usage rows would stamp provider='voyage' + ~10x cost via price-table fallback; adds explicit `RAG_EMBED_PROVIDER` gate), `claude/sec-ingest-worker-wiring` (Codex 2/3/4: manifest generator emits validator-conformant FrozenSecUniverseManifest + committed artifact regenerated + CI-validated; manifest→sec_ingest_jobs seeder with dead-letter discipline; SecIngestWorker started via `SEC_INGEST_WORKER_ENABLED` + admin seed route), `claude/corpus-reembed` (idempotent, budget-fused, lease-serialized re-embed of DB-persisted corpus into the active embed space + explicit legacy-Voyage purge action). SEQUENCING: metering-gate must deploy with/before the flip; corpus re-embed runs immediately after. Embed host: OpenRouter `baai/bge-m3` primary ($0.01/M, existing key; full 1.2M-chunk program ~= $7.50), CF Workers AI fallback.
- **[Socratic.Trade][CLAUDE] Codex-audit execution wave (owner-directed 2026-07-18) — 7 lanes IN BUILD.** `claude/stop-intent-idempotency` (Codex 5/6), `claude/stop-coverage-alpaca-tif` (7/10), `claude/egress-ssrf-body-caps` (11/13), `claude/cf-jwt-enckey-fingerprints` (12/14/15), `claude/ops-display-truth-batch` (33/38/43/45/46-remainder), `claude/decision-status-truth` (22/23/24/26/29), `claude/ios-client-fixes` (30/31/32, uncompiled-Swift caveat). EXCLUDED — other lanes own: Codex 8/9/16/17 (#1738/#1733/#1737), MONET visual-tour wave, #1735/#1736, CODEX CI-runner rerouting. Landing serially via land.sh after adversarial verify.
- **[Socratic.Trade][CLAUDE→OWNER] BLOCKER: prod deploy drift** — socratictrade.com serves `70a2a39d` (2026-07-17 19:03 CT) while main is 7+ commits ahead; 06:48Z container restart kept the old SHA, so auto-deploy is not building new commits. Coolify API token in the handoff file 401s (rotated?) and the Coolify MCP won't connect, so the deployment queue can't be inspected from this seat. NEED: fresh COOLIFY_API_TOKEN or an owner glance at socratic-trade-prod deployments (+ whether GitHub-App webhook deliveries are 403ing on the CF zone allowlist again). All landed work queues behind this.
- **[Socratic.Trade][CLAUDE] Top-to-bottom expert app review + backlog quantification/clearing plan (branch `claude/app-review-backlog-analysis-428ff7`) — CLOSED 2026-07-18, pivoted to execution (rows above).** Quantification complete: Pinecone corpus 8,476 vectors vs 600k-1.2M baseline target; the scheduled ingest path structurally cannot clear it (weekly TTL x 200/run paid cap); worker+manifest+seeder are the unlock, now in build. Review superseded by the Codex 46-item audit + MONET's visual-tour wave; surviving findings folded into the execution lanes.
...
- **2026-07-19 Prod deploy-wedge RESOLVED + litestream socket-leak mitigation (CLAUDE, branch `claude/litestream-leak-mitigation`).** Prod had stopped advancing (stuck `7be71390` vs main `79803667`); every deploy died at git-clone. SSH'd the box: litestream had leaked **92,700+ ESTABLISHED sockets** to R2 (~15k/hr), pinning ~2.3GB of kernel buffers and pushing `TCP: mem` **past** the raised `net.ipv4.tcp_mem` ceiling -> receive-window clamping -> bulk transfers cut. Owner-authorized container restart released everything (alloc 93,132->117, mem ~2.3GB->~140KB, free RAM 186MB->5,483MB); deploy then succeeded, **prod == main, healthy**. **Root cause PROVEN:** litestream 0.5.x opens a new connection per R2 request and abandons the response without draining/closing (100% ESTABLISHED, keepalive timer, frozen non-empty Recv-Q, identical `bytes_sent:2516` per socket, fires on no-op ticks). **DISPROVES the 2026-07-10 "durable fix"** -- the 0.5.12 pin was verified running and leaks anyway; the HTTP Transport block is byte-identical across 0.5.11/0.5.12/0.5.14 and 0.5.14 is HEAD, so version is a dead axis. This PR: `sync-interval: 10s` (leak rate 10x lower, time-to-wedge ~5h->~50h; **tradeoff: replica RPO 1s->10s**) + `region: auto` literal (the old `${VAR:-auto}` was bash syntax litestream doesn't support -> resolved EMPTY). Code-level *why* still open (H-A HTTP/2+GOAWAY retention leading; discriminating test = `GODEBUG=http2client=0` for 5min, needs an owner-chosen restart window). Rollout: `docs/rollouts/2026-07-19-litestream-socket-leak-mitigation.md`. State: **In Progress (PR open, NOT auto-merged -- RPO tradeoff is an owner call).**
