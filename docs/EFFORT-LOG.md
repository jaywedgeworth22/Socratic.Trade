...
## In Progress
- **Account-relative risk post-merge Codex P2 follow-up (CODEX, branch `codex/account-relative-risk-review-fixes`, claimed 2026-07-13) — IN PROGRESS.** PR #1561 auto-merged before its Codex review posted. Fixes all three non-outdated findings plus independent review refinements: migration v26 covers the legacy `settings.policy` sequence while v27 is schema-only so a later intentional fixed `$500` survives; Socratic cases persist/read `greenTeamRationale` and `sizingSnapshot`; UI/evidence use only final override truth; and the early Red path emits `red_team_veto_override_requested` rather than falsely claiming application. The Guardrails Dollar/Percent selector now follows persisted account state after discard/save/account changes; console/API/mobile mode tests pass 54/54. Combined focused load run passed 64/65 with one unchanged 20-second dashboard timeout; that file then passed 3/3 alone. Touched lint has 0 errors, TypeScript and diff-check are clean. Full Node 24 gate, ready PR, original-thread resolution, and production verification remain. No broker-protective-stops or infrastructure mutation.
