- **[Socratic.Trade][AG] Alpaca-trade-api v4 migration & compatibility fix (PR #3077) — IN PR 2026-08-24.**  Upgraded to @alpacahq/alpaca-trade-api 4.0.1 with universal module interop adapter in src/lib/alpaca.ts supporting ESM/CJS instantiate, merged latest origin/main with congress-trading-shared v2.6.0. Full verification passed (lint, tsc, 7581 vitest tests across 682 files, Next.js build, check-pin). Rollout: `docs/rollouts/2026-08-24-alpaca-v4-dependabot-bump.md`.
- **2026-08-24 — AG — IN PROGRESS — Switch iOS CI & Actions workflows to GitHub-hosted cloud macOS runners (branch `antigravity/cloud-ios-actions-runners`).** Update ios-build.yml and ios-ship.yml to runs-on macos-latest (free unlimited minutes on public repo), and update Xcode assertion.
...
- **[Socratic.Trade][CURSOR] Alpaca Paper Approve greyed after Retry Red Team — IN PROGRESS 2026-08-22 (branch `cursor/alpaca-paper-approve-greyed-8ba8`).**  Owner: Retry Red Team yesterday did not work; Approve is greyed on Paper pending cards.  Paper is Exit-only.  Split web retry/approve busy; iOS Approve stays available on a stale snapshot; retry applies half-size / reject-hold (llm-06); owner Approve of an opening in `close_only` is the override.  Rollout: `docs/rollouts/2026-08-22-alpaca-paper-approve-greyed.md`.
