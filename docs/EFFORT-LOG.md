...
- **PRs #816 / #819 / #820 / #822 - CLAUDE planned-backlog train: prompt-safety fencing, usage-budget
  advisory wiring, durable due-jobs substrate, HyDE+multi-query retrieval (CLAUDE).** All merged to
...
- **PR #808 - Cursor session: P0 checkRegimeFlip RMW fix + P1 backlog exhaustiveness (CURSOR) — ✅ COMPLETED via PR #844 (merged 2026-07-05).**
  _2026-07-05 (CLAUDE): the P0 multi-user regime RMW race + all P1 items + the AG connection-health
  slice + all 16 Codex review fixes LANDED on `main` via PR #844 (`claude/pr805-remediation`). #805
  CLOSED as superseded; all 16 review threads replied + resolved. This row, the AG connection-health
  row below, and the cycle-2 "Disentangle PR #805" row are all Completed by #844. Kept in place with
  the full correction history rather than deleted (never delete another agent's row)._
  _2026-07-05 (CLAUDE next-wave): CORRECTION — this row was previously (wrongly) filed under
  Completed as "Merged to `main` 2026-07-05". **PR #808 does not exist**
  (`gh pr view 808` → "Could not resolve to a PullRequest"). The actual work is commit `0ce39474`
  ("feat(cursor): P0 checkRegimeFlip RMW fix + P1 backlog exhaustiveness pass") sitting on branch
  `cursor/session-2026-07-05`, entangled inside **open PR #805** ("feat(health): Implement
  connection health routing & headroom monitoring", AG's row below) — and #805's mergeable state is
  **CONFLICTING**. `0ce39474` is confirmed NOT an ancestor of `origin/main`. This means the **P0
  multi-user `regime:current` read-modify-write race is still live on `main` today** — the described
  fix (dropping the `"local"` default, per-user `regime:current:${userId}` keys, per-user scheduler
  iteration) has not landed, nor have the claimed P1 items (security response headers, unpriced-model
  cost fallback, synthetic bid/ask boolean provenance, scheduler health threshold, operator LLM spend
  ceiling, effort-mirror orphan report, Litestream PITR retention) — none of these are on `main`._
  _2026-07-05 (CLAUDE, RESOLVED): the real vehicle is now **PR #844** (`claude/pr805-remediation`,
  auto-merge armed). It merges BOTH lanes cleanly onto current `main` (AG health `b88981c4` + this
  Cursor `0ce39474`, board-file edits dropped) AND fixes all 16 Codex review comments on #805 (each
  thread replied + resolved). #805 CLOSED as superseded. Gate green via land.sh: lint 0, tsc clean,
  2644 tests, build ok. When #844 merges, the P0 multi-user regime race + the P1 items + the AG
  connection-health slice all land together — move this row + the AG row + the "Disentangle PR #805"
  row to Completed then. The "Disentangle PR #805" cycle-2 CURSOR row is effectively DONE by #844
  (done as one honest PR rather than a split, per the row's stated option)._
  Real vehicle going forward: ~~land PR #805~~ → **PR #844** (see resolution note just above).
  Original claimed scope, unverified against `main` (now landing via #844):
  **P0 fix:** removed `"local"` default from `checkRegimeFlip`, per-user regime keys
  (`regime:current:${userId}`), per-user scheduler iteration — eliminates the multi-user RMW race on
  a single `regime:current` settings row. **P1 backlog (9 already-complete confirmed, 7 implemented,
  1 blocked):** added security response headers (HSTS, X-Content-Type-Options, Permissions-Policy),
  unpriced-model default cost fallback, synthetic bid/ask boolean provenance (replacing string-based
  checks in `strategy.ts`/`market.ts`), scheduler health threshold (abdicate after N heartbeat
  failures), operator monthly LLM spend ceiling (`LLM_SPEND_CEILING`), effort-mirror orphan report
  script, Litestream PITR retention. Global symbol omnibox blocked by Codex console/UI keepout.
  Rollout doc (describes intended, not confirmed-merged, state):
  `docs/rollouts/2026-07-05-cursor-session.md`.
...
- **Admin connection health and backend-failure notification pass (AG, L) — IN PROGRESS 2026-07-05, blocked on conflict.** Branch `cursor/session-2026-07-05` (PR #805).
  _2026-07-05 (CLAUDE next-wave): CORRECTION — this row was previously marked COMPLETED /
  "ready to land"; per protocol Completed = merged to `main`. **PR #805 is OPEN and its mergeable
  state is CONFLICTING** — it cannot land as-is. Its branch (`cursor/session-2026-07-05`) also
  carries the unrelated Cursor P0/P1 commit `0ce39474` (see the corrected "PR #808" row above),
  so landing #805 today would ship two lanes' work under one AG-titled PR. Re-marked In Progress /
  blocked-on-conflict. Needs: resolve the merge conflict against current `main`, then either land
  both changes together with an honest PR description naming both authors' work, or split the
  Cursor commit out into its own PR first (see the new "Disentangle PR #805" Planned row below)._
  Surfaces every backend dependency in `/api/health` and the `ops-snapshot` payload (Database,
  Pinecone, Voyage, FMP, Massive, etc.); fails the health check (503 status) on critical global
  outages (5 consecutive failures on Database, Pinecone, or Voyage). Routes global connection
  failures (Sentry, audit log) and degradation warning emails to the admin
  (`process.env.PRIMARY_USER_EMAIL` via Resend), while user-key failures remain strictly on user
  in-app notifications. Also integrates disk headroom, database + WAL size, and Litestream
  last-sync age monitoring into `/api/health` and the snapshot, with cooldown-controlled alerts on
  degradation. Verification (pre-conflict, on the branch): added `test/connection-health-routing.test.ts`,
  2454 unit tests pass green, tsc clean, Next.js build green — re-verify after the conflict resolves.
