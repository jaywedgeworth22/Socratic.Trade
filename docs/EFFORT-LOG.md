...
## In Progress
- **[Socratic.Trade][GROK] PR #1892 P2 review threads (rerank no_memory + sec-8k FTS) — IN PROGRESS 2026-07-22.** Worktree `/tmp/st-1892-p2` → push `codex/rag-retrieval-integration-20260722`. Fixes: preserve no_memory on clean empty when rerank unavailable; FTS mirror for 8-K body ingest.
- **[Socratic.Trade][CODEX team] RAG strategic-performance implementation program (claimed 2026-07-21) — IN PROGRESS / READY PR #1892 REVIEW REMEDIATION GREEN 2026-07-22.** Provider-aware/mock-safe managed ingestion, strict production-path PIT evaluation with runtime route receipts, tenant/current-version-safe FTS5 recall, dense+lexical RRF, independently routed/default-off adaptive reranking, text-free stage telemetry, exact prompt consumption, structured/narrative routing, bounded parent expansion, Pinecone hosted inference, and truthful Turso/Assistant probes are integrated. Independent reviews found and the branch fixed tenant bypass, stale legacy admission, synthetic-vector poisoning, broad relevance selectors, PIT/usage/cap gaps, false truncated-evidence credit, sibling-parent duplication, evidence-ref collisions, default-off union truncation, browser-bundle Node imports, and the PR review id-less attribution/prompt identity mismatch. Current review remediation also moves lexical metadata predicates ahead of the bounded FTS cap, matches filing text only, and separates the credentialed retrieval user from the generated isolated evaluation run id. Focused verification is in progress before pushing the updated PR head. All activation flags remain off; no provider/corpus/config/production write or legacy purge. Integration worktree: `/Users/jay/.codex/worktrees/rag-retrieval-integration-20260722`, branch `codex/rag-retrieval-integration-20260722`.
