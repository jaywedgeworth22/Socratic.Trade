...
## In Progress
- **Decision-detail dissent content deduplication (CODEX, ready PR #1593, branch `codex/decision-dissent-dedup`, 2026-07-14) — LOCAL COMPLETE / HOSTED CHECKS PENDING.** The decision trace keeps one canonical structured Red Team verdict, removes only its exact generic echo and known generated policy wrappers, and retains distinct policy/override objections. Display-only; no stored-case, broker, provider, production, or deploy mutation. Browser QA passed at desktop and 390 px mobile. Exact-tree Node 24 gate: focused 4/4, lint, TypeScript, 369 files / 4,132 tests, production build with 32 static pages, and diff-check all green; `scripts/land.sh` repeated TypeScript, all 4,132 tests, and the build before opening the ready PR. Hosted checks, review, merge/autodeploy, and production verification remain.
