...
## Deployed
- **Account-relative risk limits + Green/Red decision clarity (CODEX, PR #1561, merged as `3e105e17`) — DEPLOYED / PRODUCTION VERIFIED 2026-07-13; POST-MERGE P2 FOLLOW-UP ACTIVE.** Added one canonical dollar-or-percent daily opening cap with a 20%-NAV default and exact-$500 legacy migration while preserving explicit dollar settings; fixed the EXE fractional-Alpaca bracket contradiction; captured app-computed sizing arithmetic; split Live Thesis into Green, deterministic sizing, Red, and final outcome sections; replaced ambiguous “survived” and false “Bought / Blocked” copy. Node 24 local gate and required hosted verify passed TypeScript, 359 files / 4,021 tests, production build, Playwright smoke, and gitleaks. Production reported exact SHA `3e105e171a3f2122e71d5d60e991b50e1d59604c`, DB ok, scheduler current, Litestream current, and one healthy app container. A Codex review posted after auto-merge found three P2 gaps; focused follow-up is active below. The cold build coincided with one host OOM kill/restart of the prior app's Litestream parent; it recovered before cutover, and no manual deploy/host mutation was performed. `src/lib/broker-protective-stops.ts` remained excluded for AG PR #1548 ownership.
...
## In Progress
- **Account-relative risk + final-size/lifecycle truth follow-up (CODEX, branch `codex/account-relative-risk-review-fixes`, claimed 2026-07-13) — IN PROGRESS / HOSTILE BLOCKERS REMEDIATED / RE-REVIEW PENDING.** Closes PR #1561's post-merge findings and the deeper money-path audit: all four legacy policy stores migrate once while later explicit dollar caps (including `$500` and values above `$500k`) persist; Guardrails Dollar/Percent mode follows the active account; exact Green/sizing receipts survive lifecycle writes; final applied override truth drives evidence/UI; and GPT-5.6 catalog/Responses effort invariants are tested. A successful broker-minimum bump refreshes deterministic arithmetic and reruns Red once on the exact risk-adding order; half-size is down-only, reject/unavailable/unplaceable half requires one owner decision, exits are exempt, and reason-scoped holds preserve rationale-collapse/override gates. The four resumed-review blockers are implemented: `filled` consumes daily/hourly caps and remains successful in every count/UI/outcome consumer; structured owner holds do not invent Red outage; lifecycle sync preserves broker evidence/outcome/lessons/coach notes; and approval's atomic claim requires a proposed Socratic case. Node 24 TypeScript plus 15 files/132 focused tests pass. PR #1578's toolchain repair merged; current-main integration, fresh acceptance, and the authoritative full gate remain. No broker-protective-stop, host, secret, or corpus mutation.
...
- **[P1][Tooling/CI][M] Restore one supported TypeScript toolchain and real Next build type validation (CODEX, branch `codex/typescript-gate-repair`, claimed 2026-07-13) — READY FOR MAIN RECONCILIATION / FRESH REVIEW + FULL NODE 24 GATE GREEN.** One TypeScript 6.0.3 graph replaces the TypeScript 7 CLI / TypeScript 5 compiler-API split; postinstall/module-resolution mutations, the Next alias, and `ignoreBuildErrors` are removed. Review findings are closed and independently accepted: self-hosted CI selects and hard-checks Homebrew Node 24 while hosted stays setup-node 24; `scripts/land.sh` rejects non-24 runtimes before git mutation; `@types/node` is 24.13.3 with a Dependabot major hold; parsed lock/YAML and active-source policy coverage replaces string-only checks; and the ESLint comment correctly says 9. Clean install/lock regeneration, one compiler/Node-types graph, 5/5 policy tests, Bash 3/runtime guards, and YAML parsing pass. Final ordered gate: lint 0 errors / 458 inherited warnings, TypeScript clean, 363 files / 4,043 tests, production build with `Running TypeScript`/`Finished TypeScript`, and diff-check. Reconcile current main, commit, ready PR, hosted verification, merge/autodeploy, and production verification remain. Rollout: `docs/rollouts/2026-07-13-typescript-toolchain-gate-repair.md`.
- **Development background-worker fail-closed gate (CODEX, branch `codex/dev-background-workers`, ready PR #1576, claimed 2026-07-13) — READY PR OPEN / INDEPENDENT REVIEW + FULL NODE 24 GATE GREEN.** Production scheduler, usage replay, and enabled outbound streams remain default-on; every non-production runtime now fails closed unless `DEV_BACKGROUND_WORKERS=on` is explicit. One tested boot decision emits a visible receipt. Final gate: lint 0 errors / 458 inherited warnings, TypeScript clean, 363 files / 4,051 tests, production build, and diff-check green; `scripts/land.sh` repeated TypeScript, all 4,051 tests, and build before pushing. The accidental Node 26 test attempt reproduced the known `better-sqlite3` ABI mismatch; Node 24 passed completely. A stripped-environment `next dev` printed the disabled receipt with no scheduler-start line. Current-main Tailwind and skipped-build-type warnings belong to separately owned lanes. Hosted verify, merge/autodeploy, and production verification remain. No broker/provider/corpus/production call or config write.
