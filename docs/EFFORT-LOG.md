...
## 🚧 In Progress
- **[Socratic.Trade][CODEX] Coolify CI runner routing unblock (branch `codex/coolify-ci-runner-routing`, 2026-07-18) — LANDING.** GitHub-hosted `ubuntu-latest` jobs are failing before runner assignment on every required open-PR check (`runner_id=0`, no steps/log blob). Verified Coolify Hetzner Linux runners exist while `trading-live-mac` is offline; the narrower `socratic-ci` runner later flapped/disappeared during checkout, so required PR checks and PR-visible helper workflows now target the Coolify Linux pool (`[self-hosted, Linux, X64]`) instead of a single runner label. After this lands, rerun #1728/#1733/#1735/#1736/#1737/#1738 so their existing auto-merge can fire.
