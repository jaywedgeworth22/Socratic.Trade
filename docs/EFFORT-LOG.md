- **[Socratic.Trade][CURSOR] Hosted macos-latest TestFlight ship (in-repo ios-fleet) — IN PROGRESS 2026-08-24 (branch `cursor/ios-hosted-testflight-ship-0e2b`).**  Owner: do not run `xcodebuild` locally; do not restart the Mac runner.  #3083 already set `runs-on: macos-latest`; the wrapper still exec'd `/Users/jay/apps/ios-fleet/ship-testflight.sh` so every hosted tick failed in ~15s.  Vendor CT `scripts/ios-fleet/` + `ios-appstore-gm-prepare.sh`, pin in-repo hashes, import ASC/P12 from GitHub secrets (copy from CT, do not mint).  #3028 iOS UI is on `main`; this is the TestFlight deploy path.  Slack #agent-sync stop-local-xcodebuild posted.  Rollout: `docs/rollouts/2026-08-24-ios-hosted-testflight-ship.md`.
