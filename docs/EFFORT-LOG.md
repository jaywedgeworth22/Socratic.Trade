...
- **[CODEX] Native iOS mobile-first product replacement — IN PROGRESS 2026-07-21 (PR #1859 open; protected landing pending).** Branch `codex/mobile-first-ios-20260721`, worktree `/Users/jay/apps/socratic-mobile-first-ios`. Five-tab shell, full current mobile-snapshot presentation, typed HTTP/SSE/live-confirmation correctness, canonical bundle identity/entitlement/URL scheme, canonical XcodeGen app+test spec, and review remediation for deletion admission, immediate protective commands, terminal reconciliation, unknown execution mode, and app icon. Debug/generic simulator, Release simulator, and final iOS-26 test-target `build-for-testing` pass under Xcode 27 beta; targeted Node tests (7/7), TypeScript, ESLint, plist/asset validation, and diff check pass. XCTest execution is blocked only by zero installed simulator runtimes. Generated `.xcodeproj` is intentionally ephemeral. PR #1790/#1851 were selectively composed, not merged wholesale. KEEPOUT remains production/config/secrets and active non-mobile lanes.
