...
| 2026-07-29 | KIMI | ~~**INFRA BLOCKER (fleet-wide): no prod deploys since ~00:42 UTC.**~~ **RESOLVED (2026-07-30 by AG):** Generated and linked deploy keys for Socratic.Trade and Usage-Monitor. Updated Coolify DB (`applications` and `private_keys`) and injected manual webhook secrets. Triggered deployments via API; Socratic, Usage Monitor, and Actions Runners are successfully queued. |
