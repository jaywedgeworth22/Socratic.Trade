...
- **[CODEX] Native iOS mobile-first product replacement — IN PROGRESS 2026-07-21 (PR #1859 open; protected landing pending).** Branch `codex/mobile-first-ios-20260721`, worktree `/Users/jay/apps/socratic-mobile-first-ios`. Five-tab shell, full current mobile-snapshot presentation, typed HTTP/SSE/live-confirmation correctness, canonical bundle identity/entitlement/URL scheme, canonical XcodeGen app+test spec, and review remediation for deletion admission, immediate protective commands, terminal reconciliation, unknown execution mode, app icon, and verifier-bound opaque Google/GitHub auth handoff (no Auth.js JWT in a callback URL). Debug/generic simulator, Release simulator, test-target `build-for-testing`, and the checked-in Xcode project all build at iOS 26 under Xcode 27 beta; targeted Node tests (11/11), TypeScript, ESLint, plist/asset validation, and diff check pass. XCTest execution is blocked only by zero installed simulator runtimes. `ios/project.yml` remains source-of-truth; its generated `.xcodeproj` is checked in for direct Xcode use. PR #1790/#1851 were selectively composed, not merged wholesale. KEEPOUT remains production/config/secrets and active non-mobile lanes.
