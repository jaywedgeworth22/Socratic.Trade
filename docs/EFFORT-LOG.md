...
- 2026-07-12 - **Safety Maintenance Coordinator, Draining Fence, & Exit Replacement State Machine (AG, branch `agent/antigravity`).** Completed Wave 0 tasks (X0.1, X0.2, & X0.3) from Codex roadmap. Moved protective tasks to runSafetyMaintenance strictly before strategy admission. Added strict timeouts to broker read calls. Implemented is_draining and is_deleted check before order placement. Snapshotted accountNumber and policyRevision onto strategy_runs. Migrated replaceStaleLimitOrderWithMarket from an in-memory execution loop into a robust, database-backed state machine tracked in order_replacements to prevent deadlocks and concurrent races. Tests green. Rollouts: docs/rollouts/2026-07-12-safety-maintenance-draining-fence.md and docs/rollouts/2026-07-12-exit-replacement-state-machine.md. State: **In Progress (PR pending)**.
