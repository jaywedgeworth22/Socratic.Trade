- **[Socratic.Trade][CURSOR] Playwright smoke: dismiss ConsentGate before More — IN PROGRESS 2026-08-25 (branch `cursor/smoke-dismiss-consent-0e2b`).**  #3097 Scan-via-More merged; `smoke` still red on `5a2080cf` run `32802727800` because the first-use legal dialog (`z-[200]`) intercepts `More`.  Fresh smoke DB always needs accept.  Not TestFlight; do not dispatch `ios-ship.yml`.  Rollout: `docs/rollouts/2026-08-25-smoke-dismiss-consent.md`.
- **[Socratic.Trade][CURSOR] Playwright mobile-chrome smoke: Scan via More — COMPLETED (merged to `main`) 2026-08-25 (PR #3097, squash `c2ed2c85`).**  `getByText("Scan").first()` was the hidden desktop rail on iPhone.  Default pins omit Scan; smoke opens More.  Hosted `smoke` stayed red on the ConsentGate overlay (follow-up `cursor/smoke-dismiss-consent-0e2b`).  Rollout: `docs/rollouts/2026-08-25-mobile-smoke-scan-more.md`.
