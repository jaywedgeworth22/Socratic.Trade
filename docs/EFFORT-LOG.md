...
- **[Socratic.Trade][CODEX] Usage telemetry v2 producer adoption (branch `codex/usage-telemetry-v2-20260721`, worktree `/Users/jay/.codex/worktrees/socratic-telemetry-v2`) — IMPLEMENTED / RECEIVER GATE CLEARED 2026-07-22.** Exact-pins immutable shared `v2.0.0` over HTTPS; fresh live events use strict v2 `schemaVersion`/`producerId`/`eventId`/`producerKeyRef` fields with typed ACK parsing. One-time HMR normalization retains buffered pre-v2 events; durable replay now freezes each ledger's high-water mark, drains the bounded pre-v2 window through legacy idempotency, then records strict-v2 cutover before processing newer rows. Cold `GIT_SSH_COMMAND=false npm ci`, TypeScript, lint (zero errors), production build, 83 broader focused tests, final producer/replay/FMP regression set, and focused replay 10/10 pass. Usage-Monitor exact main `335723775ef0f8114ee1ca77b4716139018026dc` is committed live on Oracle; producer PR/deploy receipts follow. Rollout: `docs/rollouts/2026-07-22-usage-telemetry-v2-producer.md`.
