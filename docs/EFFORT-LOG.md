...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, MERGED PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, DRAFT PR #1429, branch `codex/strategy-lease-correctness`) — FINAL LOCAL GATE GREEN / TRUSTED PUSH PENDING 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Current `main@67e1536d` is merged; the run is account-snapshotted, long awaited phases re-prove ownership, durable proposal/broker truth survives ancillary loss, signal shutdown retains the scheduler lease until TTL, and completed-run-only auto-tuning is scheduled-account scoped under its own renewed account lease and LLM reservation. The final independent review also bound the prompt and walk-forward evidence to the account, honored a lost pending-to-blocked transition, and moved the tuning clock to follow-up start. Node24 final gate: focused 11 files/129 tests, touched lint 0 errors/36 inherited warnings, repository lint, tsc, full 341 files/3,801 tests, production build, and diff-check green. PR remains draft; trusted push, hosted checks, readiness, merge, and production verification remain. No provider-boundary, production-config, auto-merge, merge, or deploy mutation is included.
...
- **Strategy lease correctness hotfix reservation (CODEX) — ABSORBED INTO DRAFT PR #1429 2026-07-11.** This stale duplicate reservation is fulfilled by the active scoped-green row above; it is not a separate effort.
