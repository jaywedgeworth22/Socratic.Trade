- **[Socratic.Trade][GROK] Alert email sign-off (sent by Socratic.Trade) — IN PR #2696 2026-08-13 (branch `grok/alert-email-sent-by`).** Owner Litestream mail from `alerts@updates.jays.services` was ST notify(), not UM.  Body had no app footer.  Append `(sent by Socratic.Trade)`.  UM sibling #1171.
