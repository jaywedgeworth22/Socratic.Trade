...
## In Progress
- **Account-relative risk limits + Green/Red decision clarity (CODEX, branch `codex/account-relative-risk-clarity`, claimed 2026-07-13) — IN PROGRESS; IMPLEMENTATION READY, LANDING NEXT.** Implemented one canonical dollar-or-percent daily opening cap with a 20%-NAV default and exact-$500 legacy migration; preserved explicit dollar mode; fixed the EXE fractional-Alpaca bracket receipt/transport contradiction; persisted app-computed sizing arithmetic for Red Team/UI; split Live Thesis into Green, deterministic sizing, Red, and final outcome sections; replaced “survived” wording and false “Bought / Blocked” copy. Focused tests, lint, TypeScript, and build pass. Two canonical full-suite reruns reached 4,003 and 4,019 passing tests but hit unrelated host-contention timeouts; all timed-out files pass in isolation (25/25 and 4/4). `scripts/land.sh`, hosted clean-environment verification, merge/deploy, and production verification remain. `src/lib/broker-protective-stops.ts` remains excluded pending open AG PR #1548.
