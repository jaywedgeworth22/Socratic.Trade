- **[Socratic.Trade][CURSOR] Exclude node:test ios-fleet publisher suite from vitest — IN PROGRESS 2026-08-25 (branch `cursor/ci-autofix-automation-bcfd`).**  #3091 `verify`/`verify-hosted` failed because vitest loaded `scripts/ios-fleet/publish-ios-versions.test.mjs` (node:test) and reported "No test suite found".  Exclude `scripts/**/*.test.mjs` from `vitest.config.ts`; `node --test` in `ci.yml` stays.  Rollout: `docs/rollouts/2026-08-25-vitest-exclude-node-test-ios-versions.md`.
