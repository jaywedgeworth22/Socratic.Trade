- **[Socratic.Trade][CURSOR] Kimi fleet audit D/E/F — IN PROGRESS 2026-08-20 (branch `cursor/kimi-audit-def`).** Removal of operator Robinhood would starve CT peer **intraday** (quotes do not use RH). Bound to `/api/market/intraday` + operator stored token. `npm ci` on verify/e2e; SHA-pin gh CLI 2.98.0; Dependabot github-actions; shared pkg SHA `b2847eb9`; restore required `check-pin`. Rollout: `docs/rollouts/2026-08-20-kimi-audit-def.md`.
