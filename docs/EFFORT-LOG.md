- **[Socratic.Trade][AG] Kalshi first-class account & tailored event views, real-time paper data, and universal shorting/options trading — IN PROGRESS 2026-08-28 (branch `agent/ag-kalshi-and-options-expansion`).**  Link Kalshi accounts (demo & live) alongside broker accounts with customized event-contract settings, guardrails, portfolio, order execution, and LLM prompt views; remove delayed Alpaca paper keys from Infisical and guarantee real-time market data performance for paper accounts; build universal financial-grade shorting and options trading across all supported brokers (Alpaca, Tradier, Robinhood, Webull, Public, Test). Rollout: `docs/rollouts/2026-08-28-kalshi-and-options-expansion.md`.
