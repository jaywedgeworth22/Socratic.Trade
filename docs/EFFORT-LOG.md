...
- **[Socratic.Trade][AG] Prevent deleted API keys from auto-reseeding on server startup (branch `agent/antigravity-key-deletion-tombstone`) — IN PROGRESS.** Implemented `__DELETED__` tombstone row handling for `LOCAL_ENV_MIGRATION_SERVICES` in `src/lib/db-api-keys.ts` so deleted LLM keys (Gemini, DeepSeek) stay deleted across server reboots.
