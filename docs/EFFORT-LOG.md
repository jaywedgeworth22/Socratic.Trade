...
- **[Socratic.Trade][UNCLAIMED] iOS adaptive-tabs follow-ups: run the tests, take the screenshots, unhang the iOS CI check — PLANNED 2026-08-21.**  Six independent items, claimable in pieces, written up in `docs/rollouts/2026-08-21-ios-adaptive-tabs-followups.md`.  (A) The 45 XCTests added by #2987 are **compiled but have never been executed** — no lane runs `xcodebuild test`; and there is **no screenshot** of the new iPad layout.  Needs a Mac.  (B) Rename `WrappingHStackTests.swift` -> `LayoutMathTests.swift`; needs `xcodegen`.  (C) **`xcodebuild (unsigned)` hangs on EVERY iOS PR, not just #2987** — PR #2794 has the identical `cancelled` outcome.  `SWBBuildService` outlives `xcodebuild` and holds the step's stdout pipe until `timeout-minutes` kills it; raising the timeout does NOT fix it.  Proposed patch (redirect build output to a file) is in the note.  Owner calls that travel with it: make `ios-build` a required check (cf. board `830c892f`), and add an `xcodebuild test` lane.  (D) `PrivacyInfo.xcprivacy` is declared in `project.yml` but has **zero** references in the checked-in `.pbxproj` and the ship script does not run `xcodegen` — the shipped binary has no privacy manifest.  Pre-existing from #2794.  (E) Owner decision: keep or drop tab auto-fill.  (F) Tuning knobs table.
