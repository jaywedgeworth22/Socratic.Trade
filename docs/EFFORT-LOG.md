...
## Completed
- **Watchlist & Order Row Button Tooltip Alignment (AG, PR #1575, branch `agent/ag-watchlist-tooltip-fix`) — COMPLETED 2026-07-14.** Aligned watchlist action button and order row action button tooltips to the right (`align="end"`) to prevent clipping at the screen's right edge. Passed verification gate (tsc, tests, build) and pushed to main via auto-merge PR.
