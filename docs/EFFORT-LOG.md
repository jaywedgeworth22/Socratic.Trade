- **[Socratic.Trade][GROK] Verify APNs send (#2681) + APNS_P8 alias + halt coverage — IN PROGRESS 2026-08-13 (branch `grok/apns-send`).** Did not steal monet/apns-push.  Sender already on main.  Added APNS_P8/APNS_PRIVATE_KEY aliases and a kill_switch (protective halt) push test.  Infisical prod had no APNS_* (will seed from ~/.secrets/ST_AuthKey_SYV2VT3PQ6.p8, lengths only).
