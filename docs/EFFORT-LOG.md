- **[Socratic.Trade][CURSOR] Scan card tap + honest iOS last/next run — IN PROGRESS 2026-08-21 (branch `cursor/ios-scan-card-schedule-e224`).**  Whole scan card opens company info (iOS + website phone cards).  iOS company name is primary/semibold vs industry secondary.  `getSchedulerState` hydrates last run from `strategy_runs` and fills next run while autonomy is active (including after restart / cash close).  Last Run copy is `never`, never "not scheduled".  Rollout: `docs/rollouts/2026-08-21-ios-scan-card-schedule.md`.
