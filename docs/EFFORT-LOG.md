...
## In Progress
- **2026-08-25 — CURSOR — IN PROGRESS — Console honesty: Guardrails Discard, approve typed-confirm, Home Coach chips + leftover UX.**  Branch `cursor/console-discard-approve-chips-1ffb`, PR #3093.  Rebased onto `origin/main` `46865940` (#3096 TestFlight 1.0.69 docs).  Post-rebase `verify-hosted` failed copy-rules lint; busy toast and bulk Reject confirm now use `SENTENCE_GAP`.  Designer discard hypothesis confirmed: Discard now runs one `discardAll` that clears `universeDraft`.  Approve cards/bulk resolve `row.executionMode ?? currentMode` and render the 409 phrase.  Home no longer toasts Approved on `busy`.  Coach chips prefill/focus the note.  Leftover UX on the same PR: iOS Open Guardrails tab, Open Connections copy, Tradier (paper).  Stay off Datadog #3094.  Do not merge from this seat.  Rollout: `docs/rollouts/2026-08-25-console-discard-approve-chips.md`.
