- **[Socratic.Trade][AG] Active management & protective stops for held shorts when short selling is disabled — IN PROGRESS 2026-08-23 (branch `agent/ag-manage-existing-shorts`).**  Fix position management across strategy proactive risk exits, take-profit trims, synthetic stop monitor, broker protective stops, and console UI to manage and protect existing short positions regardless of the `shortSellingEnabled` toggle (which now strictly gates opening new shorts). Rollout: `docs/rollouts/2026-08-23-manage-existing-shorts.md`.
