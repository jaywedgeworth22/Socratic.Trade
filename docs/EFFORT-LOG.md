...
## 🚧 In Progress
- **[Socratic.Trade][CLAUDE] Money-path/reliability follow-ups from PR #1705 (branch `claude/money-path-followups-1701`) — GATE RUNNING, PR PENDING 2026-07-18.** Fixing 4 unfixed money-path/reliability findings that rode into `main` via PR #1705 (a 5th was already resolved by PR #1713 and is skipped). (1) `synthetic-stops.ts`: a halted+`protectWhileHalted` tick passed `running=true` straight into `reconcileBrokerProtectiveStops`, so a halted account could PLACE/REPLACE new broker-held protective stops — now gated so halted protection may only FIRE existing exits + run risk-reducing cancels, never place/replace. (2) `strategy.ts` `enrichOpeningProposal`: a Tradier `market` entry that the marketable-limit conversion turns into a `limit` (a type Tradier's native bracket DOES support) was having its brackets stripped BEFORE the conversion → converted limit with no native protection; strip now skipped when the conversion will apply. (3) `notifications.ts` `checkAndDispatchOptionAlerts`: concurrent dashboard snapshots both read `sentAlerts` before any row inserted → duplicate option alerts; added an atomic `option_alert_reservations` UNIQUE-constraint reservation (migration in `db.ts`), released on non-delivery. (4) `dashboard.ts`: best-effort `getOptionPositions` await sat OUTSIDE `withDeadline` — a hung options/MCP endpoint hung the whole snapshot; now wrapped with an 8s deadline → `[]` fallback. (5) `strategy.ts` active-protection live-exit filter — ALREADY FIXED on main by PR #1713 (`isLiveExitOrder`/`isLiveOrderState`), skipped. Regression tests added/extended for 1–4; each verified to fail pre-fix. Rollout: `docs/rollouts/2026-07-18-money-path-followups.md`. **UPDATE 2026-07-18: PR #1738 opened, auto-merge (squash) armed. Codex review raised 2 REAL round-2 findings, both fixed on the same branch: (a) the round-1 halted fix passed `running=false` which also suppressed the section-3 oversized-stop CANCEL (an out-of-band-shrunk position could keep an over-selling stop resting) — re-fixed with a `haltedProtectOnly` flag that blocks only placement + non-shrink replacement while the oversized/shrink cancel still runs; (b) the surviving bracket legs were priced off the pre-conversion `entryPrice`, so a converted marketable-limit above the reference could carry a take-profit at/below the fill — the converted limit is now computed once up front and both the legs and the conversion anchor to it. Full gate green: tsc clean, lint 0 errors, 4802 tests pass, build exit 0. BLOCKED on merge ONLY by the account-level GitHub Actions runner-provisioning outage affecting every open PR's `verify` (owner action: raise Actions spending limit / minutes). ROUND 8 (2026-07-18): codex-autofix pushed `4425b1a` (durable `pending_replace` halted right-size retry marker; mark `haltedRightsizeSymbols` only after a confirmed cancel; purge `option_alert_reservations` on account deletion) — it did NOT compile (TS2304 `oversized` out of scope) and its F1 marker was never read back (`listBrokerProtectiveStops` still filtered `resting`/`pending_cancel` only). Repaired: hoisted the mark-intent into a `markRightsizeOnCancel` flag (fixes tsc + preserves "mark only after live cancel"); added `pending_replace` to the `listBrokerProtectiveStops` status filter; guarded the two pre-section-1 consumer loops (`cancelBrokerProtectiveStop` + `kind===null` teardown) to DROP a `pending_replace` marker rather than cancel its synthetic id. Added F1 regression `SYN-HALT-F1RETRY` (placement throws → marker persisted → next halted tick reads + retries → resting). tsc clean, lint 0 errors, synthetic-stops 65 + broker-protective-stops/broker-side 91 + account-delete + option-alert-dedupe all pass; `npm run build` exit 0. ROUND 9 (2026-07-18): Codex raised 3 more P2 findings on the durable `pending_replace` retry marker, all genuine, fixed together: (F#1) section 1 deleted the marker before section 4 proved it could place -> a placement SKIP lost the owed right-size (unprotected until unhalted); now section 1 KEEPS the marker for halted+live+kind and section 4's `existing` excludes `pending_replace`. (F#2) with markers surviving, cancel-only paths could cancel the marker's synthetic id (404 -> stuck pending_cancel) -> added explicit skip guards to cancel-on-close + plan-teardown. (F#3) a placement that threw after broker-accept lost the submitted ref -> marker now preserves the client ref and the next tick ADOPTS the ref-matched live order instead of orphaning/duplicating (reuses ref for idempotency). Tests SYN-HALT-KEEPMARK/MARKCLOSE/ADOPT. tsc clean, lint 0, 168 affected-file tests pass; full suite + build re-running. ROUND 10 (2026-07-18): Codex raised 4 more P2 findings, all consequences of round-9's F#3 (markers can now hold a REAL client ref). Consolidated marker/ref resolution into ONE owner (section 1) + a loosening guard: (F#1) `cancelBrokerProtectiveStop` reconciles a real-ref marker (cancel accepted order by REAL id / drop-terminal / keep-invisible) not blindly drop it; (F#2) section 1 reconciles real-ref markers up front (adopt-live / book-filled / drop-dead / keep-invisible), never losing the handle; (F#4) section 1 books terminal fills (adopt filter ignored FILLED → missing from P&L); removed the redundant section-4 adopt block, kept ref-reuse idempotency; (F#3) `haltedRightsizeFloor` clamps a halted fixed right-size to the cancelled stop's tighter trigger so a widened stopLossPct can't loosen mid-halt (trailing arm-gated). Merged origin/main (4e04bea) which carries #1739 (CI → self-hosted Coolify runner — may lift the provisioning outage). Tests PS-REFCANCEL/REFKEEP/FLOOR/REFFILL. tsc clean, lint 0, 172 affected-file tests pass; full suite + build re-running. NOTE: rounds 8→9→10 self-compounding on the halted right-size machinery; owner's offered keep-oversized-while-halted simplification remains available (not switched unilaterally).**

**[Socratic.Trade][CODEX] Admin smoke memory headroom (branch `codex/admin-console-shell`, 2026-07-18) — IN PROGRESS.** CI-only Playwright heap reduced to 2048 MiB after the local Next webServer hit exit 137 under the 3 GiB runner cap. Owner: CODEX.
...
**[Socratic.Trade][CODEX] CI shallow-checkout recovery (PR #1741, branch `codex/ci-checkout-fast`, 2026-07-18) — INTEGRATED INTO PR #1739; LANDING WITH PARENT.** Lightweight classifier checks avoid full-history/tag fetches on the single Coolify CI runner; security deliberately retains full history for Gitleaks. PR #1741 merged into the routing branch as `c5ae4984`; no separate implementation remains active. Diff/YAML/actionlint checks passed.
- **[Socratic.Trade][CODEX] Coolify CI runner routing unblock (branch `codex/coolify-ci-runner-routing`, PR #1739, 2026-07-18) — LANDING / BRANCH FREEZE POSTED.** GitHub-hosted `ubuntu-latest` jobs fail before assignment (`runner_id=0`, no steps/log blob). Coolify service `github-runner` had both Socratic containers exited; API restart recovered `socratic-deploy` and `socratic-ci` while production stayed healthy. Required PR checks and helper workflows now target only `[self-hosted, socratic-ci]`, preserving the deploy lane and serializing memory-heavy jobs; the failure/schedule-only Sentry reporter uses the separate `socratic-deploy` runner so CI-runner outages remain observable. Gitleaks' optional SARIF artifact upload is disabled because it failed after a clean scan when `/_work` was outside `/root`. TypeScript hit Node's default ~1 GiB ceiling and a 1536 MiB retry moved the failure to Next build, so the dedicated CI container is now capped at 3 GiB with a 2560 MiB Node heap while retaining low CPU shares/high OOM priority. Playwright's CI-only server-start timeout is 600 seconds after the low-CPU build compiled but exceeded the old 240-second limit; local stays 240 seconds. Codex autofix and CI/E2E/package-pin jobs now reject fork PRs at job admission before runner assignment, checkout, write credentials, or secrets reach the persistent runner. Manually dispatched merge-shepherd runs call the same-repo implementation pinned to trusted `main` before inheriting write permissions and secrets. Coolify's `EPHEMERAL=1` runners had reused the same container filesystem under `restart: always`; both Socratic runners now clear only unmounted `/_work` before each registration, and the first fresh checkout/check-pin passed. Parallel direct pushes were merged non-destructively, but generic-Linux labels and 2-minute checkout timeouts were rejected because they re-admit the deploy lane and are below measured 3m31s-3m57s checkouts. Coolify production had drifted to branch `agent/ag-recovery-v48-migration`; restored it to `main` with auto-deploy enabled, without a manual deployment. After this lands, rerun #1728/#1733/#1735/#1736/#1737/#1738/#1740 so their existing auto-merge can fire.
- **[Socratic.Trade][CLAUDE] Mobile bottom tab bar: drop redundant safe-area band under labels (branch `claude/mobile-view-spacing-oetyav`) — GATE GREEN, PR PENDING 2026-07-18.** Owner reported (screenshots) wasted vertical space between the console's fixed bottom tab bar labels and Safari's address bar on mobile. Root cause: the `<nav>` applied `padding-bottom: env(safe-area-inset-bottom)` unconditionally, which in a normal browser tab stacks a SECOND clearance on top of the one mobile Safari already provides for a `position:fixed; bottom:0` bar — rendered as an empty band (nav bg == page bg). Fix: moved the inline padding to a `.con-tabbar` class in `console.css` that reserves the inset ONLY under `@media (display-mode: standalone), (display-mode: fullscreen)` (installed PWA / physical home indicator), padding-bottom 0 in browser. Files: `app/console/components/nav.tsx`, `app/console/console.css`. Gate: tsc clean, eslint 0 errors, 4758 tests pass, build clean. Rollout: `docs/rollouts/2026-07-18-mobile-tabbar-safe-area-band.md`.
- **[Socratic.Trade][AG] Fix candidate ATR stops and Alpaca short cover-buy recognition (PR #1713, branch `agent/strategy-atr-and-short-fixes`) — IN PROGRESS 2026-07-17.** Responded to automated Codex review findings on PR #1705 by: (1) passing `input.candidateAtrStopPctBySymbol` into `compactMarketScanForPrompt` so candidate stop distances are correctly formatted in prompt compaction, and (2) replacing the exitSide/side checks in `openExitOrders` filtering with the centralized `isLiveExitOrder` helper to properly recognize Alpaca cover buy orders for short positions. Full land.sh verify gate passed (Node 24 lint clean, tsc clean, all 4,758 unit tests passing, production Next.js build clean). PR #1713 is open with auto-merge enabled.
- **[Socratic.Trade][AG] Suppress earningscalls 401/403 connection alert spam and increase SQLite busy_timeout to 30s (branch `agent/earningscalls-sentry-and-sqlite-fixes`) — IN PROGRESS 2026-07-18.** Addressed persistent Sentry connection-failed alerts from the dormant `earningscalls` transcript integration (RapidAPI subscription inactive in prod) by: (1) passing `keySource: "env"` and (2) adding HTTP `401` and `403` to `suppressHealthStatuses` in `fetchWithRetry` options inside `earningsCallsGet`. Also increased SQLite's `busy_timeout` from 5s to 30s in `src/lib/db.ts` to make database writes resilient during high-disk-load Docker builds on the Hetzner box. Verified clean Next.js build, typescript check, and passing unit tests.
- **[Socratic.Trade][AG] Suppress earningscalls 401/403 connection alert spam and increase SQLite busy_timeout to 30s (branch `agent/earningscalls-sentry-and-sqlite-fixes`) — PR PENDING 2026-07-18.** Addressed persistent Sentry connection-failed alerts from the dormant `earningscalls` transcript integration (RapidAPI subscription inactive in prod) by: (1) passing `keySource: "env"` and (2) adding HTTP `401` and `403` to `suppressHealthStatuses` in `fetchWithRetry` options inside `earningsCallsGet`. Also increased SQLite's `busy_timeout` from 5s to 30s in `src/lib/db.ts` to make database writes resilient during high-disk-load Docker builds on the Hetzner box. Resolved two test suite issues: (1) isolated database connections in `test/market-custom-symbol.test.ts` to prevent decryption failures from the developer DB, and (2) updated `llmModelFamily` in `src/lib/llm-provider.ts` to handle namespace-qualified OpenRouter model names and refactored `test/model-rotation.test.ts` to target specific expected kept/skipped models instead of using a brittle global regex loop. Typecheck, lint, and all 4,791 tests green.
- **[Socratic.Trade][AG] Suppress earningscalls 401/403 alert spam, SQLite busy_timeout 30s, + priceForModel OpenRouter prefix fix (branch `agent/earningscalls-sentry-and-sqlite-fixes`, PR #1728) — GATE VERIFIED 2026-07-18, PR READY TO MERGE.** (1) Earningscalls: `keySource: "env"` + add HTTP 401/403 to `suppressHealthStatuses` to silence Sentry noise from the dormant RapidAPI integration. (2) SQLite `busy_timeout` 5s→30s to survive disk-thrash during Docker builds on the Hetzner box. (3) Model rotation tests: refactored to explicit model assertions; `llmModelFamily` in `llm-provider.ts` updated to strip `openrouter/` before family detection. (4) Market custom symbol test: database isolation fix. (5) `priceForModel` in `src/lib/llm-usage.ts`: single-slash strip was broken for 3-part `openrouter/vendor/model` IDs (produced `vendor/model`, no price-table hit, fell back to $15/M default); now mirrors `stripRoutingPrefix()` in `model-merge.ts` — strips `openrouter/` first, then one vendor segment. New regression test in `test/llm-cache-usage.test.ts`. Full land.sh gate: tsc clean, lint clean, 4,794/4,794 tests green (412 files), build clean. Rollout: `docs/rollouts/2026-07-18-earningscalls-sentry-and-sqlite-fixes.md`.
...
- **[Socratic.Trade][CLAUDE] Top-to-bottom expert app review + backlog quantification/clearing plan (branch `claude/app-review-backlog-analysis-428ff7`, worktree `kind-chaum-b7bbb3`) — IN PROGRESS 2026-07-17, owner-directed.** Multi-agent expert-panel review of the whole app: UI/UX intuitiveness + aesthetic polish (incl. wholesale-redesign candidates), backend/framework issues (incl. OpenRouter-transition-related — reviews around but does NOT touch in-flight PRs #1677/#1696/#1701/#1703), and accurate quantification of the RAG/filings/etc. backlogs (SEC 1,000-stock backfill state, transcript sources, ingest queues) with a concrete clearing plan. Analysis/docs deliverable (`docs/reviews/`), no product code changes planned this session. Do not double-work: AG holds the SEC/RAG backfill implementation lane (#1669 open). Routine dependency bump — `@jaywedgeworth22/congress-trading-shared` pinned to `fee9937c25db1de75c1a676826801e3399f36106` from `ef17b72`. Both `package.json` and `package-lock.json` updated. Rollout: `docs/rollouts/2026-07-16-shared-v183-dependency-bump.md`.
- **EarningsCalls.dev transcript source — free-plan budget design, dual transport (MONET, worktree `socratic-trade-audit-subagents-a100e1`, branch `monet/earningscalls-transcripts`, owner-directed 2026-07-16) — PR VIA land.sh, auto-merge armed (merge==auto-deploy; lands DORMANT).** Owner picked the EarningsCalls.dev FREE plan (RapidAPI channel, HARD 200 req/month; FMP transcripts remain entitlement-gated on both FMP channels — direct 402 + RapidAPI "Exclusive Endpoint" 403, live-probed). Durable UTC-month budget (default 180, reserve-before-call, `retries: 0`, refund only pre-dispatch); fetch-once-forever (symbol, fy, fq) cache + 3-day negative TTL (migration v47, renumbered around #1667's v46); holdings-first ≤6-req daily pass gated on the LLM/RAG spend ceiling; ingest via the #1586 rights-gated boundary (`source "earningscalls-dev"`), retrieval gated symmetrically (key pull = corpus un-retrievable); dual transport `EARNINGSCALLS_RAPIDAPI_KEY` (owner's channel, already in Infisical prod) / `EARNINGSCALLS_API_KEY` (direct, wins). 2 adversarial reviews SAFE_TO_LAND zero must-fix; TZ-unsafe event-date parse fixed + regression-tested under two host TZs. ACTIVATION: complete the free-plan subscription on rapidapi.com/earningscallsdev/api/earnings-call-transcripts1 (probes currently 405 pre-subscription), then next deploy is live. Rollout: `docs/rollouts/2026-07-16-earningscalls-transcripts.md`. **2026-07-17 cap-reset pickup: all 7 Codex review findings on PR #1680 fixed + regression-tested (P1 provenance, negative-cache/watermark discipline, per-pass clamp 6, FMP-calendar fallback, RAG_REINDEX lease, receipt-complete ingest — `docs/rollouts/2026-07-17-earningscalls-codex-triage-pickup.md`); all 8 threads (3 rounds) replied + resolved — round-2 mid-pass lease-loss fence fixed in `359cf72`, round-3 query-time source-filter edge deferred to the activation pass with rationale on-thread. **COMPLETED (merged to `main` as `1fcf274`, PR #1697 — supersedes #1680, which GitHub's Actions outage-window left with no dispatchable `verify` runs despite fresh heads + close/reopen; fresh PR dispatched normally once Actions recovered) + DEPLOYED TO PRODUCTION, DORMANT** (verified 2026-07-17 ~05:50Z: `/api/health` release sha matches, db ok, scheduler ticking, litestream replicating; zero behavior change until the owner completes the RapidAPI subscription).**
- **[Socratic.Trade] Bump congress-trading-shared to fee9937c (PR #1686, branch `antigravity/company-name-standardization-part2`) — GATE GREEN, LANDING.** (tsc/lint/403f-4724t all green; build via land.sh) Routine dependency bump — `@jaywedgeworth22/congress-trading-shared` pinned to `fee9937c25db1de75c1a676826801e3399f36106` from `ef17b72`. Both `package.json` and `package-lock.json` updated. Rollout: `docs/rollouts/2026-07-16-shared-v183-dependency-bump.md`.
...
- **Fleet-procedure skills: land-lane/unstick-pr/codex-triage/pickup-seat/deploy-verify (CLAUDE, branch `claude/fleet-skills`) — owner-directed, IN PROGRESS 2026-07-11, landing.** Five Claude Code skills under `.claude/skills/` encode the pickup-era fleet procedures (landing a branch via `land.sh`, unsticking a blocked PR, triaging codex-connector review threads, picking up a capped peer seat's work, verifying a post-deploy state) as on-demand skills instead of per-prompt re-spelling. `.claude/` stays git-ignored for per-agent/per-machine local settings and session hooks; `.gitignore` now carves out `!.claude/skills/` specifically so these five files are tracked. Skills are Claude Code-only — cross-agent rules (all tools) remain in `AGENTS.md`, which every skill cites as canon. Rollout: `docs/rollouts/2026-07-10-fleet-procedure-skills.md`.
- **Native iOS App Overhaul (AG) — IN PROGRESS.** Rebuilding the ios/SocraticTrade app with modern SwiftUI, ASWebAuthenticationSession, and premium aesthetic.
...
- **2026-07-18 Merged-worktree cleanup sweep + Voyage /api/health RCA (CLAUDE, branch `claude/cleanup-merged-worktrees-bdbc08`).** Removed 5 verified merged worktree checkouts (#1740-tmp/#1587/#1559/#1624/#1563; ancestry-verified, branches retained); kept `codex/reconcile-pr1745` (7 unlanded commits, NO PR — CODEX disposition), `socratic-admin-console-shell` (dirty), `trading-ag-rag` (standing lane). Voyage RCA: /api/health 200 ok; red `voyage` lane = prod bge-m3 embeds via OpenRouter failing 402 — OpenRouter account EXHAUSTED (25.00/25.31); Voyage key valid; RAG ingestion stalled pending owner credit top-up (or SiliconFlow key, same bge-m3 space). Rollout: `docs/rollouts/2026-07-18-worktree-cleanup-voyage-rca.md`. State: **Completed (docs PR)**.
