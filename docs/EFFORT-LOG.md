...
- **[Socratic.Trade] UX Wave D mobile/iOS parity (brand teal, Home checklist, command feedback, PWA polish) — PLANNED / UNASSIGNED (D4 claimed separately).** Program §Wave D. Prefer single iOS claim stream.
- **[Socratic.Trade][GROK] UX PR-D4 PWA polish — IN PROGRESS 2026-08-04 (branch `grok/ux-d4-pwa-polish`).** Humanize command labels; authority Ask-first/Autopilot; stale/offline banners keep controls usable; header "Control remote — full desk on desktop"; Proposals queue title. Program §PR-D4. Rollout: `docs/rollouts/2026-08-04-ux-d4-pwa-polish.md`.
