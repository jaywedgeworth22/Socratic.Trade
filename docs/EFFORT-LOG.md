...
- **[Socratic.Trade][CODEX] FMP coverage, market-scan reliability, and non-scan ticker-sheet enrichment (ready PR #1618, branch `codex/fmp-market-data-reliability`, worktree `/Users/jay/apps/trading-codex-fmp`, claimed 2026-07-15) — HOSTED P2S FIXED / FINAL EXACT-TREE GATE NEXT.** Production RCA found three July 14 interactive 25-second timeouts caused by a cold 150-symbol all-provider cascade; Finnhub can enqueue 750 paced requests while the old timeout leaked work. Interactive scans keep deep ingestion off-request, accept only 24-hour slow facts, single-flight refreshes, bound discovery, and show an explicitly stale strategy fallback. FMP maps broader stable profile/ratio/insider data with durable endpoint attribution; out-of-scan ticker sheets have a current identity/quote floor and dynamic header. PR #1616's FMP adapters are preserved on exact `main@d3efc9a6` and upgraded to header auth plus durable accounting. Hosted review's three P2s are fixed: hard 20-second scan deadline with Nasdaq/BlackRock abort propagation, complete single-flight identity including weights/floor/positions, and 30-second eviction for hung quote entries. Scoped lint/TypeScript and 5 files / 26 tests pass. Final exact-tree land gate, refreshed hosted checks/review, protected merge, and production verification remain.
