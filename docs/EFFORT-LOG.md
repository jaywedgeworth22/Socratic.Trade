- **[Socratic.Trade][GROK] Stop paging Starter 2M Pinecone write units — IN PROGRESS 2026-08-21 (branch `grok/pinecone-wu-cap`, worktree `~/apps/trading-grok-pinecone-wu`, board 7b0bf414).**  Owner: change monthly WUs and stop the notifications.  Live org is Standard trial (unlimited WUs).  Matcher missed official monthly-quota text without `429` in the body → hourly connection-failed.  Breaker only parks when a monthly budget is set; post-trial no 1.6M snap.  Infisical `PINECONE_MONTHLY_WU_BUDGET=0`.  Rollout: `docs/rollouts/2026-08-21-pinecone-wu-no-starter-wall.md`.
