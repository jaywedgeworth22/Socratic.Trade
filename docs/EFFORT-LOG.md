...
- **[Socratic.Trade][GROK] Activity-audit leftovers (P2.4 + P3 batch) — COMPLETED 2026-08-05 (branch `grok/activity-audit-leftovers`).** Verified most §1.4 / §1 P3 already on main; board hygiene + tiny polish (`bracket_sibling_*` account attribution + congress 60m backoff test). Residual P3.5 (`'undefined'` fill flip) + P3.8 (evidence_age assertedAt key) left to sibling agents. #1324 owner decisions remain Planned. Rollout: `docs/rollouts/2026-08-05-activity-audit-leftovers.md`.
...
- **Activity-audit P3 batch (8 small items) — mostly COMPLETED on main 2026-08-05 (board hygiene).** Already shipped: (1) feed-storm coalesce in `buildUnifiedFeed`; (2) AAPL trim/#1297 dollar-sell; (3) `policy_change` attribution via `setPolicy`/`setStrategyPrompt`; (4) broker-protective-stops attribution (incl. leftover `bracket_sibling_*` account id polish on leftovers branch); (6) `storage_warning` event type + notify path; (7) `KNOWN_GLOBAL_AUDIT_KINDS` System-wide footer; (8) `evidence_age_anomaly` first-sight per `(id, assertedAt)` + LRU eviction (no 6h TTL / no `Map.clear()`) on `grok/activity-audit-leftovers` — `fix(strategy):` commit, not pushed. **Residual (sibling agents):** (5) one-time flip of stuck `'undefined'` fill_events → `unreconcilable` (may be WIP on same branch — leave db-fills alone if mid-commit). Report §1 P3.
...
## Completed
- **2026-08-05 — GROK — COMPLETED — Activity-audit leftovers board hygiene + residual polish (branch `grok/activity-audit-leftovers`).** Marked P2.4 (#1492 code half), shipped P2 backlog items, and P3 items 1–4/6–7 COMPLETED+DEPLOYED / already-on-main. Residual P3.5+P3.8 for sibling agents; #1324 stays Planned. Optional code: `bracket_sibling_*` audits take `connectedAccountId`; congress-share 60m failure-backoff unit test. Rollout: `docs/rollouts/2026-08-05-activity-audit-leftovers.md`.
