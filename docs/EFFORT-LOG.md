...
- **LLM failover UI & account bursts (AG) — COMPLETED 2026-07-13.** Added jitter/stagger to concurrent account scheduling in `scheduler.ts` to mitigate shared OpenAI key bursts. Implemented failover loops for both Green and Red Teams. Exposed `llmFallbackModels` and `redTeamFallbackModels` in `app/console/strategy/page.tsx` via a togglable checkbox UI and persistence logic.
