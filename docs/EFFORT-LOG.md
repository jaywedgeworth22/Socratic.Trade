...
- **[Socratic.Trade][GROK] UX Wave D PR-D1+D2 iOS brand teal + Home hero — IN PR 2026-08-04 (branch `grok/ux-d-ios-brand-home`).** Adaptive `AppPalette.accent` #12616f/#58c7d3; Home readiness checklist + pending-proposals top CTA → Proposals tab; ready hero (equity/unrealized/agent/Run once); Proposals tab badge. `xcodebuild` simulator BUILD SUCCEEDED. Rollout: `docs/rollouts/2026-08-04-ux-d-ios-brand-home.md`. Residual Wave D: D3 command feedback, D4 PWA polish.
- **[Socratic.Trade] UX Wave D residual (D3 command feedback, D4 PWA polish) — PLANNED / UNASSIGNED.** D1+D2 landing separately. Program §Wave D.
