- **[Socratic.Trade][CODEX] Alpaca-trade-api v4 migration (PR #3077) — BLOCKED 2026-08-24, owner decision needed.**  Dependabot major bump 3.1.3 → 4.0.1; v4 is a full SDK rewrite (named export only, flat methods → `trading.*`/`marketData.*` namespaces, camelCase responses).  `src/lib/alpaca.ts` still uses the v3 flat surface, so the PR breaks the build (`tsc` TS2613 at line 1; runtime `new Alpaca` undefined).  Codex P1 threads (quote parsing `bp`/`ap`/`t`; order parsing snake_case) are both symptoms of the one migration.  Codex's second review (2026-08-24) added three more P1 threads: account parsing (`account_number`/`portfolio_value`/`buying_power`), position cost-basis (`avg_entry_price`/`average_entry_price`) — further symptoms of the same migration — and a commit-identity finding naming a `Codex <codex@openai.com>` author that does not exist on the branch (all PR commits are `dependabot[bot]` or `claude[bot]`; a false positive).  Owner decision required: migrate adapter + fixtures or revert to v3.  Rollout: `docs/rollouts/2026-08-24-alpaca-v4-dependabot-blocked.md`.
...
- **2026-08-24 — CODEX — PLANNED/BLOCKED — Alpaca-trade-api v4 adapter migration (PR #3077).**  Dependabot major bump 3.1.3 → 4.0.1; v4 is a full SDK rewrite — named export only (no default), flat client methods replaced by `alpaca.trading.*`/`alpaca.marketData.*` namespaces, camelCase responses.  `src/lib/alpaca.ts` (1144 lines) is built on the v3 flat surface, so the PR breaks `tsc` (TS2613 default-import at line 1) and would fail at runtime (`new Alpaca` undefined).  Codex P1 threads (quote parsing `bp`/`ap`/`t` at `src/lib/alpaca.ts:693-707`; order parsing `client_order_id`/`filled_qty`/`created_at` at `mapAlpacaOrder` `:1086-1107`) are symptoms of the same migration.  Autofix did not guess (real-money path).  Owner decision required: migrate adapter + test fixtures (`test/alpaca-brackets.test.ts`, `test/alpaca-order-mapping.test.ts`, `test/alpaca-mcp.test.ts`, `test/data-providers.test.ts`) or revert to v3.
