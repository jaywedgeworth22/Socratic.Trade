- **[Socratic.Trade][CURSOR] Unhang ios-build SWBBuildService pipe + run XCTests — IN PR #3023 2026-08-21 (branch `cursor/ios-ci-unhang-ac14`).**  Item C verified: Mac job run 32529663287 concluded `success` in ~2 min (not `cancelled`); BUILD SUCCEEDED; TEST SUCCEEDED; 232 XCTests / 0 failures including all 30 `TabPreferencesTests` on iPhone 17 Pro.  Item D already on `main` via #3012; vitest asserts `PrivacyInfo.xcprivacy in Resources`.  Screenshots / rename / auto-fill veto still open.  `ios-build` can now be made required (board `830c892f`).  Rollout: `docs/rollouts/2026-08-21-ios-ci-unhang.md`.
