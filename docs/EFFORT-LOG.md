...
- **[Socratic.Trade][CURSOR] iOS Scan empty table is a quote miss — IN PR #2830 2026-08-18 (branch `cursor/scan-empty-screener-a128`).** Verified abort: 8s stub-UA `controller.abort()` with no reason (“This operation was aborted”); not the 20s deadline.  Shared `BROWSER_UA` helper (scan + congress-share), 15s timeout, one abort retry, Yahoo whole-set, 503.  Rollout: `docs/rollouts/2026-08-18-scan-empty-screener.md`.
