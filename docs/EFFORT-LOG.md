...
| 2026-08-21 | CLAUDE | **iPad Air 11" / Mac Catalyst layout + tab bar + login wordmark (branch `claude/ipad-layout-login`).**  Login: the candlestick wordmark was FROZEN — `TimelineView(.periodic(from: .now,…))` re-anchors its phase on every body evaluation, so inside a ScrollView the next tick kept being pushed into the future; the web ticker drives the same integer tick off rAF and has no anchor, which is why the site moved and the app did not.  Same defect fixed in SnapshotScaffold's 30s "updated N ago" tick.  Wordmark -20%, "strategic" dropped (iOS + web bullets), lock line + disclaimer merged into ONE justified paragraph (new `JustifiedText` — SwiftUI has no justified case).  Tabs: Home required, the slot before More swaps to whatever screen you open from More, capacity from LIVE WIDTH (4 phone → 7 wide iPad/Mac) falling back to defaults without forgetting pins; `ensureVisible` stops a narrowing window stranding the screen you are on.  Layout: 72-agent audit → 58 confirmed findings → one keystone (SnapshotScaffold had no content column; 8 of 9 tabs were 1148pt letterboxes).  Shipped column + large titles + measured metric grids + card grids + Home 2-up + control rail.  Two fixes NOT on the list: confirmation popovers anchored to their buttons (was the middle of a ~1100pt card — irreversible broker cancel, ambiguous once cards pair up), and SwipeRevealAction no longer advertising a dead VoiceOver action + a false gesture hint.  **My own tests caught a real iPhone regression in the audited plan:** `AppLayout.action` 360 BOUND inside an AppCard on a 440pt Pro Max (~376pt interior), narrowing Run Once/Start/Stop by 16pt — raised to 420.  Also found both new test files were absent from the pbxproj and had been running nowhere; regenerated via xcodegen + re-applied objectVersion 100 (8-line diff).  Gate: build clean, **199 tests 0 failures on the Mac Catalyst destination** — the iOS simulator is unusable (root-owned `simdiskimaged` wedged; `simctl` blocks, boot times out).  **NOT visually verified — screenshot gate still owed.**  Rollout: `docs/rollouts/2026-08-21-ipad-layout-and-tabs.md`. | **In review (PR)** |
