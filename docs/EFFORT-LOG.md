- **[Socratic.Trade][KIMI] Macro feed resilience + Unknown-side regime-flip suppression (branch `agent/kimi-lane`) — Implemented 2026-07-29.** Keyless VIX cascade (Yahoo -> Cboe -> Stooq) behind the shared per-lane circuit breaker (60s trip + half-open probe via api-circuit-breaker/api_health_log); checkRegimeFlip holds last-known regime on feed outage (no flip/dirty/event, once-hourly macro_feed_unavailable diagnostic, recovery announces real changes with existing semantics). Finding: macro chain never used FMP — flap driver was the single Yahoo lane. 12 new tests; tsc/lint/vitest(5387)/build all green. Committed locally only — landing via owner/parent. Rollout: docs/rollouts/2026-07-29-macro-feed-resilience.md.
