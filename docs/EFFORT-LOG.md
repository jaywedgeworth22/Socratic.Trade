...
- **[CODEX] Native iOS mobile-first product replacement — IN PROGRESS 2026-07-21 (implementation + parent safety review complete; commit/PR pending).** Branch `codex/mobile-first-ios-20260721`, worktree `/Users/jay/apps/socratic-mobile-first-ios`. Five-tab shell, full current mobile-snapshot presentation, typed HTTP/SSE/live-confirmation correctness, canonical bundle identity/entitlement/URL scheme, canonical XcodeGen app+test spec, and focused tests are implemented. Parent review fixed deletion-contract drift, stale/readiness command gating, refresh ordering, durable retry idempotency, live-account confirmation, proposal review provenance, and accessibility sizing/layout. Debug/generic simulator, Release simulator, and final iOS-26 test-target `build-for-testing` pass under Xcode 27 beta; Release settings retain automatic signing for team `CC8UTF7ATG`. XCTest execution is blocked only by zero installed simulator runtimes. The focused Apple client-ID resolver suite is 3/3. Generated `.xcodeproj` is intentionally ephemeral. The only backend change aligns the native Apple audience fallback; no trading or unrelated web paths changed. PR #1790/#1851 were selectively composed, not merged wholesale. KEEPOUT remains production/config/secrets and active non-iOS lanes.
