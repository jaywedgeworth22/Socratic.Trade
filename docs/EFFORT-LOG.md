- **[Socratic.Trade][GROK] Surface R2 weekly status on /api/health — IN PROGRESS 2026-08-14 (branch `grok/r2-weekly-health`, worktree `~/apps/socratic-grok-r2-weekly`).** Persist last successful `r2_cold_snapshot` (`r2coldsnap:lastSuccess`) and publish public `checks.storage.r2Weekly` `{ok, ageSeconds, key, reason}` with an 8-day window matching UM.  Observability only — does not fold into `storageDegraded`/503.  No R2 network on the health path.
