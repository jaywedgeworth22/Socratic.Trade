...
## 🚧 In Progress
- **Candlestick "ST" favicon (CLAUDE cloud, branch `claude/socratic-trade-logos-p0hxk7`) — OWNER
  SIGNED OFF 2026-07-15, PR #1626 merging (auto-merge enabled on approval).** New favicon/app icons
  spelling "ST" in candlesticks, rendered by the EXACT header-logo pipeline verbatim
  (`sampleWordmark("ST")` + `buildTickerUnits()` + `drawTicker`), so it's the identical candlestick
  rendering as the on-page wordmark. Tick 6 (green-biased, tall green T stem, no pink column), dark
  rounded bg. Static-asset swap (`public/icon.svg` + `public/icons/*`); `app/layout.tsx` already
  references them. tsc 0, build 0. See `docs/rollouts/2026-07-13-candlestick-st-favicon.md`.
