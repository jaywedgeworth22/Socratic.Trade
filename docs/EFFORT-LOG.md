- **[Socratic.Trade][AG] iOS App Mac Catalyst Rendering Fixes — COMPLETED 2026-08-12 (branch `ag/ios-logo-fixes`).** Replaced CoreGraphics manual flipped coordinate context (`ctx.scaleBy(1, -1)`) with `UIGraphicsImageRenderer` in `CandleWordmarkView.swift` to natively handle text orientation across iOS and Mac Catalyst. Refactored `CandleWordmarkModel` to dynamically cache arbitrary strings. Updated `LaunchStateView` inside `SocraticTradeApp.swift` to stack "SOCRATIC" and "TRADE" wordmarks instead of the old chart icon. PR landed.
...
- **[Socratic.Trade][MONET] Backup tier monitor extended to levels 2+3 — IN PROGRESS 2026-08-12 ~2:20am CT (branch `monet/backup-tiers-2-3`).** Day-old 0/1/9 monitor was structurally blind to the 2026-08-12 level-2 wedge; now watches all active levels (0/1/2/3/9), panel + tests updated, 51 green. Follow-up section in the backup-status rollout note.
- **[Socratic.Trade][MONET] iOS customizable tab bar (web mobile-tabs parity, native glass TabView) + xcodegen version-regression ROOT CAUSE fix + web<->iOS parity expert panel — IN PROGRESS 2026-08-12 (branch `monet/ios-customizable-tabs`).** Owner ask: customizable tabs like the mobile website, glass style. TabPreferences (min 2/max 4, canonical order, More overflow, UserDefaults) + 8 XCTests; programmatic jumps to unpinned screens reroute into More. Root cause killed: xcodegen regen rewrote Info.plist version keys to literal 1.0/1 (project.yml never declared them — exactly how #2637 introduced the #2639 regression); now declared as substitutions. Parallel: expert-panel workflow (2 catalogers + 4 lenses + synthesis) reviewing full parity incl. mobile-admin-access question; native Designed-for-iPad Mac run being attempted per owner ask. Rollout: `docs/rollouts/2026-08-12-ios-customizable-tabs.md`.
...
| 2026-08-12 | CLAUDE | **Planned (dsa-lessons backlog, from ZhuLinsen/daily_stock_analysis gap analysis): structured decision scorecard** — unify scattered proposal receipts (thesis, levels, red-team verdict, gate reasons, factor attribution) into one typed renderable ProposalScorecard + decisionChain validator; sketch in docs/reviews/2026-08-12-dsa-lessons-gap-analysis.md (lands with CLAUDE's round-1 PR). | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (dsa-lessons backlog): social/prediction-market sentiment axis** — per-ticker Polymarket (real-money crowd) + optional Reddit/X buzz as a fourth sentiment axis; needs an owner decision on data source/API key (NEVER agent-created keys). | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (dsa-lessons backlog): strategy overlay template library** — named, regime-tagged strategy lenses auto-routed by the existing regime classifier; incremental over thesis playbook. | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (dsa-lessons backlog): unify Bayesian trust-weight implementations** — 4 modules independently reimplement sample-gated shrinkage + bounded clamp; extract one shared calibration helper. | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (dsa-lessons backlog): analysis-chat SSE stage events + budget skip + real cancellation** — named event vocabulary for the chat orchestrator so multi-step tool loops are visible/stoppable. | **Planned** |
| 2026-08-12 | CLAUDE | **Round 2 (owner scope expansion): broad GitHub sweep for definitive wins** — deep-read TauricResearch/TradingAgents (97k), virattt/ai-hedge-fund (62k), freqtrade (53k), microsoft/qlib (47k), microsoft/RD-Agent (14k), pipiku915/FinMem (layered agent memory); synthesis + gap analysis vs ST (8 lessons, 7 recommended). | **In Progress (research DONE; implementing 3 wins: benchmark-alpha outcome grading, live signal-health monitor + drift alarm, cancel-dust advisory; PR follows round-1)** |
| 2026-08-12 | CLAUDE | **Planned (round-2 backlog): point-in-time fundamentals revision chain** — replace latest-wins symbol_field_latest reads on as-of paths with (value, first_published_at, superseded_by) revision rows; scoped-down version per gap analysis (no acute backtester bug today). Sketch in docs/reviews/2026-08-12-dsa-lessons-gap-analysis.md. | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (round-2 backlog): truncated-replay lookahead harness** — empirical future-leak detector: recompute features with history truncated at decision time, diff vs full-history run (freqtrade lookahead-analysis port). | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (round-2 backlog): scoped protection locks (L)** — generalize accuracy-breaker into per-symbol/side TradeLock rows (stoploss-streak, drawdown, cooldown, losing-streak triggers), advisory-by-default. | **Planned** |
| 2026-08-12 | CLAUDE | **Planned (round-2 backlog): vector-memory decay + pruning (L)** — FinMem-style importance/recency scoring + corpus pruning for Pinecone (complements the WU breaker which only throttles new writes). Fable/memory swimlane. | **Planned** |
| 2026-08-12 | CLAUDE | **UI lane (owner request): symbol taps open company drawer everywhere + iOS fills-card redesign** — (W) reuse Market Scan's SymbolButton/useSymbolDrawer across activity, orders, watchlist, approvals, decisions, dashboard feed (+ PWA feasibility); (I1) iOS ActivityView fill cards: bolder/larger ticker + date/time, denser layout; (I2) new iOS SymbolInfoSheet tappable from fills/positions/orders/watchlist rows. Throwaway worktree trading-claude-ui, branch claude/ui-symbol-drawer-fills, own PR. iOS scope = ActivityView/MarketsView rows + new sheet only (no overlap with MONET tab-bar/parity lanes). | **In Progress** |
