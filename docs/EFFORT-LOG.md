...
- **[Socratic.Trade][UNCLAIMED] iOS adaptive-tabs follow-ups: run the tests, take the screenshots, unhang the iOS CI check — PLANNED 2026-08-21; item C claimed by CURSOR (`cursor/ios-ci-unhang-ac14`); item D CLOSED on main by #3012.**  Six independent items in `docs/rollouts/2026-08-21-ios-adaptive-tabs-followups.md`.  (A) XCTest execution moves to CI; **screenshots still need a Mac**.  (B) Rename `WrappingHStackTests.swift` -> `LayoutMathTests.swift`; still needs `xcodegen` on a Mac.  (C) claimed -- file-redirect + test lane.  (D) CLOSED -- pbxproj now has `PrivacyInfo.xcprivacy in Resources`.  (E) Owner decision: keep or drop tab auto-fill.  (F) Tuning knobs table.
