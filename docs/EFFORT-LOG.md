- **[Socratic.Trade][AG] Toggle styling, account-specific extended hours, and active short management — IN PROGRESS 2026-08-23 (branch `fix/settings-toggle-styling`).**  Fixed toggle switches deforming into 44px round circles on touch/mobile by moving coarse-pointer target to a `::before` hit area; added account-specific extended market hours hints across schedule and protective stop settings; ensure held short positions remain managed and protected across proactive risk exits, take-profit trims, and synthetic stops when `shortSellingEnabled` is false. Rollout: `docs/rollouts/2026-08-23-toggle-styling-extended-hours-short-management.md`.
