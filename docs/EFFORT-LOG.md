...
## In Progress
- **[Socratic.Trade][GROK4] Multi-wave expert-review implementation (claimed 2026-07-20) — IN PROGRESS.** PR #1847 `grok/multi-wave-a-onward`. Waves A + C partial + coach/lesson writers committed. **Wave D partial (coach/chat → durable learning) in flight 2026-07-20:** `src/lib/chat/coach-learning.ts` detects strategy directives + https URLs; strong directives → strategy-directive pending (approval); mild prefs → ingestLearned(origin ingest); URLs SSRF-safe fetch → fact lesson + optional lesson vector; orchestrator prepends honest receipt. Tests: `test/coach-learning.test.ts`. **Prod bge-m3 corpus re-embed STARTED 2026-07-21** (admin token via Infisical; dry-run 2644 sec-filings). Prior progress said completed with 0 candidates — re-running real embed. Poll GET /api/admin/reembed.
