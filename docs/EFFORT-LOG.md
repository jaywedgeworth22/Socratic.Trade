- **[Socratic.Trade][CURSOR] AppUpdatePrompt Apple IDs off Swift (live DealDex net.dealdex) — IN PROGRESS 2026-08-25 (branch `cursor/app-update-prompt-apple-ids-ddca`).**  Keep in-repo ios-fleet pin/copy.  Drop `knownAppleIds` (stale `online.dealdex`).  Prompt reads `appleId` from `jaywedgeworth22/ios-app-versions` / `scripts/ios-fleet/ios-app-versions.json`; registry live bundle is `net.dealdex` 6802474288.  Do not treat `online.dealdex` as live.  Do not upload `me.grok.dealdex`.  No Swift package.  No `testers.json`.  No `--force-ship`.  No spend.
