...
- **[Socratic.Trade][CURSOR] iOS Scan empty table is a quote miss — IN PR #2830 2026-08-18 (branch `cursor/scan-empty-screener-a128`).** Verified abort: 8s `controller.abort()` with no reason in `fetchNasdaqScreener`, timer held through 8000-row JSON (“This operation was aborted”); stub UA.  Not the 20s deadline.  `BROWSER_UA` + clear-on-headers + 12s named timeout + Yahoo whole-set + 503.  Rollout: `docs/rollouts/2026-08-18-scan-empty-screener.md`.
