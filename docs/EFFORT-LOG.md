...
## In Progress
- **Expensive admin-operation abuse/cost controls (CODEX, branch `codex/admin-rate-limits`, MERGED PR #1409, worktree `/Users/jay/.codex/worktrees/socratic-admin-rate-limits`) — COMPLETED; SHARED v1.5.0 FOLLOW-UP PR #1426 REFRESHING 2026-07-11.** Named per-admin anti-repeat budgets and contract-aligned 429/409 responses wrap high-cost admin actions (`reindex-8k`, `reindex-10k`, `backtest-ic`, `tuning-dry-run`, Congress score-eval/share, `refresh-websource`, Robinhood probe). PR #1409 merged as `9552b648`; that merge dropped the wrappers, but later Tradier merge `e3d04221` restored all eight. Current-main reconciliation verifies `requireAdmin` still precedes every guard, so #1426 now carries only the exact released/tag-install-verified shared `v1.5.0` pin, local HTTP-adapter delegation, shared-schema assertions, and restored real Auth.js provenance coverage. Pre-reconciliation Node24 gate green: focused 29/29, lint 0 errors/404 warnings, tsc, 329 files/3,684 tests, build. Current-main gate and hosted refresh pending. Scheduler/background convergence remains separately reserved.
