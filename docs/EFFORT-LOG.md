...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, READY PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, branch `codex/strategy-lease-correctness`) — IN PROGRESS; ADVERSARIAL FIXES GREEN 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Approval executions now use invocation-unique lock owners; a shared heartbeat guard records refused/thrown renewals and re-proves ownership before broker placement; obsolete wrong-signature teardown calls are removed. `SCHEDULER_SINGLE_LEADER` remains default-on even when the env value is unset/empty. Adversarial review fixed a pre-cleanup setup leak when the run receipt insert throws, added a typed approval `busy` result with no broker call after ownership loss, and preserves already-completed autonomous proposal results on a later loss. Node 24 focused verification is green (original 6 files/34 tests plus integration 4 files/22 tests), touched ESLint has 0 errors/35 inherited warnings, and TypeScript is clean; the parent serialized gate owns final-tree full verification. No scheduler provider-boundary, production-config, merge, or deploy work is included; current-main reconciliation and landing remain next.
