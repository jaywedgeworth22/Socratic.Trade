- **[Socratic.Trade][GROK] Surface asc-seq stderr + honest unverified die-text — IN PROGRESS 2026-08-30 (PR #3111, branch `compiler/asc-seq-surface-stderr`).**  CI-red was `ops-snapshot` `trial.active` after the 2026-08-27/08-30 pins expired; merged `origin/main` and pinned the fixture to now+7d.  iOS fleet intent unchanged.  No extra-ship.  No `--force-ship`.  Rollout: `docs/rollouts/2026-08-30-asc-seq-surface-stderr.md`.
- **[Socratic.Trade][MONET->CLAUDE] iOS OAuth return-to-app + workspace decode fixes — COMPLETED / Deployed to production 2026-08-29 (PR #3124, squash `f1d3a3e6f`, live `f1d3a3e6f`, board 0b58f822).**  Owner re-report after #3116/#3117.  MONET root-caused both and committed to the lane but never pushed — CLAUDE picked up the unpushed commit, re-verified both causes against live prod, hardened, and landed.  (A) auth-start clamped `callbackUrl` against `request.url` = INTERNAL container origin, collapsing every absolute mobile callback to "/" so the OAuth sheet stayed on the signed-in website; origin now from `resolvePublicAppOrigin()`, forwarded headers never trusted (Codex correctly flagged an open redirect in the first cut).  (B) "Couldn't load your workspace" was a snapshot DECODE failure, not Apple auth: server keys equity-curve points on `timestamp`, shipped Swift required `date` — latent until the FIRST curve point exists.  Mobile wire aliases `date` (fixes installed builds on deploy); Swift decodes either key.  Verified live: absolute callbackUrl round-trips, legacy GET entry translates, spoofed X-Forwarded-Host and cross-origin callbacks both clamp.  Rollout: `docs/rollouts/2026-08-27-ios-auth-redirect-workspace-decode.md`.
- **[Socratic.Trade][AG] Kalshi first-class account & tailored event views, real-time paper data, and universal shorting/options trading — COMPLETED / Deployed to production 2026-08-28 (PR #3122, squash `65c6a7021`, live `65c6a7021`).**  Link Kalshi accounts (demo & live) alongside broker accounts with customized event-contract settings, guardrails, portfolio, order execution, and LLM prompt views; remove delayed Alpaca paper keys from Infisical and guarantee real-time market data performance for paper accounts; build universal financial-grade shorting and options trading across all supported brokers (Alpaca, Tradier, Webull, Public, Test). Live sha verified via `scripts/verify-deploy-sha.sh`. Rollout: `docs/rollouts/2026-08-28-kalshi-and-options-expansion.md`.
- **[Socratic.Trade][CLAUDE] iOS workspace-load failure: snapshot latency + first-load retry — COMPLETED / Deployed to production 2026-08-27 (PR #3117, squash `795fb6004`).**  "Couldn't load your workspace" after Apple sign-in = timeout/retry asymmetry: one 30s iOS attempt, no retry, no cached snapshot on fresh sign-in vs getDashboardSnapshot's sequential 24s+16s+16s broker chain (Sentry showed real 13-20s loads in the degraded window).  Options now fetch concurrently with the portfolio bundle (the never-landed 2026-08-12 follow-up); iOS retries the first load once (45s) before failing.  Rollout: `docs/rollouts/2026-08-27-ios-snapshot-latency.md`.
- **[Socratic.Trade][CLAUDE] iOS Google/GitHub sign-in: server-side GET OAuth initiator + middleware translation — COMPLETED / Deployed to production 2026-08-27 (PR #3116, squash `d62a4adfd`; live-verified: legacy URL -> auth-start -> 307 accounts.google.com with PKCE cookies).**  Auth.js v5 only initiates OAuth on POST; the iOS `ASWebAuthenticationSession` GET dead-ended on `/access-denied?error=Configuration` since the flow shipped (#1859/#1886).  New `/api/mobile/auth-start` calls server-side `signIn()`; middleware translates the legacy `GET /api/auth/signin/<provider>` so installed TestFlight builds work with no app update; LoginView targets auth-start going forward.  Verified: 7 new vitest cases + live dev-server proof (307 → accounts.google.com with PKCE cookies).  Rollout: `docs/rollouts/2026-08-27-ios-web-oauth-get-initiator.md`.
- **[Socratic.Trade][CURSOR] Review-debt leftovers: ios-fleet publish guard + vitest exclude + ios-ship beta gate — COMPLETED (merged to `main`) 2026-08-26 (PR #3113, squash `17991fbcc`).**  Consolidates #3091/#3095/#3109.  Remote seed + refuse empty/key-drop on `publish-ios-versions.sh`; vitest excludes `scripts/**/*.test.mjs`; stale fixture test; ios-ship rejects beta in `xcodebuild -version`.  Also pins `PINECONE_TRIAL_ENDS_AT` in ops-snapshot test after trial calendar ended.  Rollout: `docs/rollouts/2026-08-26-review-debt-leftovers.md`.
- **[Socratic.Trade][CURSOR] CI autofix: APNs contract missing roic_status_advisory — IN PROGRESS 2026-08-26 (branch `cursor/ci-autofix-automation-9634`, stacked on #3107).**  `verify-hosted` failed because #3107 added the event type without the iOS `PushDeepLinkContractTests` Row.  Rollout: `docs/rollouts/2026-08-26-roic-apns-contract.md`.
- **[Socratic.Trade][AG] Resolve reviewer feedback on guardrail capabilities, market hours hints, unmanaged shorts, and stop fallbacks — COMPLETED (merged to `main`) 2026-08-26 (PR #3105, squash `4f51827b`).**  Pass live account capabilities into `mergeAccountCapabilities` in Guardrails page; align broker-specific `syntheticStopHoursHint` in `market-hours.ts` with executable order windows; fallback `shortTrailFallback` to `stopLossPct` when short stop loss is unconfigured; check effective stop distance configuration in `deriveUnmanagedShorts`.  Rollout: `docs/rollouts/2026-08-26-reviewer-fixes-guardrails-stophours.md`.

- **[Socratic.Trade][CURSOR] AppUpdatePromptTests pbxproj leftover after #3102 — COMPLETED (merged to `main`) 2026-08-25 (PR #3103, squash `64a06e78`).**  #3102 squash `df75ca6f` landed the pin/copy; hosted `ios-build` `32808123472` generated `AppUpdatePromptTests.swift` refs that the squash missed.  Generated pbxproj + privacy-manifest assert so TestFlight (no xcodegen) stays in sync.  Leftover `ios-build` `32809716228` / `32810441765` ** TEST SUCCEEDED ** (242/0).  `verify-hosted` `32809716125` success.  Do not `--force-ship`.  Do not dispatch `ios-ship.yml` unless the owner asks.  Rollout: `docs/rollouts/2026-08-25-app-update-prompt-pbxproj.md`.
- **[Socratic.Trade][CURSOR] AppUpdatePrompt Apple IDs off Swift (live DealDex net.dealdex) — COMPLETED (merged to `main`) 2026-08-25 (PR #3102, squash `df75ca6f`).**  Keep in-repo ios-fleet pin/copy.  Drop `knownAppleIds` (stale `online.dealdex`).  Prompt reads `appleId` from `jaywedgeworth22/ios-app-versions` / `scripts/ios-fleet/ios-app-versions.json`; registry live bundle is `net.dealdex` 6802474288.  Do not treat `online.dealdex` as live.  Do not upload `me.grok.dealdex`.  No Swift package.  No `testers.json`.  No `--force-ship`.  No spend.  Hosted `ios-build` `32808123472` ** TEST SUCCEEDED ** (242/0).  Generated pbxproj leftover is the row above.  Rollout: `docs/rollouts/2026-08-25-app-update-prompt-apple-ids.md`.
- **[Socratic.Trade][CURSOR] Playwright smoke: dismiss ConsentGate before More — COMPLETED (merged to `main`) 2026-08-25 (PR #3100, squash `1662cdcd`, smoke `32805872756` green).**  #3097 opened More on iPhone; first-use legal overlay intercepted the click on a fresh smoke DB.  Spec accepts the gate, then keeps More vs rail Scan.  Not TestFlight; do not dispatch `ios-ship.yml`.  Rollout: `docs/rollouts/2026-08-25-smoke-dismiss-consent.md`.
- **[Socratic.Trade][CURSOR] Playwright mobile-chrome smoke: Scan via More — COMPLETED (merged to `main`) 2026-08-25 (PR #3097, squash `c2ed2c85`).**  `getByText("Scan").first()` was the hidden desktop rail on iPhone.  Default pins omit Scan; smoke opens More.  Overlay leftover closed by #3100 (`1662cdcd`, smoke `32805872756` green).  Rollout: `docs/rollouts/2026-08-25-mobile-smoke-scan-more.md`.
- **[Socratic.Trade][CURSOR] Hosted macos-latest TestFlight ship (in-repo ios-fleet) — COMPLETED (merged to `main`) + TestFlight 1.0.69 2026-08-25 (PR #3089, squash `ef725f26`, ship run `32796413908`).**  Owner: do not run `xcodebuild` locally; do not restart the Mac runner.  Vendored CT `scripts/ios-fleet/` + copied ASC/P12 GitHub secrets (do not mint).  Merge-push archive `32794753487` cancelled mid-upload; next hosted tick uploaded **1.0.69 (202608250109)**, `IN_BETA_TESTING`.  #3028 chrome is in that binary.  Playwright smoke fail on `b9421cbf` is #3090, not this row.  Rollout: `docs/rollouts/2026-08-24-ios-hosted-testflight-ship.md`.
- **[Socratic.Trade][CURSOR] Datadog Logs + APM + RUM on existing us5 account — IN PROGRESS 2026-08-25 (branch `cursor/datadog-logs-apm-rum-6ce3`).**  Reuse `DD_API_KEY` / `DD_SITE` / RUM app id + client token.  No new org, key, or paid add-on.  Fail-closed no-op when keys are missing.  Server HTTP logs + `dd-trace` preload via Coolify `--import`.  Browser RUM on the web console only.  Sentry/PagerDuty unchanged.  Designer UX and Oracle RAG/Pinecone untouched.  Coolify deploy is not this PR.  Rollout: `docs/rollouts/2026-08-25-datadog-logs-apm-rum.md`.
...
- **[Socratic.Trade][CURSOR] Hosted macos-latest TestFlight ship (in-repo ios-fleet) — IN PROGRESS 2026-08-25 (branch `cursor/ios-hosted-testflight-ship-0e2b`, PR #3089).**  Owner: do not run `xcodebuild` locally; do not restart the Mac runner.  #3083 already set `runs-on: macos-latest`; the wrapper still exec'd `/Users/jay/apps/ios-fleet/ship-testflight.sh` so every hosted tick failed in ~15s.  Vendor CT `scripts/ios-fleet/` + `ios-appstore-gm-prepare.sh`, pin in-repo hashes, import ASC/P12 from GitHub secrets (copy from CT, do not mint).  Hosted unsigned `xcodebuild` green; `verify-hosted` failed on Sentry observer still listing `(Mac runner)` after YAML `name:` rename — observer + `CRON_SCHEDULES` retargeted.  #3028 iOS UI is on `main`; this is the TestFlight deploy path.  Slack #agent-sync stop-local-xcodebuild posted.  Rollout: `docs/rollouts/2026-08-24-ios-hosted-testflight-ship.md`.
- **2026-08-24 — AG — COMPLETED (merged to `main`) — Switch iOS CI & Actions workflows to GitHub-hosted cloud macOS runners (PR #3083, squash `49b17278`).**  `ios-build.yml` and `ios-ship.yml` `runs-on: macos-latest`.  Wrapper still pointed at `/Users/jay/apps/ios-fleet` until CURSOR `cursor/ios-hosted-testflight-ship-0e2b`.  Do not re-register `mac-xcode26-socratic`.
- **[Socratic.Trade][AG] Toggle styling, account-specific extended hours, and active short management — COMPLETED / Deployed to production 2026-08-23 (PR #3071, squash `120c6c4d7`, live `120c6c4d7`).**  Fixed toggle switches deforming into 44px round circles on touch/mobile by moving coarse-pointer target to a `::before` hit area; added account-specific extended market hours hints across schedule and protective stop settings; ensure held short positions remain managed and protected across proactive risk exits, take-profit trims, and synthetic stops when `shortSellingEnabled` is false. Live sha verified via `scripts/verify-deploy-sha.sh`. Rollout: `docs/rollouts/2026-08-23-manage-existing-shorts.md`.
- **2026-08-24 — ANTIGRAVITY — COMPLETED / PR OPEN — Kalshi & Guardrail Scope.** Hide Kalshi specific settings from Alpaca accounts. Use broker `caps` to explicitly label Short Selling and Options capabilities per account. Removed "Live vs Paper" capability wording. Verified via build. PR #3052 (amended).
- **[Socratic.Trade][AG] X (Twitter) OAuth 2.0 login integration — COMPLETED / Deployed to production 2026-08-23 (PR #3070, squash `fe813e6e0`).**  Copied `ST_X_CLIENT_ID` and `ST_X_CLIENT_SECRET` from `~/.secrets/global-api-keys` to Infisical as `AUTH_TWITTER_ID` and `AUTH_TWITTER_SECRET`. Added Twitter provider in `src/lib/auth/auth.ts` and X sign-in button in `app/login/page.tsx`. Verified build gate (tsc, lint, vitest, next build). Rollout: `docs/rollouts/2026-08-23-twitter-oauth-login.md`.
- **[Socratic.Trade][CLAUDE] Litestream L2/L3 empty wedge RESOLVED (ops heal, no code) — COMPLETED 2026-08-22.**  `litestream tier 2 empty wedged`: B2 L1 had 4 non-contiguous TXID holes (`85fd4->92b5`, `92b7->ad55b`, `ad572->b08a1`, `b08e7->ca40b`) so Compact could not promote L1->L2 and L2/L3 sat empty while L1 kept advancing.  Heal via S3 API (boto3 + the container's own `AWS_*` env; repo heal script's native-B2 key 401'd — stale): kept newest 48 contiguous L1, deleted 514 stale L1 (~1.5 GB), never L0/L9/cold-snapshots, no Coolify bounce.  L2 recompacted 17:02:43Z (first since 08-18), L3 17:10:23Z; L1 now 0 holes.  `/api/health` clears on next 30-min inventory.  Rollout: `docs/rollouts/2026-08-22-litestream-l2l3-unwedge.md`.
- **[Socratic.Trade][CURSOR] Full-stack review (web desktop/mobile, iOS, pipeline, RAG/embed/ingest, recent fills, broker, errors, connections) — FILED 2026-08-23 (branch `cursor/st-full-review-2026-08-23`, worktree `~/apps/trading-cursor-st-review`).**  Live health `2de5856fd` ok with `tradingLiveness` degraded (~3d since last completed run).  12 new board rows + GH #3056–#3067.  P0: Alpaca `stop_market` wire (#3056 / `d4cb5e75`), ingest budget misclass + requeue (#3057 / `9f875d62`), MCP place double-submit (#3058 / `ef0dccb3`).  P1: oldest-500 fills, skip+error counted as success, iOS uniqueKeys crash, web 401 freeze, liveness calendar, client_order_id provenance, embed-null retrieval.  P2: TestFlight/Playwright/freshness CI, Delayed Quote on session close.  Did not implement product fixes this session.  Review: `docs/reviews/2026-08-23-cursor-full-stack-review.md`.
- **[Socratic.Trade][CURSOR] Honor shared tickerLogoPolicy (CTS v2.6.0) — IN PROGRESS 2026-08-23 (branch `cursor/ticker-logo-policy`).**  `/api/logos/ticker` walks `sourceOrderFor` with GitHub-first fallback for ungraded names.  Disk cache is per source.  Rollout: `docs/rollouts/2026-08-23-ticker-logo-policy.md`.
- **[Socratic.Trade][AG] Full-system fixes & native iOS website parity — IN PROGRESS 2026-08-22 (branch `agent/antigravity-fixes-and-parity`).**  Frontend dependency hygiene (pruned xyflow/lightweight-charts, fixed /console/settings link, safe-area offset), backend DB composite query indexes (Migration 87), standardized error JSON, native iOS Swift Charts equity curve, scoped TimelineView invalidation, sensory haptics, and resilient SSE reconnection. Full gates verified (tsc, lint, vitest, next build, xcodebuild). Rollout: `docs/rollouts/2026-08-22-full-system-fixes-and-ios-parity.md`.
- **[Socratic.Trade][AG] Admin Operations knobs blur-commit & empty-default fallback fix — IN PR 2026-08-22 (branch `agent/ag-operations-knobs-blur-commit`, closes #2958).**  Fix per-keystroke PATCH calls and empty-field default overwrites in Admin > Operations, Tax Settings, Learning Review, and Strategy scoring weights. Add pure commit resolvers in `app/console/lib/number-commit.ts`. Verified full gates (tsc, lint, vitest, next build). Rollout: `docs/rollouts/2026-08-22-operations-knobs-blur-commit.md`.
- **[Socratic.Trade][AG] Hardened SQLite migrations with tableExists and columnExists guards — IN PR 2026-08-22 (branch `agent/ag-migration-table-info-guards`, closes #2964).**  Fix SQLite table_info empty-array trap across migrations 2-86 and backfill helpers with exported `tableExists` and `columnExists` helpers. Verified on blank `:memory:` DB and full 7,566-test suite. Rollout: `docs/rollouts/2026-08-22-migration-table-info-guards.md`.
- **[Socratic.Trade][CURSOR] Owner candlestick ST favicon, ASC listing icon, Android master — IN PROGRESS 2026-08-22 (branch `cursor/favicon-asc-android`, worktree `~/apps/trading-cursor-favicon-asc`).**  Website + future Android use owner transparent 1024 ST.  ASC listing is opaque RGB.  iOS App Icon not written.  GitHub OAuth callback documented as `https://socratictrade.com/api/auth/callback/github`.  Rollout: `docs/rollouts/2026-08-22-favicon-asc-android.md`.
- **[Socratic.Trade][GROK] Litestream L2/L3 mega-file unwedge + cascade last-resort + RAG hydrate — IN PR 2026-08-22 (branch `grok/litestream-cascade-rag`, worktree `~/apps/trading-grok-litestream-cascade`, boards 1e3df744 / b88b6675 / f7ffb62f, PR #3041).**  Mac B2 heal stuck SYN_SENT.  Cascade: Wave B alias + news drop + `/api/quote` Yahoo-floor 1.5s grace.  RAG: Red pack puts 1A/MD&A before cards; Item 7 reserve; Form4/13F/ARK corpus snapshots; skip signal extras under full-body.  Webpack: bare fs/http/path.  Do not flip `RAG_PINECONE_WRITE_CLASS`.  Keepout: Claude 06df80cf.  Rollout: `docs/rollouts/2026-08-22-litestream-cascade-rag.md`.
- **[Socratic.Trade][GROK] Integrate leftover Kimi peer-intraday token path — IN PROGRESS 2026-08-22 (branch `grok/kimi-integrate`, worktree `~/apps/trading-grok-kimi-integrate`, board `6a7fbecc`).**  Unique vs main from retired-KIMI #3044: operator Robinhood for CT peer INTRADAY uses stored `local` token + `operatorPeerRead` after `APP_B_INGEST_TOKEN`.  Env var is boot-seed only.  Did not take #3044 CI/lockfile/png deletes (already on main or regressive).  Broker-conformance Aug 13 patch already on main.  Targeted tests 21/21.
- **[Socratic.Trade][CURSOR] Alpaca Paper Approve greyed after Retry Red Team — COMPLETED (merged to `main`) + Deployed to production 2026-08-24 (PR #3049, squash `06a5418b`; docs #3087 `3fcf876e`).**  Retry no longer greys Approve; retry applies Red verdict; owner Approve opens in Exit-only.  Live sha `dcfd04d7` contains fix (re-verified 23:45Z).  Native iOS stale-gate is NOT on TestFlight: `ios-ship.yml` on `macos-latest` missing `/Users/jay/apps/ios-fleet` (80 red scheduled ticks); no ASC secrets on the GitHub repo.  Website is the live path.  Rollout: `docs/rollouts/2026-08-22-alpaca-paper-approve-greyed.md`.
- **[Socratic.Trade][AG] Test DB isolation & UI sparkline gradient cleanup — IN PROGRESS 2026-08-22 (branch `agent/ag-test-isolation-and-ui-cleanup`).**  Isolate test SQLite DB in `test/api-keys-tombstone.test.ts` to prevent credential leakage across test suites; add `afterAll` unstub cleanup in `test/encryption-key-guard.test.ts`; fix hardcoded SVG gradient IDs in `ServerMetricsClient` via `useId()`. Rollout: `docs/rollouts/2026-08-22-test-isolation-and-ui-cleanup.md`.
- **[Socratic.Trade][CURSOR] Rename WrappingHStackTests.swift -> LayoutMathTests.swift + Mac CI xcodegen — IN PR 2026-08-21 (PR #3027, branch `cursor/ios-layout-math-tests-rename-3110`).**  Independent re-check of adaptive-tabs follow-ups: C/D/A-tests already on main (#3023/#3012, Mac 32529663287 232/0).  Item B done: file renamed; Mac job 32534166394 `success` (xcodegen 2.46.0, BUILD/TEST SUCCEEDED); generated pbxproj committed from artifact.  Screenshots / auto-fill / knobs still open.  Rollout: `docs/rollouts/2026-08-21-ios-layout-math-tests-rename.md`.
- **[Socratic.Trade][CURSOR] Activity layout redesign — IN PR 2026-08-21 (PR #3031, branch `cursor/activity-layout-redesign-085b`, rematched onto `origin/main` `cd4c700d` / #3018).**  Website + iOS Activity: Alerts Center, Notifications, Strategy Runs, Order Fills, Audit Log (former All).  `ios-build` 233/0.  Tooltip sentence-gap + AuditLogPanel wiring marker fixed.  Rollout: `docs/rollouts/2026-08-21-activity-layout-redesign.md`.
- **[Socratic.Trade][CURSOR] iOS wide-window layout + Admin tab + chrome (gear/bell) — IN PR 2026-08-21 (PR #3028, branch `cursor/ios-wide-layout-admin-tab-0e2b`).**  Adaptive regular-width layout (sidebar-adaptable TabView, larger settings/inbox sheets, native Admin sidebar).  Gear top-trailing on every tab.  Bell top-leading opens the notification inbox (price-alert composer deleted from Assets).  Admin Portal is a first-class tab for `isAdmin` only; WKWebView intercepts "Back to Console" instead of loading the website.  Owner merged `origin/main` to `472f3cfe` (#3032); `verify` 32540455277 and `ios-build` 32540455303 succeeded.  Then #3031 (`d588387b`) made GitHub CONFLICTING.  Real conflicts in ActivityView + MobileControlView: keep #3031 five-section Activity; keep this PR's `NotificationsInboxView`; Assets More copy has no price alerts.  Rollout: `docs/rollouts/2026-08-21-ios-wide-layout-admin-tab.md`.
- **[Socratic.Trade][CURSOR] iOS wide-window layout + Admin tab + chrome (gear/bell) — COMPLETED (merged to `main`) 2026-08-22 (PR #3028, squash `a851a68d` / `a851a68da16b9c7ec722897a3ab4f378e0117111`).**  Adaptive regular-width layout (sidebar-adaptable TabView, larger settings/inbox sheets, native Admin sidebar).  Gear top-trailing on every tab.  Bell top-leading opens the notification inbox (price-alert composer deleted from Assets).  Admin Portal is a first-class tab for `isAdmin` only; WKWebView intercepts "Back to Console" instead of loading the website.  `ios-build` 32536626553 on `b747872d` was 237/0.  Later rematch `8471509c` failed `ios-build` 32541612565 (duplicate `NotificationHistoryRow`); owner fix `ac9bafd5`.  iOS-only; not Deployed to production (`ios/**` outside Coolify `watch_paths`); TestFlight only if the owner asks.  Rollout: `docs/rollouts/2026-08-21-ios-wide-layout-admin-tab.md`.
- **[Socratic.Trade][CURSOR] Activity layout redesign — COMPLETED (merged to `main`) 2026-08-21 (PR #3031, squash `d588387b`).**  Website + iOS Activity: Alerts Center, Notifications, Strategy Runs, Order Fills, Audit Log (former All).  `ios-build` 233/0.  Tooltip sentence-gap + AuditLogPanel wiring marker fixed.  Rollout: `docs/rollouts/2026-08-21-activity-layout-redesign.md`.
- **[Socratic.Trade][GROK] Honor 5M WU this week then snap to free-tier 2026-08-27 — COMPLETED/MERGED 2026-08-22 (PR #3037, squash `f5500b610`, board 603fef7b).**  Infisical monthly 5M, trial end Aug 27.  Runtime snap 60k/20k/1.6M.  Cursor #3038 stacked: ignore numberless monthly-WU 429s during trial.  Website auto-deploy not yet claimed (live sha still `a84ce0bed`).  Rollout: `docs/rollouts/2026-08-21-pinecone-free-snap.md`.
- **[Socratic.Trade][GROK] Expert panel: condense RAG for Green/Red — COMPLETED/MERGED 2026-08-22 (PR #3037, board 596abb59).**  Highlights/signal/analog cards in Pinecone; full filings/transcripts/ledgers on our server.  Synthesis: `docs/audits/2026-08-22-panel-synthesis.md`.  Do not flip `RAG_PINECONE_WRITE_CLASS` until PR B.  Builder only if synthesis §5 gates pass.
- **[Socratic.Trade][GROK] Stop paging Starter 2M Pinecone write units — COMPLETED/MERGED 2026-08-21 #3032 `c6f33086`.** (branch `grok/pinecone-wu-cap`, worktree `~/apps/trading-grok-pinecone-wu`, board 7b0bf414).**  Owner: change monthly WUs and stop the notifications.  Live org is Standard trial (unlimited WUs).  Matcher missed official monthly-quota text without `429` in the body → hourly connection-failed.  Breaker only parks when a monthly budget is set; post-trial no 1.6M snap.  Infisical `PINECONE_MONTHLY_WU_BUDGET=0`.  Rollout: `docs/rollouts/2026-08-21-pinecone-wu-no-starter-wall.md`.
- **[Socratic.Trade][CURSOR] Abort keyed Finnhub wave on gather timeout — IN PR #3018 2026-08-21 (branch `cursor/gather-timeout-abort-0388`).**  Leftover after #3013 `eef81eb6`: first gather-abort dropped `AbortSignal` on keyed/scarce waves so Finnhub's paced free-tier queue kept running after the 8m deadline.  Not a paid Finnhub plan — Wave B means has an API key.  Rematched onto `origin/main`.  Parent owns Coolify.  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
- **[Socratic.Trade][CURSOR] Activity layout redesign — COMPLETED (merged to `main`) 2026-08-22 (PR #3031, squash `d588387b`).**  Website + iOS Activity: Alerts Center, Notifications, Strategy Runs, Order Fills, Audit Log (former All).  Plain-English run failures.  APNs-first + admin system alerts.  Alpaca session-close not Delayed Quote.  Additive `?tab=` deep links only.  Live sha at merge was still `c6f33086` — website auto-deploy not claimed.  iOS via TestFlight.  Rollout: `docs/rollouts/2026-08-21-activity-layout-redesign.md`.
- **[Socratic.Trade][GROK] Stop paging Starter 2M Pinecone write units — COMPLETED (merged to `main`) 2026-08-22 (PR #3032, squash `c6f33086`).**  Owner: change monthly WUs and stop the notifications.  Live org is Standard trial (unlimited WUs).  Matcher missed official monthly-quota text without `429` in the body → hourly connection-failed.  Breaker only parks when a monthly budget is set; post-trial no 1.6M snap.  Infisical `PINECONE_MONTHLY_WU_BUDGET=0`.  Rollout: `docs/rollouts/2026-08-21-pinecone-wu-no-starter-wall.md`.
- **[Socratic.Trade][CURSOR] Abort keyed Finnhub wave on gather timeout — COMPLETED (merged to `main`) 2026-08-21 (PR #3018, squash `cd4c700d`).**  Leftover after #3013 `eef81eb6`: first gather-abort dropped `AbortSignal` on keyed/scarce waves so Finnhub's paced free-tier queue kept running after the 8m deadline.  Not a paid Finnhub plan — Wave B means has an API key.  Parent owns Coolify.  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
- **[Socratic.Trade][CURSOR] Abort abandoned strategy gather + last-good tape — COMPLETED/MERGED 2026-08-21 (PR #3013, squash `eef81eb6`).**  Abort the controller, honor the signal in cascade/enrichment, reuse last completed MarketScan + 45s quote refresh so Green can start.  Follow-up leftover is #3018 (keyed Finnhub wave).  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
- **[Socratic.Trade][GROK] Website login provider family after iOS #3008 — IN PROGRESS 2026-08-21 (branch `grok/login-unify-web`, worktree `~/apps/trading-grok-login-unify`, board 02b5cf01).**  iOS already ships one family.  Website still had teal Google + outline GitHub + black Apple.  Shared Google Light/Dark chrome, official Invertocat, Apple ink stays pure black/white.  Rollout: `docs/rollouts/2026-08-21-web-login-provider-family.md`.
- **[Socratic.Trade][DEEPSEEK] Full-stack review: desktop web + mobile web + iOS app — COMPLETE + HANDED OFF 2026-08-21 (owner directed DEEPSEEK not to implement; handoff IN PR #2990, board 682e7e3467cd4def97a13ee67335cbb1).**  Review: outline merged #2982; all four track reports merged #2984 (`docs/audits/2026-08-20-deepseek-*`); ~105 findings, 6 P1s.  Handoff: master note `docs/handoffs/2026-08-21-deepseek-review-handoff.md` + per-area handoffs (desktop-web/mobile-web/ios/backend-ops) with file:line anchors, fix approaches, failing-first tests, verify commands, pitfalls.  All 23 review-findings open on the board; claims released.  First wave (S): 99ab01c7, 8c9ce3b9, d9f81e44, c53ad066, cdee1562, 410bda84, 3a8bcdcf, 28c50b28, 7e93bbcc, 67558af0.  M-effort offers: 34b8fbe7, 179cc4b2, 06edecc0, acc07df6, 1dbdc227, 220c6cc6.
- **[Socratic.Trade][CURSOR] Unhang ios-build SWBBuildService pipe + run XCTests — IN PROGRESS 2026-08-21 (branch `cursor/ios-ci-unhang-ac14`).**  Adaptive-tabs follow-up item C: redirect `xcodebuild` stdout/stderr to a file so the daemon cannot hold the Actions step open; add `xcodebuild test` on the same Mac job (destination discovered: iPhone 17 Pro, else 16 Pro, else any iPhone).  Item D already on `main` via #3012; vitest now asserts `PrivacyInfo.xcprivacy in Resources` in the pbxproj.  Did not make `ios-build` required.  Did not rename `WrappingHStackTests.swift`.  Did not take iPad screenshots.  Rollout: `docs/rollouts/2026-08-21-ios-ci-unhang.md`.
- **[Socratic.Trade][CURSOR] Unhang ios-build SWBBuildService pipe + run XCTests — COMPLETED (merged to `main`) 2026-08-21 (PR #3023, squash `7ba178e1`).**  Item C verified: Mac job run 32529663287 concluded `success` in ~2 min (not `cancelled`); BUILD SUCCEEDED; TEST SUCCEEDED; 232 XCTests / 0 failures including all 30 `TabPreferencesTests` on iPhone 17 Pro.  Item D already on `main` via #3012.  Screenshots / rename / auto-fill veto still open.  `ios-build` can now be made required (board `830c892f`).  Rollout: `docs/rollouts/2026-08-21-ios-ci-unhang.md`.
- **[Socratic.Trade][CURSOR] Scan card tap + honest iOS last/next run — IN PR 2026-08-21 (PR #3019, branch `cursor/ios-scan-card-schedule-e224`, merged onto `origin/main` `e3278a5f`).**  Whole scan card opens company info (iOS + website phone cards).  iOS company name is primary/semibold vs industry secondary.  `getSchedulerState` hydrates last run from `strategy_runs` and fills next run while autonomy is active (including after restart / cash close).  Last Run copy is `never`, never "not scheduled".  Rollout: `docs/rollouts/2026-08-21-ios-scan-card-schedule.md`.
- **[Socratic.Trade][CURSOR] Abort abandoned strategy gather + last-good tape — IN PROGRESS 2026-08-21 (branch `cursor/gather-timeout-abort-0388`).**  Prod 2026-08-21 RTH: Roth + Paper every run `strategy gather timeout` after yesterday's 20:16/20:22Z completes.  Live `e0a4959a` already has #2852/#2854.  8m gather deadline was a pure race, so timed-out scans kept running and stacked.  Abort the controller, honor the signal in cascade/enrichment, reuse last completed MarketScan + 45s quote refresh so Green can start.  Do not HOTFIX during cash hours.  Rollout: `docs/rollouts/2026-08-21-gather-timeout-abort.md`.
- **[Socratic.Trade][GROK] RTH drain actually nudges Coolify — IN PROGRESS 2026-08-21 (branch `grok/rth-drain-nudge`, worktree `~/apps/trading-grok-drain`, board 99ab01c7).**  Live sha `e0a4959a73a7` is 39 commits behind main because weekday image builds are RTH-latched and the 21:20 UTC drain exited 0 without a nudge (GITHUB_TOKEN cannot redeliver hooks; `COOLIFY_DEPLOY_WEBHOOK_URL` unset).  Drain now uses GH_PAT + Coolify deploy API and fails the job when it cannot nudge.  Do not HOTFIX during cash hours.  Rollout: `docs/rollouts/2026-08-21-rth-drain-nudge.md`.
- **[Socratic.Trade][CURSOR] Native weekly value + momentum screens (option 3) — IN PR 2026-08-21 (PR #3009, branch `cursor/weekly-market-screens-2b0c`).**  Rebuild the owner's Perplexity weekly ritual from ST quotes + field store + daily bars.  Cards on Macro/Scan; iOS Scan thin card; Green `weeklyScreens` DATA only (`agentic-strategy@2.15.0`).  No Perplexity key/scrape.  Rollout: `docs/rollouts/2026-08-21-weekly-market-screens.md`.
- **[Socratic.Trade][CURSOR] Three-column LLM catalog (display / OpenRouter wire / native) — COMPLETED/DEPLOYED 2026-08-21 (PR #3003, squash `36f0a3c8`, 09:56 UTC).**  Owner table is the catalog.  Settings store display slugs; live OpenRouter calls send column 2; `nativeSlugFor` exports column 3.  Aliases map gpt-5.4-mini / claude-sonnet-5 / grok-4.5 / deepseek-v4-* / gemini-3.5-flash-lite onto the new display slugs.  Merge == auto-deploy.
- **[Socratic.Trade][CLAUDE] Login hero wordmark + equal-size provider buttons + expanded legal copy — IN PR #3002 2026-08-21 (branch `claude/login-hero-copy`, worktree `~/apps/trading-claude-login`).**  Owner screenshot review of the iOS sign-in screen, mirrored on the website login.  New first bullet "Configure strategic framework and guardrails"; wordmark scaled from a 24pt glyph to a column-width headline (`CandleWordmarkView(fillsWidth:)`, web `HeaderLogo` 20 -> 30); all three provider buttons at one width/height/radius (content column 400 -> 352 so Apple's 375pt hard cap stops clipping it narrower); owner's expanded disclaimer with Terms/Privacy moved below the paragraph.  Apple's own label size left alone deliberately — matching it needs a custom control and takes on branding compliance at App Review.  Gates: tsc clean, lint 0 errors, 652 files / 7306 tests, `npm run build` ok, `xcodebuild` BUILD SUCCEEDED, simulator screenshot.  Rollout: `docs/rollouts/2026-08-20-login-hero-and-legal-copy.md`.
- **[Socratic.Trade][CLAUDE] Width-aware iOS/Mac tab bar (>4 tabs on iPad and wide Mac) + iPad Air 11" layout pass — COMPLETED (merged to `main`) 2026-08-21 (PR #2987, squash `9298c29`, 06:56 UTC).**  Supersedes the IN PROGRESS row below; that row is left in place per append-mostly.  `TabBarCapacity` converts live width to slots (compact 4; iPad Air 11" 6 portrait / 8 landscape; ceiling 8, floor 2); Home required; the slot before More is borrowed by whatever is opened from the More list; too narrow falls back to the DEFAULT tabs without writing storage; an uncustomized bar auto-fills to what fits.  `SnapshotScaffold` picks 1/2/3 card columns — one column is the pre-existing `LazyVStack`, so the iPhone path is unchanged.  **NOT deployed and not shippable-verified:** `ios/**` is outside Coolify `watch_paths` so no production deploy fires, and the iOS app only changes when `scripts/ios-ship-testflight.sh` runs.  Swift compiles (Mac runner `** BUILD SUCCEEDED **` twice, 05:22:30 and 06:21:48); `verify` green.  Rollout: `docs/rollouts/2026-08-21-ios-adaptive-tabs-ipad-layout.md`.
- **[Socratic.Trade][GROK] iOS adaptive-tabs item A screenshots (iPad Air 11 + Mac window) — PLANNED 2026-08-21 (local Mac; Cursor local is down).**  Brief: `docs/rollouts/2026-08-21-ios-adaptive-tabs-mac-qa.md`.  Start from `origin/main` in `~/apps/trading-grok-tabs` (do not clobber `trading-grok-drain`).  Shots do not need #3027 (`origin/main` merged 2026-08-21).  Do not redo B/C/D.  Do not touch E/F unless shots prove it.
- **[Socratic.Trade][CURSOR] iOS adaptive-tabs follow-ups leftovers after #3023 — IN PR 2026-08-21 (item B on PR #3027).**  Item C VERIFIED (Mac job 32529663287 `success`, 232/0).  Item D CLOSED on main by #3012.  Item B rename done (LayoutMathTests.swift + Mac CI xcodegen, job 32534166394).  Still open: (A) screenshots of iPad Air 11" / borrowed slot / Mac window drag; (E) owner keep-or-drop auto-fill; (F) tuning knobs.  Local Grok brief: `docs/rollouts/2026-08-21-ios-adaptive-tabs-mac-qa.md`.  Note: `docs/rollouts/2026-08-21-ios-adaptive-tabs-followups.md`.
- **[Socratic.Trade][CLAUDE] Width-aware iOS/Mac tab bar (>4 tabs on iPad and wide Mac) + iPad Air 11" layout pass — IN PROGRESS 2026-08-21 (branch `claude/dynamic-menu-tabs-ipad-mac-phddkr`, Claude Code web/cloud session).**  Owner ask: more than four tabs on iPad and when a Mac window is wider; fall back to the DEFAULT tabs when too narrow (without losing the stored picks); Home required; the slot before More (Activity by default) swapped out by whatever screen is opened from the More list; and "make the layout actually nicer for iPad Air 11" in all other ways too", carried into the Mac app.  New `TabBarCapacity` converts live width to slots (compact 4; 820pt portrait 6; 1180pt landscape 8; ceiling 8; floor 2), `TabPreferences` gains `requiredTab`/`dynamicTab`/`capacity`/`hasCustomSelection` + a pure `resolve(barTabs:dynamicTab:capacity:)`, and the `Tab` blocks become POSITIONAL (`bar[0]`…`bar[8]`) so the borrowed occupant renders in the slot before More — still unrolled (ForEach+Tab times out the type-checker) and still exactly ten result-builder children.  An uncustomized bar auto-fills to what fits, so a fresh iPad shows more than four without anyone asking.  More carries an aggregate badge for tabs the bar cannot show.  Layout: `ContentColumns` + `CardColumns` (a `Layout`, math in testable `CardColumnsLayout`) give `SnapshotScaffold` 1/2/3 columns with `.cardSpansAllColumns()` for banners and heroes — **one column is the existing `LazyVStack` untouched, so the iPhone does not change**; `AppMetricGrid` replaces six hardcoded 2-up grids and widens with the CARD not the screen; Coach keeps a 760pt readable measure; the Home hero CTA is capped at 520pt.  30 XCTests on the tab model + 15 on the layout math.  **NOT COMPILED: this is a Linux remote container (no `xcodebuild`/`swiftc`/`xcodegen`/simulator) and the repo's tsc/vitest/build gate compiles no Swift — `.github/workflows/ios-build.yml` on the Mac runner is the verification of record.**  No new test FILE (the checked-in `.xcodeproj` uses explicit file refs and `xcodegen` cannot run here).  Web console `MOBILE_TABS_MAX` deliberately untouched — iOS and web now diverge on the ceiling.  Rollout: `docs/rollouts/2026-08-21-ios-adaptive-tabs-ipad-layout.md`.
- **[Socratic.Trade][DEEPSEEK] Full-stack review: desktop web + mobile web + iOS app — COMPLETED/MERGED 2026-08-20 #2982 `f671c5b2`, track reports IN PR #2984 (branch `deepseek/backend-review`, board 682e7e3467cd4def97a13ee67335cbb1).**  Zero-code review: prioritized fix/improve outline in `docs/reviews/2026-08-20-deepseek-full-review.md`; rollout `docs/rollouts/2026-08-20-deepseek-full-stack-review.md`.  All four deep-dive tracks COMPLETED and committed under `docs/audits/2026-08-20-deepseek-{backend-ops-docs,api-routes,scripts,docs,desktop-web,ios,mobile-web}.md` — ~105 findings, 6 P1s (approve dead-end cf62f87a, Guardrails Discard c53ad066, iOS dict crash d9f81e44, drain exit-0 99ab01c7, deployment B2 mislabel 8c9ce3b9, plus state-confirmed 830c892f/410bda84).  23 new board findings; state comments on 10 open items.  Verified good: exit-code contract, admin gate, identity chain, scheduler safety, broker-I/O discipline, xcodebuild clean, tone-token AA, light-default theme.
- **[Socratic.Trade][DEEPSEEK] Full-stack review: desktop web + mobile web + iOS app — COMPLETED/MERGED 2026-08-20 #2982 `f671c5b2` (branch `deepseek/lane`, board 682e7e3467cd4def97a13ee67335cbb1).**  Zero-code review: prioritized fix/improve outline in `docs/reviews/2026-08-20-deepseek-full-review.md`; rollout `docs/rollouts/2026-08-20-deepseek-full-stack-review.md`.  New board findings da8a93bf (404 pages), 34b8fbe7 (CSP report-only), 436a9b98 (prod 8 behind main + 1 MB live icon); state comments on 830c892f/89249c60/3b343933/620ef423.  Deep-dive tracks stalled on model calls — re-run listed as follow-up.
- **[Socratic.Trade][CURSOR] Web / mobile-web / iOS parity audit — IN PROGRESS 2026-08-17 (branch `cursor/web-ios-parity-audit-e83a`).** Report-only.  Matrix + P1 deep-link/Activity gaps + safe PWA deletion plan in `docs/audits/2026-08-17-web-ios-parity.md`.  No product code.
- **[Socratic.Trade][CLAUDE] Login hero wordmark + equal-size provider buttons + expanded legal copy — IN PROGRESS 2026-08-20 (branch `claude/login-hero-copy`, worktree `~/apps/trading-claude-login`).**  Owner screenshot review of the iOS sign-in screen, mirrored on the website login.  New first bullet "Configure strategic framework and guardrails"; wordmark scaled from a 24pt glyph to a column-width headline (`CandleWordmarkView(fillsWidth:)`, web `HeaderLogo` 20 -> 30); all three provider buttons at one width/height/radius (content column 400 -> 352 so Apple's 375pt hard cap stops clipping it narrower); owner's expanded disclaimer with Terms/Privacy moved below the paragraph.  Apple's own label size left alone deliberately — matching it needs a custom control and takes on branding compliance at App Review.  Gates: tsc clean, lint 0 errors, 652 files / 7306 tests, `npm run build` ok, `xcodebuild` BUILD SUCCEEDED, simulator screenshot.  Rollout: `docs/rollouts/2026-08-20-login-hero-and-legal-copy.md`.
- **[Socratic.Trade][DEEPSEEK] Full-stack review: desktop web + mobile web + iOS app — IN PROGRESS 2026-08-20 (branch `deepseek/lane`, worktree `~/apps/trading-deepseek`, board 682e7e3467cd4def97a13ee67335cbb1).**  Top-to-bottom review of the console site at desktop + phone widths and the native iOS app.  Deliverable: prioritized fix/improve outline filed as board findings + a review doc; docs-only PR.  Complements the 2026-08-18 expert review (references its cluster ids) and the open Kimi board findings — verifies current state rather than re-filing.
- **[Socratic.Trade][CURSOR-BUGBOT] #2853 drain adopt steals a frozen-but-live Manual Run — IN PR 2026-08-20 (branch `cursor/drain-adopt-live-heartbeat`).** Heartbeat age >90s was treated as a dead worker.  An event-loop freeze pauses the 15s beat while `runStrategyOnce` is still on the stack; drain released that run's lock and started a second gather/place.  Liveness is now map presence on this process.  Rollout: `docs/rollouts/2026-08-20-drain-adopt-live-heartbeat.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] Owner-cancel protective-stop tombstone on cancel timeout — IN PR 2026-08-20 (branch `cursor/cancel-timeout-stop-tombstone`).** #2949 wrote the tombstone only after `cancelEquityOrder` returned.  #2886 deadlines can throw after the broker accepted the cancel, so the next reconcile re-placed the stop.  Tombstone on throw; leave the tracked row.  Rollout: `docs/rollouts/2026-08-20-owner-cancel-stop-tombstone-timeout.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] Alpaca MCP getEquityOrders hid just-filled orders — IN PR 2026-08-20 (branch `cursor/alpaca-mcp-orders-include-terminal`).** #2886 left MCP `get_orders` at `status:"open"` while `ordersListIncludesTerminal` stays true, so a just-filled market order looks never-placed and retries.  REST open+closed path unchanged.  Rollout: `docs/rollouts/2026-08-20-alpaca-mcp-orders-include-terminal.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] #2953 peer quotes/intraday 401 at the edge — IN PR 2026-08-20 (branch `cursor/peer-quotes-intraday-middleware`).** Middleware bearer pass-through still listed only `/api/market/prices/` and `/api/market/spx`.  Peer `APP_B_INGEST_TOKEN` calls to the new routes 401 before the handler token check.  Same exception, flatfile still session-gated.  Rollout: `docs/rollouts/2026-08-20-peer-quotes-intraday-middleware.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] #2953 intraday provider failure is 502 — IN PR 2026-08-20 (branch `cursor/intraday-provider-fail-502`).** `fetchIntradayBars` collapsed Alpaca 403/timeout/missing creds into the same `null` as a confirmed empty window; the route served `200 { bars: [] }`, so CT skipped fallback and kept writing `missed_window`.  Discriminate `ok` vs `unavailable`.  Did not touch #2957/#2947/#2952.  Rollout: `docs/rollouts/2026-08-20-intraday-provider-fail-502.md`.
- **2026-08-20 — CLAUDE — IN PR — ST->CT price service: real-time quotes + intraday bars, FMP-free (branch `claude/ct-price-service`).**  CT latency capture sourced every price from one FMP key that 402d in prod: 7 prices recorded out of 2955 snapshots (2937 `missed_window`).  Owner ruling: FMP never for market data (still valid as a latency competitor being timed).  New `src/lib/market-realtime.ts` + token-gated peer routes `/api/market/quotes` and `/api/market/intraday/[symbol]` on the existing APP_B_INGEST_TOKEN path.  INTRADAY is the important half — CT schedules snapshots retrospectively so a live quote can never honestly answer a past due-time; minute bars can, and allow rebuilding history.  Robinhood first (15s floor vs Alpaca 1min; ~100 research calls with no quota refusal) gated on the OPERATOR-level bypass because the Robinhood token is per-user and a peer route has no user in scope.  Delayed Yahoo quotes are opt-in and flagged `delayed: true`.  ROIC evaluated and REJECTED: daily-only, no interval param, latest = daily close with 4h cache — cannot answer "price now" or "price at 14:43".  Gates: lint 0 errors; tsc clean; 639 files / 7191 tests; build ok.  DO NOT MERGE until prod catches up — ST recovered on last-good image 77d7d7b6 and is 4 merges behind main.  Receipt: `docs/rollouts/2026-08-20-ct-price-service.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] Tradier default getEquityOrders must keep live pending_cancel GTC stops — IN PR 2026-08-20 (branch `cursor/tradier-live-pending-cancel-scope`).** #2886 24h filter treated pending_cancel / pending_replace as terminal, hiding a week-old GTC stop mid-cancel from liveExitOrderCoverage.  Use isLiveOrderState.  Distinct from #2947.  Rollout: `docs/rollouts/2026-08-20-tradier-live-pending-cancel-scope.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] Tradier default getEquityOrders must keep live pending_cancel GTC stops — IN PR #2952 2026-08-20 (branch `cursor/tradier-live-pending-cancel-scope`).** #2886 24h filter treated pending_cancel / pending_replace as terminal, hiding a week-old GTC stop mid-cancel from liveExitOrderCoverage.  Use isLiveOrderState.  Distinct from #2947.  Rollout: `docs/rollouts/2026-08-20-tradier-live-pending-cancel-scope.md`.
- **[Socratic.Trade][CURSOR-BUGBOT] Owner-cancel protective-stop tombstone on lookup miss — IN PR #2949 2026-08-20 (branch `cursor/owner-cancel-stop-tombstone-lookup`).** #2882 required `lookup.order.symbol` to write the do-not-replace tombstone.  Console cancel is fail-open on that advisory read, so a timeout/empty list cancelled the broker order and then the next reconcile re-placed the stop.  Use the tracked `broker_protective_stops` symbol.  Did not touch #2861.  Rollout: `docs/rollouts/2026-08-20-owner-cancel-stop-tombstone-lookup.md`.
- **[Socratic.Trade][CURSOR] Stale hosting/stack copy sweep — IN PROGRESS 2026-08-20 (branch `cursor/stale-hosting-docs-b392`).** Docs/metadata only.  Production is Coolify at socratictrade.com (README + GitHub About already current).  Fixing PLAN Current Status / acceptance (Mac pm2 + `trading-beta`), leftover AGENTS preview/PWA/Mac-pm2 present-tense, ops-diagnostics `trading.jays.services`.  PWA UI retired #2801.  No product code.  Rollout: `docs/rollouts/2026-08-20-stale-hosting-docs.md`.
...
- **[Socratic.Trade][CURSOR] Delayed Yahoo fallback stamp + keep trading — REBASED 2026-08-20 (PR #2818, branch `cursor/delayed-yahoo-fallback-stamp-c120`).** Rebase-only onto `origin/main` `ce31c367`.  Conflicts: DataSourcesSettings + HomeView — kept main (copy already landed; #2857 Desk subtitle fold kept).  Scope stayed Delayed Quote stamp + keep trading.  Do not merge from this rebase.  Rollout: `docs/rollouts/2026-08-18-delayed-yahoo-fallback-stamp.md`.
- **[Socratic.Trade][CURSOR] Notification history + web/iOS inbox parity — IN PR #2841 2026-08-20 (branch `cursor/notification-history-parity-4bbc`).** Rebased onto `origin/main` `1d6bbf68` after #2892/#2876/#2942/#2834.  One last-100 inbox field; unread badges + header inbox kept.  Did not absorb other clusters.  Not merging.  Rollout: `docs/rollouts/2026-08-18-notification-history-parity.md`.
...
- **[Socratic.Trade][CURSOR] Alert-noise leftover: retired FilingAPI + live boot pages — IN PROGRESS / rematched 2026-08-20 (PR #2798, branch `cursor/alert-noise-retired-boot-64c1`).** Unique leftover after #2799/#2800: 5 min `DB_BOOTSTRAP=live` connection-alert mute + stamp `intentionalOff` so leftover 401s cannot paint retired vendors STOPPED.  Kept main's `pineconeIngest` and kind-based hard-stop.
- **[Socratic.Trade][CURSOR] Web / iOS parity P1+P2 fixes — IN PR #2942 2026-08-20 (branch `cursor/web-ios-parity-fixes-e83a`).** Audit #2804 P1+ranked P2: `?proposal=` / `?symbol=` focus, iOS Activity alerts, Exit-only copy, Watchlist cards, skip link / toasts / TypedConfirm / More a11y, offline banner, PWA UI delete, Playwright 390 landmark smoke, iOS More Postures + Connections handoff.  Did not take P3.  Local lint 0 / tsc clean / focused 11.  Dispatched verify green on `d52b354a` (run 32329145996).  Merged `origin/main` `ee1286e0`.  Rollout: `docs/rollouts/2026-08-20-web-ios-parity-fixes.md`.
- **[Socratic.Trade][CURSOR] Pinecone trial end in 7 days — IN PR #2940 2026-08-19 (branch `cursor/pinecone-trial-end-7d-c9a3`).** Owner: set the Standard-trial calendar to 7 days from 2026-08-19 (`2026-08-27T00:00:00.000Z`). App snap + Infisical `PINECONE_TRIAL_ENDS_AT`. Not changing Pinecone's own console trial. Not raising the daily WU fuse. Local gate green.
- **[Socratic.Trade][CURSOR] Deploy freshness alert + isolate CT OCR load — IN PR #2796 2026-08-20 (branch `cursor/deploy-freshness-ocr-isolate-d4cf`, closes #2545).** 20-minute cron pages when oldest undeployed main commit is >1h.  OCR CPU ceiling is **5.0 of 8 vCPUs** (cpu-shares 256) -- as high as is reasonably advisable; 3 cores stay free for Coolify/ST/UM.  CT compose `scan-cpu-worker` is still 2.0 (throttles below the 2.83 peak); durable fix is raise that to 5.0.  Did not touch prod.
- **[Socratic.Trade][CURSOR] Web / iOS parity P1+P2 fixes — IN PROGRESS 2026-08-20 (branch `cursor/web-ios-parity-fixes-e83a`).** Implementing the 2026-08-17 audit: `?proposal=` / `?symbol=` focus, iOS Activity alerts, Exit-only copy, Watchlist cards, skip link / toasts / TypedConfirm / More a11y, offline banner, PWA UI delete, Playwright 390 landmark smoke, iOS More Postures + Connections handoff.  Did not take P3.  Rollout: `docs/rollouts/2026-08-20-web-ios-parity-fixes.md`.
- **[Socratic.Trade][CURSOR] Cross-app coordination follow-ups (audit #2802 §7 ST slice) — IN PROGRESS 2026-08-20 (branch `cursor/cross-app-coordination-followups-1212`).** Vendor-era pin-check (ST/UM/CT triangle, fail-closed on missing CT).  Durable call-volume persist + replay.  `congress-read` label on peer-serving OHLC misses.  Infisical merge-order + congress/Quiver docs.  Peer CT/UM items are separate PRs.  Did not promote pin-check to required.  Did not mint a Massive key.  Rollout: `docs/rollouts/2026-08-20-cross-app-coordination-followups.md`.
- **[Socratic.Trade][CURSOR] Cross-app coordination follow-ups (audit #2802 §7 ST slice) — IN PR #2941 2026-08-20 (branch `cursor/cross-app-coordination-followups-1212`).** Vendor-era pin-check (ST/UM/CT triangle, fail-closed on missing CT).  Durable call-volume persist + replay.  `congress-read` label on peer-serving OHLC misses.  Infisical merge-order + congress/Quiver docs.  Peer PRs: CT #2064, UM #1245, FLEET #44, CTS #278, DD #93.  Did not promote pin-check to required.  Did not mint a Massive key.  Rollout: `docs/rollouts/2026-08-20-cross-app-coordination-followups.md`.
- **[Socratic.Trade][CURSOR] Cross-app coordination audit (ST/CT/UM/CTS/DD/FLEET) — COMPLETED/MERGED 2026-08-17 #2802.** Report only.  Pins matched CTS `v2.5.2`; ST pin-check was a no-op vs vendor-era CT.  Follow-ups: `cursor/cross-app-coordination-followups-1212`.  Audit: `docs/audits/2026-08-17-cross-app-coordination.md`.
...
- **[Socratic.Trade][CURSOR] Account write guards (preserve Autopilot + block draining reactivate) — IN PR 2026-08-19 (branch `cursor/account-write-guards-940d`).** Expert review tranche-1 `account-write-guards`.  Pin `strategyAuthority` on profile activate/copy/import; refuse draining activate; clear drain on reconnect upsert.  No new TypedConfirm.  Rollout: `docs/rollouts/2026-08-19-account-write-guards.md`.
- **[Socratic.Trade][CURSOR] Coach fail-closed tool inputs + abort in-flight turns — IN PROGRESS 2026-08-19 (branch `cursor/coach-tools-fail-closed-1c54`).** Review cluster `coach-tools-and-turns`.  `draft_order` rejects invalid side/order_type/limit; `kb_search` clamps k; chat transports pass `abortSignal`.  Rollout: `docs/rollouts/2026-08-19-coach-tools-fail-closed.md`.
- **[Socratic.Trade][CURSOR] Coach fail-closed tool inputs + abort in-flight turns — IN PR #2874 2026-08-19 (branch `cursor/coach-tools-fail-closed-1c54`).** Review cluster `coach-tools-and-turns`.  `draft_order` rejects invalid side/order_type/limit; `kb_search` clamps k; chat transports pass `abortSignal`.  Rollout: `docs/rollouts/2026-08-19-coach-tools-fail-closed.md`.
- **[Socratic.Trade][CURSOR] iOS session snapshot, edit alerts, nested stop-loss — IN PR 2026-08-19 (branch `cursor/ios-session-stop-loss-03c2`).** Part II `ios-state-outcome-truth` + `web-ios-contract-drift` minimum.  Sign-out clears disk snapshot; guardrail errors as alerts; `FullPolicy` nested `riskRules` stop decode + fixture test.  No TF.  Rollout: `docs/rollouts/2026-08-19-ios-session-stop-loss.md`.
- **[Socratic.Trade][CURSOR] CI docs-only fast path excludes build-imported docs/benchmarks — IN PR 2026-08-19 (branch `cursor/fix-ci-docs-benchmarks-skip-e4ac`).** Expert review qa-03: `docs/benchmarks/*.json` is bundled by `model-stats/route.ts`; edits must not skip verify.  Carve-out in `ci.yml` + `e2e.yml` only; no Swift gate.  Rollout: `docs/rollouts/2026-08-19-ci-benchmarks-docs-only-fix.md`.
- **[Socratic.Trade][CURSOR] Identity fail-closed in live bootstrap (`identity-fails-open`) — IN PR 2026-08-19 (branch `cursor/identity-fails-open-535a`).** Boot assert when `DB_BOOTSTRAP=live` and no `AUTH_SECRET`/armed CF Access; middleware dev fallback gated; `resolveRequestUser` refuses anonymous in live.  Rollout: `docs/rollouts/2026-08-19-identity-fails-open.md`.
- **[Socratic.Trade][CURSOR] Broker I/O deadlines + scoped order history — IN PROGRESS 2026-08-19 (branch `cursor/broker-io-deadlines-85a9`).** Part II `broker-io-deadlines`: Alpaca quotes/place/cancel + Tradier fetch deadlines; scheduler stop-monitor/stale-limit `withDeadline(15s)`; default `getEquityOrders` open + 24h terminal (`fullHistory` opt-in).  Rollout: `docs/rollouts/2026-08-19-broker-io-deadlines.md`.
- **[Socratic.Trade][CURSOR] Broker I/O deadlines + scoped order history — IN PR #2886 2026-08-19 (branch `cursor/broker-io-deadlines-85a9`, SHA `54ed4a3c`).** Part II `broker-io-deadlines`: Alpaca quotes/place/cancel + Tradier fetch deadlines; scheduler stop-monitor/stale-limit `withDeadline(15s)`; default `getEquityOrders` open + 24h terminal (`fullHistory` opt-in).  Rollout: `docs/rollouts/2026-08-19-broker-io-deadlines.md`.
- **[Socratic.Trade][CURSOR] Phone touch viewport cluster — IN PR 2026-08-19 (branch `cursor/phone-touch-viewport-b809`).** Review cluster `phone-touch-viewport`: icon-only run-state below sm, 44px coarse-pointer floor on all interactive classes, 16px input floor, `useOverlay` scroll-lock + history + visualViewport sizing.  Rollout: `docs/rollouts/2026-08-19-phone-touch-viewport.md`.
- **[Socratic.Trade][CURSOR] Placement outcome truth (approve path) — IN PR 2026-08-19 (branch `cursor/placement-outcome-truth-6d4a`).** `placement-outcome` resolver; mobile command `succeeded` only when placed; iOS decodes `result.status`.  HTTP 429/408 → `not_placed` on approval path.  Rollout: `docs/rollouts/2026-08-19-placement-outcome-truth.md`.
- **[Socratic.Trade][CURSOR] FTS indexed mirror idempotency + strategy-run yield (`event-loop-pins`) — IN PR 2026-08-19 (branch `cursor/event-loop-pins-fts-indexed-mirror-5b2a`).** `document_chunks_fts_index` v85; rowid DELETE; `mirrorFtsChunksBounded` for `persistLocalComplete` + filing-body ingest.  Rollout: `docs/rollouts/2026-08-19-fts-indexed-mirror-idempotency.md`.
- **[Socratic.Trade][CURSOR] Green Bull strict schema (`green-request-schema`) — IN PR 2026-08-19 (branch `cursor/green-bull-schema-769b`).** `exitPlan` missing from `BULL_PROPOSAL_REQUIRED_KEYS` caused OpenAI-family Green 400s.  Added key + invariant test + `json_object` post-parse gate.  Rollout: `docs/rollouts/2026-08-19-green-bull-schema.md`.
- **[Socratic.Trade][CURSOR] console-ships-too-much — server DB boundary + snapshot projection — IN PR 2026-08-19 (branch `cursor/console-ships-too-much-6790`).** `server-only` on `src/lib/db*`; `venue-contract-pure.ts`; dashboard drops raw `audit[]`, trims quotes/orders; cache bust on ack/cancel/replace.  Rollout: `docs/rollouts/2026-08-19-console-ships-too-much.md`.
- **[Socratic.Trade][CURSOR] console-ships-too-much — server DB boundary + snapshot projection — IN PR #2884 2026-08-19 (branch `cursor/console-ships-too-much-6790`).** `server-only` on `src/lib/db*`; `venue-contract-pure.ts`; dashboard drops raw `audit[]`, trims quotes/orders; cache bust on ack/cancel/replace.  Rollout: `docs/rollouts/2026-08-19-console-ships-too-much.md`.
- **[Socratic.Trade][CURSOR] Day P&L cash-flow cluster (perf-02/03/08) — IN PR 2026-08-19 (branch `cursor/fix-day-pnl-cash-flows-56a3`).** Broker Alpaca activities as flow source; inference fallback with real fills + short sign fix; Central trading-day helper.  Rollout: `docs/rollouts/2026-08-19-day-pnl-cash-flow-cluster.md`.
- **[Socratic.Trade][CURSOR] Day P&L cash-flow cluster (perf-02/03/08) — IN PR #2878 2026-08-19 (branch `cursor/fix-day-pnl-cash-flows-56a3`).** Broker Alpaca activities as flow source; inference fallback with real fills + short sign fix; Central trading-day helper.  Rollout: `docs/rollouts/2026-08-19-day-pnl-cash-flow-cluster.md`.
...
- **[Socratic.Trade][CURSOR] Gather must not inventory Pinecone or latch on congress 502 / Massive 429 — IN PR #2854 2026-08-19 (branch `cursor/gather-no-pinecone-inventory-befc`, corrected from IN PROGRESS).** Same `9d71dda4` window + this morning's 8m gather timeout.  See the rebase row immediately below.
- **[Socratic.Trade][CURSOR] Gather must not inventory Pinecone or latch on congress 502 / Massive 429 — IN PR #2854 2026-08-20 (branch `cursor/gather-no-pinecone-inventory-befc`, rebased onto `eefc4f82`).** Real conflict vs #2813 was `test/roic-transcripts.test.ts` only.  Kept gather skip test + main's skip-covered ROIC test.  Did not absorb #2813 product.  Do not flip write-class.  Do not prune.  Do not merge.  Rollout: `docs/rollouts/2026-08-19-gather-no-pinecone-inventory.md`.
- **[Socratic.Trade][CURSOR] Robinhood max-10 quote chunk for 250-name gather — COMPLETED/MERGED 2026-08-19 #2852 `c7b775c5`.** Roth `9d71dda4` llm=0 then `stalled_no_progress` 01:29:44Z.  Live error: `too many symbols (max 10, got 250)` at 00:59:15Z.  Chunk `get_equity_quotes` / tradability / fundamentals to 10.  Follow-up gather crumbs are `cursor/gather-no-pinecone-inventory-befc`.
...
- **[Socratic.Trade][CURSOR] IRA Ignore/Block already existed — wire them properly — IN PROGRESS 2026-08-18 (branch `cursor/ira-wash-sale-factor-a1df`).** Owner: do not add a third mode.  Ignore still steered Green; min-loss was hidden/unused on the IRA buyer path.  Prompt `agentic-strategy@2.13.0`; IRA blank floor = $50.
- **[Socratic.Trade][CURSOR] IRA Ignore / Auto / Block + optional min-loss — IN PROGRESS 2026-08-18 (PR #2842, branch `cursor/ira-wash-sale-factor-a1df`).** Owner: Auto is a choosable IRA option; min-loss is optional (blank = every loss).  Prompt `agentic-strategy@2.14.0`.
- **[Socratic.Trade][CURSOR] Notification history + web/iOS inbox parity — IN PROGRESS 2026-08-18 (branch `cursor/notification-history-parity-4bbc`).** Persist and reopen later alerts (time, title/body, read) on website header inbox + Activity Alert Center and iOS Activity.  Reuse `notification_events` + `/api/notifications/ack`.  Did not touch trading/OpenRouter/RAG/health.  Did not rebase onto #2831/#2812/#2830/#2800.
- **[Socratic.Trade][CURSOR] Notification history + web/iOS inbox parity — IN PR #2841 2026-08-18 (branch `cursor/notification-history-parity-4bbc`).** Persist and reopen later alerts (time, title/body, read) on website header inbox + Activity Alert Center and iOS Activity.  Reuse `notification_events` + `/api/notifications/ack`.  Did not touch trading/OpenRouter/RAG/health.  Did not rebase onto #2831/#2812/#2830/#2800.
- **[Socratic.Trade][CURSOR] Notification history + web/iOS inbox parity — IN PR #2841 2026-08-18 (branch `cursor/notification-history-parity-4bbc`).** Rebased onto `origin/main` `13b60747` (#2830 + #2832).  Persist and reopen later alerts (time, title/body, read) on website header inbox + Activity Alert Center and iOS Activity.  Reuse `notification_events` + `/api/notifications/ack`.  Kept #2830 scan-copy tests.  Did not touch #2831/#2812/#2840, trading, broker, OpenRouter, RAG, or the health gate.
...
- **[Socratic.Trade][GROK] Live prod triage (Autopilot Green + Scan + Roth getAccounts) — IN PROGRESS 2026-08-18 (branch `grok/prod-triage-2026-08-18`, worktree `~/apps/trading-grok-prod-triage`).** Owner: grokbot too slow. Live: Paper 4x Green fail (terra/luna 400 + fake 3-endpoint exhaust); Roth completed 19:43Z 0 proposals; Scan empty since 08-13; L2/L3 wedge 13h (owner-ops); Roth dashboard getAccounts 6s timeout (duplicate Alpaca getAccount). Keepout: not rewriting #2831/#2830 — rematched both onto main so verify can land them. Fixing unclaimed Alpaca getAccount duplicate in this lane.
- **[Socratic.Trade][GROK] Live OpenRouter + data-cascade — IN PROGRESS 2026-08-18 (same worktree `~/apps/trading-grok-prod-triage`).** Owner: cascade and OpenRouter failing.  Chat = Green 400/404 + fake exhaust (#2831).  Embed = bge-m3 32-text 8193 tok 400 + integrity reject 32/375 (#2840).  Cascade user-visible = empty Scan since 08-13 (#2830) plus any remaining provider-all-fail.  Keepout: not rewriting #2840/#2831/#2830 product code — landing them.
- **[Socratic.Trade][GROK] Week-error expert triage + live iOS Scan/Home — IN PROGRESS 2026-08-18 (branch `grok/prod-triage-2026-08-18`, worktree `~/apps/trading-grok-prod-triage`, PR #2834, issue #2833).** Owner: team of experts on all errors last 7d + iOS shots. Sentry 7d top: Pinecone connection failed 360 (1T, terminated), OpenRouter embed 21 (1X, live on 12e8dcd), rerank 14 (22), CT SSE 13 (1V), NEW RAG embed integrity rejection 9 (27, 406 attempted / 29 rejected, key_source=none after #2812). Health ok, tradingLiveness degraded. iOS UX + Alpaca getAccount cache on this PR. Keepout: not rewriting #2830/#2831. L2/L3 owner-ops.
...
- **[Socratic.Trade][CURSOR] Mobile tab bar: shrink Safari chrome gap + white underlay — IN PROGRESS 2026-08-18 (branch `cursor/mobile-tabbar-chrome-gap-23d5`).** Owner screenshot: grey-blue band between console tabs and the floating URL chrome. Browser pad is 22% of `env(safe-area-inset-bottom)` (~78% gone); solid `--con-surface` bar + `::after` so the remainder and the URL halo match the tab strip. No negative-bottom shift (2026-08-05 hid labels). Standalone/PWA unchanged. Rollout: `docs/rollouts/2026-08-18-mobile-tabbar-chrome-gap.md`.
- **[Socratic.Trade][CURSOR] Mobile tab bar: shrink Safari chrome gap + white underlay — IN PR #2828 2026-08-18 (branch `cursor/mobile-tabbar-chrome-gap-23d5`).** Owner screenshot: grey-blue band between console tabs and the floating URL chrome. Browser pad is 22% of `env(safe-area-inset-bottom)` (~78% gone); solid `--con-surface` bar + `::after` so the remainder and the URL halo match the tab strip. No negative-bottom shift (2026-08-05 hid labels). Standalone/PWA unchanged. Rollout: `docs/rollouts/2026-08-18-mobile-tabbar-chrome-gap.md`.
...
- **[Socratic.Trade][CURSOR] Delayed Yahoo fallback stamp + keep trading — IN PROGRESS 2026-08-18 (branch `cursor/delayed-yahoo-fallback-stamp-c120`).** Owner: stamp **delayed fallback** on approval cards; openings still go through; do not fail-closed; do not block Green/Red.  Did not steal #2792/#2798/#2800/#2794.  No Stripe.  Rollout: `docs/rollouts/2026-08-18-delayed-yahoo-fallback-stamp.md`.
- **[Socratic.Trade][CURSOR] Delayed Yahoo fallback stamp + keep trading — IN PR #2818 2026-08-18 (branch `cursor/delayed-yahoo-fallback-stamp-c120`).** Owner: stamp **delayed fallback** on approval cards; openings still go through; do not fail-closed; do not block Green/Red.  Did not steal #2792/#2798/#2800/#2794.  No Stripe.  Rollout: `docs/rollouts/2026-08-18-delayed-yahoo-fallback-stamp.md`.
- **[Socratic.Trade][CURSOR] Delayed Yahoo fallback stamp + keep trading — IN PR #2818 2026-08-18 (branch `cursor/delayed-yahoo-fallback-stamp-c120`).** Owner: stamp user-facing **Delayed Quote** on approval cards (no coordinator notes / Infisical / remote asides in iOS UI); openings still go through; do not fail-closed; do not block Green/Red.  Did not steal #2792/#2798/#2800/#2794.  No Stripe.  Rollout: `docs/rollouts/2026-08-18-delayed-yahoo-fallback-stamp.md`.
- **[Socratic.Trade][CURSOR] Litestream restore drill (scratch only) — IN PR #2822 2026-08-18 (branch `cursor/litestream-restore-drill-2cd9`).** Report only.  Two B2 scratches VERIFIED (4.9G, integrity ok).  Decrypt `fred` last-4 `6dd4` VERIFIED.  R2 weekly retain=1 VERIFIED (exactly one `cold-snapshots/` object).  `R2_ARCHIVE_KEEP_GENERATIONS=2` unused on ST.  No remaining BLOCKED / NOT VERIFIED.  No product code.  No Coolify.  No rolling/zero-downtime.  Rollout: `docs/rollouts/2026-08-17-litestream-restore-drill.md`.
- **[Socratic.Trade][CURSOR] Strip leaked coordinator notes from iOS UI — IN PROGRESS 2026-08-18 (branch `cursor/ios-no-owner-note-ui-5139`).** Owner: no coordinator/owner comments in iOS UI.  Removed Home Desk subtitle `full surfaces, not just the remote` plus same-class Infisical/remote/`policy.patch` leaks.  Did not steal #2792/#2798/#2800/#2794.
- **[Socratic.Trade][CURSOR] Strip leaked coordinator notes from iOS UI — IN PR #2814 2026-08-18 (branch `cursor/ios-no-owner-note-ui-5139`).** Owner: no coordinator/owner comments in iOS UI.  Removed Home Desk subtitle `full surfaces, not just the remote` plus same-class Infisical/remote/`policy.patch` leaks.  Did not steal #2792/#2798/#2800/#2794.
- **[Socratic.Trade][CURSOR] Finish ROIC Individual local archive — IN PROGRESS 2026-08-18 (branch `cursor/roic-individual-archive-9ad4`).** Skip cached ROIC list/fetch.  Persist `earningscalls_transcripts` + `data/roic-artifacts`.  Ops `roicArchive` + remaining-gap list.  Renew-vs-expire still open.  No Stripe.  Not #2800/#2798/#2794/#2792.
- **[Socratic.Trade][CURSOR] Finish ROIC Individual local archive — IN PR #2813 2026-08-18 (branch `cursor/roic-individual-archive-9ad4`).** Skip cached ROIC list/fetch.  Persist `earningscalls_transcripts` + `data/roic-artifacts`.  Ops `roicArchive` + remaining-gap list.  Renew-vs-expire still open.  No Stripe.  Not #2800/#2798/#2794/#2792.
- **[Socratic.Trade][CURSOR] Finish ROIC Individual local archive — IN PR #2813 2026-08-20 (branch `cursor/roic-individual-archive-9ad4`).** Rebased onto `origin/main` `d3e2c9ee` (#2892).  Skip cached ROIC list/fetch.  Persist `earningscalls_transcripts` + `data/roic-artifacts`.  Ops `roicArchive` + remaining-gap list.  Kept #2848 strategy-run pause.  Renew-vs-expire still open.  No Stripe.  Did not absorb other clusters.
- **[Socratic.Trade][CURSOR] Health JSON monitors + OPS token + R2 retain=1 — IN PROGRESS 2026-08-18 (branch `cursor/health-json-monitors-ac72`).** Always emit `schedulerStale` / `tradingLiveness.degraded` / `litestreamTiersDegraded`; never 503 those flags.  OPS_DIAGNOSTIC_TOKEN required (no ADMIN fallback).  R2 weekly retain capped at 1.  Runbook: `docs/runbooks/uptime-health-json-monitors.md`.
- **[Socratic.Trade][CURSOR] Health JSON monitors + OPS token + R2 retain=1 — IN PROGRESS 2026-08-18 (branch `cursor/health-json-monitors-ac72`, PR #2816).** Always emit `schedulerStale` / `tradingLiveness.degraded` / `litestreamTiersDegraded`; never 503 those flags.  OPS_DIAGNOSTIC_TOKEN required (no ADMIN fallback).  Live leftover `R2_ARCHIVE_KEEP_GENERATIONS=2` is read then capped at 1 in code (no Coolify edit, no live R2 deletes).  Health `r2Weekly.key` is `cold-snapshots/app-2026-08-16.db`.  Runbook: `docs/runbooks/uptime-health-json-monitors.md`.
- **[Socratic.Trade][CURSOR] Health JSON monitors + OPS token + R2 retain=1 — IN PROGRESS 2026-08-18 (branch `cursor/health-json-monitors-ac72`, PR #2816).** Always emit `schedulerStale` / `tradingLiveness.degraded` / `litestreamTiersDegraded`; never 503 those flags.  OPS_DIAGNOSTIC_TOKEN required (no ADMIN fallback).  Weekly R2 retain is already verified at 1 (`R2_COLD_SNAPSHOT_DEFAULT_RETAIN=1`, retain env unset, one `cold-snapshots/app-2026-08-16.db`).  Unused leftover `R2_ARCHIVE_KEEP_GENERATIONS=2` does not drive weekly retain (`weekly/` empty).  No live R2 deletes.  Runbook: `docs/runbooks/uptime-health-json-monitors.md`.
- **[Socratic.Trade][CURSOR] Health JSON monitors + OPS token + R2 retain=1 — IN PROGRESS 2026-08-18 (branch `cursor/health-json-monitors-ac72`, PR #2816).** Rebased onto `main` 2026-08-18.  Always emit `schedulerStale` / `tradingLiveness.degraded` / `litestreamTiersDegraded`; never 503 those flags.  OPS_DIAGNOSTIC_TOKEN required (no ADMIN fallback).  Weekly R2 retain is already verified at 1; unused leftover `R2_ARCHIVE_KEEP_GENERATIONS=2` does not drive weekly retain.  No live R2 deletes.  Runbook: `docs/runbooks/uptime-health-json-monitors.md`.
- **[Socratic.Trade][CURSOR] Coolify auto-deploy only in non-RTH (HOTFIX=1 escape) — IN PROGRESS 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  Confirmed no prior latch.  Build-time Dockerfile check uses `isMarketOpen` (early-close aware).  `HOTFIX=1` env or commit token, or `RTH_DEPLOY_OVERRIDE=1`.  Evening drain workflow.  Did not touch #2792/#2798/#2800/#2794.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Coolify auto-deploy only in non-RTH (HOTFIX=1 escape) — IN PR #2817 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  Confirmed no prior latch.  Build-time Dockerfile check uses `isMarketOpen` (early-close aware).  `HOTFIX=1` env or commit token, or `RTH_DEPLOY_OVERRIDE=1`.  Evening drain workflow.  `verify-hosted` green.  Did not touch #2792/#2798/#2800/#2794.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Coolify auto-deploy only in non-RTH + skip docs-only rebuilds — IN PR #2817 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  RTH latch plus image-noop skip after tonight's #2811 503 (~7:15–7:49pm CT, sha `23412aff`, Cloudflare no available server, `last_restart_at` null).  Latch runs before `npm ci`.  Keep consistent-name; no rolling.  Did not bounce prod, FORCE_RESTORE, or PATCH Coolify.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Coolify RTH latch + skip docs-only + /api/live Traefik probe — IN PR #2817 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  ASC: #2810 ~7m build, #2811 ~6m build, processStartedAt 00:49:27Z is #2811 completing.  7:22-7:43pm CT #2810 was up (litestream log, no ERROR) while public 503 continued — running:unhealthy / Traefik.  Keep stop-old-first; no rolling.  HEALTHCHECK is /api/live.  Did not bounce prod, FORCE_RESTORE, or PATCH Coolify.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Coolify RTH latch; watch_paths already live — IN PR #2817 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  ASC applied watch_paths on socratic-app (<ST_COOLIFY_APP_UUID>); app healthy; no bounce; do not re-apply.  Auto-deploy on; stop-old-first kept; health_check_start_period 60.  Tonight 503: #2810 ~7m, #2811 ~6m, 7:22-7:43pm CT unhealthy-while-up.  PR still ships RTH latch + /api/live.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Coolify RTH latch; watch_paths already live — IN PR #2817 2026-08-18 (branch `cursor/rth-deploy-latch-c039`).**  Rebased onto main after #2824.  ASC applied watch_paths on socratic-app (<ST_COOLIFY_APP_UUID>); do not re-apply.  Auto-deploy on; stop-old-first kept; health_check_start_period 60.  PR ships RTH latch unless HOTFIX=1 plus /api/live HEALTHCHECK.  Tonight 503: #2810 ~7m, #2811 ~6m, 7:22-7:43pm CT unhealthy-while-up.  Did not steal #2792/#2798/#2800/#2794.  Rollout: `docs/rollouts/2026-08-18-rth-deploy-latch.md`.
- **[Socratic.Trade][CURSOR] Health JSON monitors + OPS token + R2 retain=1 — COMPLETED/MERGED 2026-08-20 #2816 `85cbda190` (DEEPSEEK state check).**  On main; runbook `docs/runbooks/uptime-health-json-monitors.md` live.  Corrects the IN PROGRESS rows above.
- **[Socratic.Trade][CURSOR] Coolify RTH latch + skip docs-only + /api/live probe — COMPLETED/MERGED 2026-08-20 #2817 `2f597eaa8` (DEEPSEEK state check).**  On main; latch live in the Dockerfile before npm ci; evening drain job runs.  Corrects the IN PR rows above.
- **[Socratic.Trade][CURSOR] Litestream restore drill (scratch only) — COMPLETED/MERGED 2026-08-18 #2823 `55a8613d` (follow-up #2824 `cursor/restore-receipts-followup-2cd9`).** #2822 had merged stale BLOCKED / NOT VERIFIED rows; #2823 flipped decrypt (`fred` last-4 `6dd4`) and R2 weekly retain=1 to VERIFIED.  Coolify `watch_paths` now omits `docs/**`, `STATUS.md`, `PLAN.md`.  No product code.  Rollout: `docs/rollouts/2026-08-17-litestream-restore-drill.md`.
...
- **[Socratic.Trade][CURSOR] Web / mobile-web / iOS parity audit — IN PROGRESS 2026-08-17 (branch `cursor/web-ios-parity-audit-e83a`).** Report-only.  Matrix + P1 deep-link/Activity gaps + safe PWA deletion plan in `docs/audits/2026-08-17-web-ios-parity.md`.  No product code.
- **[Socratic.Trade][CURSOR] Trading-outcomes audit (macro, Green/Red/Bull, risk, backtest, leakage, paper/live, learning) — IN PROGRESS 2026-08-17 (branch `cursor/trading-outcomes-audit-c37a`).** Read-only.  Report `docs/audits/2026-08-17-trading-outcomes.md`.  No money-path changes.  Related: #2280 (PIT masking still planned), #2563/#2793 (curl-only backtest-ic UI).
- **[Socratic.Trade][CURSOR] Architecture & backend audit (read-only) — IN PROGRESS 2026-08-17 (branch `cursor/architecture-backend-audit-6186`).** Docs-only report at `docs/audits/2026-08-17-architecture-backend.md`.  No product fixes.  Cross-checked open issues/PRs so F1/F4/F5/ROIC stacking are not re-filed.
- **[Socratic.Trade][CURSOR] Architecture & backend audit (read-only) — IN PR #2807 2026-08-17 (branch `cursor/architecture-backend-audit-6186`).** Docs-only report at `docs/audits/2026-08-17-architecture-backend.md`.  No product fixes.  Cross-checked open issues/PRs so F1/F4/F5/ROIC stacking are not re-filed.
- **[Socratic.Trade][CURSOR] RAG / learning / recall audit — IN PROGRESS 2026-08-17 (branch `cursor/rag-learning-recall-audit-f94a`).** Read-only.  Report `docs/audits/2026-08-17-rag-learning-recall.md`: ingest/SEC/ROIC/news, chunk/embed, retrieval, grounding, PIT, lineage, learning ledger, memory, evals, recovery.  Gold-set + quantitative gates included.  No product-code change.
- **[Socratic.Trade][CURSOR] RAG / learning / recall audit — IN PR #2803 2026-08-17 (branch `cursor/rag-learning-recall-audit-f94a`).** Read-only.  Report `docs/audits/2026-08-17-rag-learning-recall.md`: ingest/SEC/ROIC/news, chunk/embed, retrieval, grounding, PIT, lineage, learning ledger, memory, evals, recovery.  Gold-set + quantitative gates included.  No product-code change.
- **[Socratic.Trade][CURSOR] RAG P0 follow-ups — IN PR #2803 2026-08-20 (branch `cursor/rag-learning-recall-audit-f94a`).** Parsed-text `buildSecDocument` for worker + ingestFiling.  Ticker→CIK, no sentinel.  Chat always passes `asOf`.  Eval harness + contract test on `retrieveContextDetailed`.
- **[Socratic.Trade][CURSOR] RAG / learning / recall audit — IN PR #2803 2026-08-17 (branch `cursor/rag-learning-recall-audit-f94a`).** Report `docs/audits/2026-08-17-rag-learning-recall.md`.  P0 follow-ups landed 2026-08-20.
- **[Socratic.Trade][CURSOR] Security / reliability audit (report-only) — IN PROGRESS 2026-08-17 (branch `cursor/security-reliability-audit-d8f6`).** Auth, isolation, secrets, Litestream/DR, alerting, spend, SLOs.  Live probe sanitized.  No runtime change.  Report: `docs/audits/2026-08-17-security-reliability.md`.
- **[Socratic.Trade][CURSOR] Security / reliability audit (report-only) — IN PR #2806 2026-08-17 (branch `cursor/security-reliability-audit-d8f6`).** Auth, isolation, secrets, Litestream/DR, alerting, spend, SLOs.  Live probe sanitized.  No runtime change.  Report: `docs/audits/2026-08-17-security-reliability.md`.
- **[Socratic.Trade][CURSOR] Brokers + data-cascade reliability audit — IN PR 2026-08-17 (branch `cursor/brokers-data-cascade-audit-bed0`).** Read-only.  Report `docs/audits/2026-08-17-brokers-data-cascade.md`.  Accounts for #2792 FilingAPI soft-skip vs #2787 retirement.  No broker mutations.  Follow-ups not in this PR.
- **[Socratic.Trade][CURSOR] Brokers + data-cascade reliability audit — IN PR #2805 2026-08-17 (branch `cursor/brokers-data-cascade-audit-bed0`).** Read-only.  Report `docs/audits/2026-08-17-brokers-data-cascade.md`.  Accounts for #2792 FilingAPI soft-skip vs #2787 retirement.  No broker mutations.  Follow-ups not in this PR.
- **[Socratic.Trade][CURSOR] Cross-app coordination audit (ST/CT/UM/CTS/DD/FLEET) — COMPLETED/MERGED 2026-08-17 #2802 (was IN PR; follow-ups on `cursor/cross-app-coordination-followups-1212`).** Report only.  Pins matched CTS `v2.5.2`; ST pin-check was a no-op vs vendor-era CT.  ST trades if CT/UM die; Coolify trio shares Hetzner; CT Senate needs the Mac.  Audit: `docs/audits/2026-08-17-cross-app-coordination.md`.
- **[Socratic.Trade][CURSOR] Remove leftover installable PWA — IN PROGRESS 2026-08-17 (branch `cursor/retire-pwa-72cb`).** Delete unused `/mobile` client + standalone manifest. 410 webmanifest. Unregister leftover service workers. Keep `/api/mobile/*` (iOS) and responsive `/console`.
- **[Socratic.Trade][CURSOR] Remove leftover installable PWA — IN PR #2801 2026-08-17 (branch `cursor/retire-pwa-72cb`).** Delete unused `/mobile` client + standalone manifest. 410 webmanifest. Unregister leftover service workers. Keep `/api/mobile/*` (iOS) and responsive `/console`.
...
- **[Socratic.Trade][CURSOR] FilingAPI optional key, degrade gracefully — IN PROGRESS 2026-08-17 (branch `cursor/filingapi-soft-skip-de61`, #2792, retarget #2778).** Owner reversed #2787/#2788 retirement. Keep FilingApiEnrichmentProvider + health lane + matrix. Missing/401 soft-skips. Do not buy Plus / do not charge Stripe. (Corrected in place: this row was the #2787 retire ask.)
- **[Socratic.Trade][CURSOR] CT+UM lane backoff — ST tolerates slow/flappy peers — IN PROGRESS 2026-08-17 (branch `cursor/ct-um-lane-backoff-7cf2`, closes #2550).** Widen UM flush/knob/budget backoff when p50 > 2s. SSE flaps are soft, 5-minute reconnect cap, no reset on short-lived 200s. Congress HTTP fails fast when already slow. No CT/UM server changes.
- **[Socratic.Trade][CURSOR] CT+UM lane backoff — ST tolerates slow/flappy peers — IN PR #2797 2026-08-17 (branch `cursor/ct-um-lane-backoff-7cf2`, closes #2550).** Widen UM flush/knob/budget backoff when p50 > 2s. SSE flaps are soft, 5-minute reconnect cap, no reset on short-lived 200s. Congress HTTP fails fast when already slow. Vitest setupFiles resets peer-lane samples. No CT/UM server changes.
...
- **[Socratic.Trade][GROK] Website favicon: cropped offset candlestick ST, transparent — IN PR #2785 2026-08-17 (branch `cursor/favicon-crop-st-e6ee`, issue #2731).**  Website uses the iOS App Icon's offset candlestick ST (S higher than T), cropped tight, transparent.  First pass wrongly rebuilt the #1626 2D ST; owner corrected that the App Icon is that offset ST, not a dollar sign.  App Icon file untouched.
...
- **[Socratic.Trade][GROK] Autopilot accounts + rotation timeout RCA — DEPLOYED 2026-08-13 (PR #2706 `d876f2c6`, live image `<ST_COOLIFY_APP_UUID>:d876f2c6`).** Roth IRA / Alpaca Paper / Sandbox (`tradier/paper`) are Autopilot (`systemState=active` + `strategyAuthority=decide`).  `autoResumeOnBoot=true`.  Script present; `--apply` not needed.  Last runs failed 7h/8h/79h (rotation timeout + mid-run sweep).  0 runs since this deploy; market closed, extended hours off.
...
- **[Socratic.Trade][AG] Fix admin.socratictrade.com DNS 525 Error & Host Routing — COMPLETED 2026-07-31.** Fixed Cloudflare Error 525 (SSL Handshake Failed) on `admin.socratictrade.com` by adding `admin.socratictrade.com` and `*.socratictrade.com` to `/etc/usage-monitor/Caddyfile` on Oracle Cloud (`<ORACLE_IP_RETIRED>`) and reloading Caddy. Added host-level routing in `middleware.ts` so `admin.socratictrade.com/` redirects directly to `/admin` and shorthand paths redirect to `/admin/<subpath>`.
...
- **2026-08-25 — CURSOR — PLANNED — Console honesty: Guardrails Discard, approve typed-confirm, Home Coach chips.**  Branch `cursor/console-discard-approve-chips-1ffb`.  Three live console bugs from the 2026-08-20 desktop review.  Do not collide with #3090 (rag) or #3077 (dependabot).  No Designer visual redesign.  No Datadog/RUM/Pinecone/RAG.
- **2026-08-21 — KIMI — PLANNED — [P0] Human/CODEOWNERS review gate for trading-execution paths.**  Board item bdc2b662.  Ruleset has `required_pull_request_reviews: null` (verified via API); merge-shepherd.sh: 'the only merge gate is the required verify check'; auto-merge + codex-autofix (the agent itself runs `gh pr merge --auto` and resolves review threads) + Coolify 'merge == live' → agent code can reach LIVE broker trading with zero human review.  Gate at minimum `src/lib/broker.ts`, `tradier.ts`, `robinhood.ts`, `execution*`, `scheduler.ts`, `scripts/execute-tradier-trades.ts`.
- **2026-08-21 — KIMI — PLANNED — [P1] codex-autofix hardening + remove committed trade re-placement script.**  Board items eed228fe, a4f09b16.  codex-autofix.yml routes PR content to api.deepseek.com/anthropic and runs an autonomous fixer with contents:write + Bash + unused `id-token: write` (drop it).  `scripts/execute-tradier-trades.ts` re-places proposals with status blocked/rejected on hardcoded account VA93389646 — bypasses the app's own approval controls and publicly discloses a real account number; remove from repo, purge from history.
- **2026-08-21 — KIMI — PLANNED — [P2] Fail-closed broker-token encryption, staging+rollback, supply-chain pins.**  Board items febaf717, 790f3809.  mcp-oauth.ts:289-305 stores plaintext when ENCRYPTION_KEY unset, then Litestream replicates to B2/R2 — refuse live boot without it.  No staging/canary/automated rollback (2026-08-06: prod silently stale ~14h from neighbor OCR wedging Coolify builds on the shared cx43).  CI uses `npm install`; cleanup-caches curls unpinned gh CLI; dependabot lacks github-actions; shared pkg pinned by mutable tag v2.5.2 and pin-check isn't required.

- **2026-08-19 — CLAUDE — PLANNED — [Review] Account-scoped write paths have no guards: one click can arm Autopilot or reactivate a draining account (P1, tranche Now, effort M).**  importAccountSettings (src/lib/db-profiles.ts:895) strips only identity and user-level fields, so strategyAuthority copies through and a single "Import settings" click can flip a live account from Ask-first to Autopil...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `account-write-guards`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Broker I/O is unbounded in time and scope, and the protective scheduler lanes latch on it (P1, tranche Now, effort M).**  The Alpaca SDK is constructed with no timeout (src/lib/alpaca.ts:247; axios default 0), so getLatestQuotes/createOrder/cancelOrder can hang indefinitely — and the stop-monitor and stale-exit lanes clear their in-fligh...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `broker-io-deadlines`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Day P&L reads $0.00 and shorts mint phantom deposits: flows are inferred, not read, and the day is not the trading day (P1, tranche Now, effort L).**  app/console/lib/derive.ts:443 calls inferExternalCashFlows([baseline, fakeCurrent], []) with a hardcoded empty fill list, so on any day the strategy trades directionally tradeCash is 0 (src/lib/cash-flows.ts:157-164),...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `cash-flow-and-trading-day`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Coach coerces invalid tool inputs instead of rejecting them, and cannot be cancelled mid-call (P1, tranche Now, effort M).**  src/lib/chat/tools.ts:92 maps any side that is not exactly "sell" — "Sell", "short", a typo — to "buy", the opposite economic direction, and :94/96 silently drops a limit price the same way; downstream promote-draft o...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `coach-tools-and-turns`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] The console ships server DB internals to the browser and the whole universe on every 15s poll (P1, tranche Now, effort M).**  app/console/settings/brokers.tsx:12 value-imports @/lib/venue-contract, which pulls source-settings → db-api-keys → db into the client bundle: SQL schema, migrations and key-store logic reach the browser, and this exa...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `console-ships-too-much`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] RAG mirroring pins the single serving+trading event loop for minutes at a time (P1, tranche Now, effort L).**  src/lib/db-learning.ts:1613-1616 makes idempotency a DELETE with equality on non-rowid FTS5 columns, which FTS5 serves as a full-corpus scan — measured 4.3ms/row at 10k rows rising to 26.3ms/row at 60k, and ~300ms/chu...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `event-loop-pins`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Bull strict JSON schema is invalid, so every OpenAI-family Green seat 400s before generation (P1, tranche Now, effort S).**  src/lib/strategy.ts:5870 declares exitPlan inside the Bull item's properties while BULL_PROPOSAL_REQUIRED_KEYS (strategy.ts:6653, used at :5851 for the required array) omits it, under additionalProperties:false — Open...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `green-request-schema`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Home's latest-run rows use synthetic ids and a private status map: green chips on failures, an Approve button that 404s, no keyboard access (P1, tranche Now, effort M).**  app/console/page.tsx:79 composes the row id as ${runId}-${symbol}-${status} while real proposal ids are crypto.randomUUID (src/lib/strategy.ts:3361), so handleApprove always 404s at app/api/proposals/[id]/approve/rout...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `home-proposal-rows`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Identity resolution falls back to the owner instead of failing closed (P1, tranche Now, effort M).**  middleware.ts:385-388 sets trustedEmail = PRIMARY_EMAIL whenever isAuthConfigured() is false, and nothing asserts AUTH_SECRET in production — instrumentation.ts:47 asserts only ENCRYPTION_KEY — so a selective env regr...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `identity-fails-open`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] iOS shows the previous account's data after sign-out and never surfaces the outcome of a money action (P1, tranche Now, effort M).**  clearLocalSession (ios/SocraticTrade/MobileStore.swift:628-650) nils in-memory state but never removes the UserDefaults blob at :136-144, so a cold launch after sign-out or account deletion re-reads it, sets isAuthent...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `ios-state-outcome-truth`, Bearer token in `~/.secrets/mac-collab.env`).
...
- **2026-08-19 — CLAUDE — PLANNED — [Review] Automation cancel-replaces orders it does not own (bracket take-profit legs, owner GTC sells, manually cancelled stops) (P1, tranche Now, effort M).**  No enqueue or pump path checks who placed the order.  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `order-provenance-guard`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Multi-account screens show the active account's data labeled as every account's (P1, tranche Now, effort M).**  app/console/settings/brokers.tsx:189 computes stateInfo only when isCurrentLoaded, so every other connected account chips a static "Inactive" even while it is genuinely running Autopilot (the scheduler runs each accou...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `per-account-visibility`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Approve reports success when nothing was placed, and misfiles retryable broker errors as terminal (P1, tranche Now, effort L).**  executeProposal returns a status object instead of throwing for every non-placement outcome (src/lib/strategy-execution.ts:239-246 declares Promise<{status:string}>; busy at :298, blocked at :498/766/1113/1136/1251/13...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `placement-outcome-truth`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Price alerts stop evaluating silently — no active account, one broker, swallowed errors, no age check (P1, tranche Now, effort M).**  src/lib/alerts.ts:67-69 returns [] whenever the console-active account has no accountNumber (zero connected accounts, or the active one was deleted — deleteConnectedAccount at src/lib/db-api-keys.ts:1376 promotes nobo...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `price-alert-evaluation`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Quote pipeline fabricates asOf and picks the wrong field as price, defeating the staleness gate (P1, tranche Now, effort L).**  Values that were not just observed are stamped with the wall clock: src/lib/data-providers.ts:1571-1590 takeScalar writes cascadeFetchedAt = new Date().toISOString() for any field lacking its own asOf, so a 6h-cached ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `quote-value-provenance`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Run-scoped code re-resolves the console-active account instead of the account being traded (P1, tranche Now, effort M).**  src/lib/red-team.ts:192 calls getActiveConnectedAccount(userId) and :203 calls getStrategyPrompt(userId) with no account argument, even though debateProposal was handed runPolicy carrying the correct connectedAccountI...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `run-scoped-account`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Hand-mirrored Swift decoders drift from route payloads with no contract test — live stop-losses render as em-dashes (P1, tranche Now, effort M).**  FullPolicy decodes stopLossPct/trailingStopPct/shortStopLossPct from top-level CodingKeys (ios/SocraticTrade/DeskModels.swift:383-388) while GET /api/policy returns them nested under riskRules (src/lib/types.ts:1188, ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `web-ios-contract-drift`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Approving is starved by the run lock and the size that gets placed is not the size that was reviewed (P2, tranche Now, effort L).**  executeProposal contends for the whole-run strategy lock (src/lib/strategy-execution.ts:297), so an approval during a 15-20 minute autonomous run returns busy — the console gives up after ~2 min and iOS shows a green ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `approval-path-integrity`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Performance tiles print numbers without the basis or window they were computed on (P2, tranche Now, effort M).**  src/lib/performance.ts:497 caps the "Versus the market" TWR at ~100 snapshots (≈7 trading days) while app/console/results/page.tsx frames it as account performance; "Realized/Unrealized P&L" means lot-ledger on Result...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `pnl-basis-labels`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Non-owner content can reach the always-trusted strategy prompt with no human and a scanner that is inert in production (P2, tranche Now, effort M).**  src/lib/learning-review.ts:640-663 applies any 'keep' verdict with no riskTier exclusion, and applyApprovedPending (src/lib/learned-context/store.ts:397-405) merges strategy-directive rows straight into the strategy p...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `prompt-trust-boundary`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Realized-P&L ledger reads the oldest fills and mismatches lots, so P&L, win rate, tax and wash-sale locks are wrong (P2, tranche Now, effort L).**  src/lib/db-fills.ts:332 is SELECT ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `realized-pnl-ledger`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Shipped copy asserts guardrails the engine does not have and reintroduces the paper/live ceremony the owner ruled out (P1, tranche Next, effort M).**  app/console/macro/indicators.ts:698/706 and again at :753 tell the owner that below-median buys are "hard-vetoed" in risk-off/crisis regimes, while src/lib/strategy-risk.ts:187-197 keeps and tags them (its own comment...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `copy-claims-and-rulings`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Shipped controls whose backend does nothing, or something other than the label claims (P1, tranche Next, effort M).**  policy.taxSettings.subtractFromResults is written by app/console/strategy/tax-settings.tsx:213 and read by nothing in the Results render path (app/console/results/page.tsx:940-994 renders P&L unmodified), so "Show res...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `dead-controls`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] The required merge gate never compiles or runs the iOS app, and can be skipped entirely for build-imported files (P1, tranche Next, effort M).**  .github/workflows/ci.yml:305's required verify job depends only on classify + verify-hosted and never on ios-build.yml, which itself only runs xcodebuild build (.github/workflows/ios-build.yml:70-78) — xcodebuild test...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `merge-gate-blindspots`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] The phone build inherits desktop control sizes and ignores the visual viewport (P1, tranche Next, effort M).**  app/console/components/chrome.tsx:326 renders the full 'Stop Agent'/'Resume Agent' label in a flex-shrink:0 button (console.css:471-484) alongside a non-shrinking StateChip, avatar and ⌘K trigger, so the only min-w-0 ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `phone-touch-viewport`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Entry paths lose their destination or dead-end: expired sessions, OAuth errors, and iOS first-run (P1, tranche Next, effort M).**  middleware.ts:412-419 answers an unauthenticated /api/* with a bare 401 and drops the destination on page redirects, so a push deep link lands on the dashboard after sign-in and app/console/lib/useConsoleData.tsx:139-...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `signin-return-first-run`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Tone tokens fail WCAG AA at the sizes they are used, and light-default is not the default in several hosts (P1, tranche Next, effort M).**  app/console/console.css:40-73 defines tone text on soft fills that measures 3.68-4.34:1 (independently recomputed) at 11px/12.5px — below the 4.5:1 bar since neither size is 'large text' — including the Stop/Start Age...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `visual-tokens-theme-contrast`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Operator surfaces present placeholders, unattributed writes and mislabeled scope as fact (P2, tranche Next, effort M).**  app/admin/page.tsx:448 renders a bare CPU percentage with no caveat although app/api/admin/server-metrics/route.ts:604-613 documents the scaling as unverified and potentially an 8x under-report (the detail page at ser...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `admin-honesty`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] A notification's lifecycle ends before delivery is known, and the tap lands on the wrong screen (P2, tranche Next, effort M).**  src/lib/alerts.ts:88-89 flips the alert to the one-shot 'triggered' state before sendNotification is awaited, and sendNotification never throws on total delivery failure (src/lib/notifications.ts:265-279 folds every e...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `alert-push-delivery`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Sessions are unrevocable 30-day bearer tokens behind a soft perimeter (P2, tranche Next, effort L).**  src/lib/auth/session-token.ts:6 issues a stateless 30-day JWT with no server-side record, so sign-out is a cookie delete and there is no revocation, no 'sign out everywhere' and no absolute lifetime; iOS then holds th...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `auth-session-hardening`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Each broker adapter rewrites the approved order differently and records nothing (P2, tranche Next, effort M).**  There is no shared venue-normalization step: src/lib/alpaca.ts:157 keeps time_in_force=gtc on extended-hours limits that Alpaca rejects with a 422, alpaca.ts:741 never forwards extended_hours on the MCP path, src/lib/...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `broker-order-shape`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Shared console overlay and menu primitives skip the keyboard and announcement contract (P2, tranche Next, effort M).**  The primitives every page composes are non-conformant: app/console/ui/sheet.tsx:116's focus sentry and Escape handler are not stack-aware, so Escape on a drawer or palette above a Sheet closes the Sheet underneath; ap...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `console-a11y-keyboard`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Console pages disagree about where things live, how they save, and how they are searched (P2, tranche Next, effort L).**  Three different save models coexist across the Configure pages (app/console/guardrails/page.tsx:335), and two numeric fields PATCH on every keystroke against the blur-commit pattern every sibling uses (app/console/set...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `console-ia-forms-blotters`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] One transient response or one bad row degrades the whole iOS app (P2, tranche Next, effort M).**  A single failed background reload flips snapshotLoadFailed and the 3-minute staleness gate then disables Approve/Run/Watchlist/Policy (ios/SocraticTrade/MobileStore.swift:223) with no polling fallback; GETs have no 5x...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `ios-network-decode`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Model choice, receipts and cost each derive from a stale or partial source (P2, tranche Next, effort M).**  src/lib/model-rotation.ts:73 hardcodes a 'dead' OpenRouter list that drops slugs the live catalog still serves; app/console/components/approval-card.tsx:127 asserts 'this run's rotation pick, not a failover' even when...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `llm-catalog-routing-cost`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Nothing pages when the desk stops, and the backup/disk story is unverified (P2, tranche Next, effort L).**  app/api/health/route.ts:88 and :128 set checks.schedulerStale and checks.tradingLiveness.degraded without changing the 200 (ok flips only at :78 and :279), and nothing pushes on them — src/lib/scheduler.ts has a singl...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `ops-liveness-durability`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] The suite covers shapes but not money paths, reaches the live internet, and serializes 14 minutes into every run (P2, tranche Next, effort L).**  vitest.config.ts has no setupFiles at all — only a globalSetup that plumbs TMPDIR — so nothing prevents a test from reaching SiliconFlow, SEC or Yahoo (STATUS.md still records those 404s), and maxWorkers:1 serializes ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `test-coverage-ci-cost`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Document structure and accessible names are stylistic rather than semantic (P2, tranche Later, effort M).**  Every console route shares the document title 'Socratic Trade' (app/console/layout.tsx:5, WCAG 2.4.2); 68 heading-styled divs carry con-card-title against 16 real h2/h3 and the collapsible Card demotes its h2 to a spa...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `a11y-structure-naming`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] First console paint waits on a large bundle and then a single blocking snapshot (P2, tranche Later, effort M).**  app/console/layout.tsx:22 makes HTML → ~1.5 MB raw JS → /api/dashboard a strict waterfall, and routes that fetch their own data still wait behind the full-screen snapshot loader (components/shell.tsx:67).  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `console-first-paint-bundle`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Owner copy rules are unenforced on the web: two-space gaps, Title Case, money format, Central time (P2, tranche Later, effort M).**  SENTENCE_GAP is used in 9 files against roughly 600 web copy strings (app/console/lib/format.ts:54), and the double ASCII spaces typed into JSX collapse to one at render (app/console/components/consent-gate.tsx:107) s...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `copy-consistency-rules`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] iOS project-level hygiene: no privacy manifest, hardcoded production origin, no strict concurrency (P2, tranche Later, effort M).**  ios/project.yml:39 ships no PrivacyInfo.xcprivacy although the app uses UserDefaults, a required-reason API — ITMS-91053 on upload; the production origin is hardcoded in LoginView.swift:265, AdminPortalView and DeepLi...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `ios-build-compliance`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] iOS renders a read-only subset of the screens it mirrors (P2, tranche Later, effort L).**  Coach replies print raw markdown (ios/SocraticTrade/CoachView.swift:272) and its Suggested Draft cannot be staged although the copy says to approve it in Proposals (:323); the Scan header advertises three side-by-side...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `ios-parity-gaps`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] Phone layouts are the desktop tree at desktop density (P2, tranche Later, effort M).**  app/console/console.css:90 defines no phone type-scale bump, so 412 uses of 11px and 185 of 12.5px render at desktop sizes on a 6-inch screen; /console/watchlist keeps a 5-column table whose row actions scroll off (wa...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `phone-layout-density`, Bearer token in `~/.secrets/mac-collab.env`).
- **2026-08-19 — CLAUDE — PLANNED — [Review] The public site's two halves do not connect and its metadata is stale (P3, tranche Later, effort M).**  An anonymous visitor never reaches the marketing page (app/page.tsx:14), /welcome offers only a mailto 'Request access' with no Sign in (welcome/page.tsx:114) and /login never explains the product, while /framework — ...  Findings tool: `https://mac.jays.services/board?app=socratic-trade` (external_uid `public-site-funnel-seo`, Bearer token in `~/.secrets/mac-collab.env`).
...
## In Progress
- **2026-08-25 — CURSOR — IN PROGRESS — Console honesty: Guardrails Discard, approve typed-confirm, Home Coach chips + leftover UX.**  Branch `cursor/console-discard-approve-chips-1ffb`, PR #3093.  Rebased onto `origin/main` `c2ed2c85` (#3097 Scan-via-More smoke, after #3094 Datadog and #3096 TestFlight 1.0.69 docs).  Kept the e2e assertion from main.  Did not edit Datadog files.  Post-rebase `verify-hosted` failed copy-rules lint; busy toast and bulk Reject confirm now use `SENTENCE_GAP`.  Designer discard hypothesis confirmed: Discard now runs one `discardAll` that clears `universeDraft`.  Approve cards/bulk resolve `row.executionMode ?? currentMode` and render the 409 phrase.  Home no longer toasts Approved on `busy`.  Coach chips prefill/focus the note.  Leftover UX on the same PR: iOS Open Guardrails tab, Open Connections copy, Tradier (paper).  Do not merge from this seat.  Rollout: `docs/rollouts/2026-08-25-console-discard-approve-chips.md`.
- **[Socratic.Trade][CURSOR] Land leftover open PRs — IN PROGRESS 2026-08-21.**  MERGED #3012 `c614391c`, #3015 `04dbc126`, #3013 `eef81eb6`.  Open: #3018 leftover Finnhub abort rematched; #3014/#3008/#2990/#2941 waiting required `verify`.  Parent owns Coolify.  No `--admin`.
- **[Socratic.Trade][CURSOR] Land leftover open PRs — IN PROGRESS 2026-08-21.**  #2990 rematched onto `origin/main`; neutralized gitleaks FP in the mobile-web handoff Playwright recipe (env names only) and ignored the introducing-commit fingerprint.
- **[Socratic.Trade][CURSOR] Land leftover open PRs — IN PROGRESS 2026-08-21.**  #3008 rematched onto `origin/main`; kept Claude's wordmark unfreeze, 80% login mark, justified lock paragraph, and "Configure framework" bullet.  Applied ScanView `@escaping` so unsigned xcodebuild compiles.
- **[Socratic.Trade][CURSOR] setup-node 4→7 + pin-check test — IN PROGRESS 2026-08-21 (branch `cursor/setup-node-7-a64b`, supersedes Dependabot #2993).**  setup-node@7 itself succeeded on #2993; `ci-workflow-queue-safety` still hardcoded `@v4`.  Accept any `actions/setup-node@` pin so Node stays available before the pin comparison.
- **[Socratic.Trade][CURSOR] Land leftover open PRs — IN PROGRESS 2026-08-21.**  #2941 rematched onto `origin/main`.  `ci-workflow-queue-safety` now accepts the vendor-era pin-check step name (`Compare ST / UM / CT shared-package pins`) so verify-hosted can pass.
- **[Socratic.Trade][CURSOR] Land leftover open PRs — IN PROGRESS 2026-08-21.**  #3013 verify-hosted failed because `enrich` now receives `{ signal }` and `market-preselection` still asserted a single-arg call.  Test updated; do not merge red.
- **[Socratic.Trade][CURSOR] iOS first-launch update prompt (fleet) — IN PR #3012 2026-08-21 (branch `cursor/ios-update-prompt-9992`, worktree `~/apps/trading-cursor-ios-update`).**  On first open, ask to update when a newer version exists.  TestFlight opens TestFlight; App Store opens the App Store.  Manifest `jaywedgeworth22/ios-app-versions`.  Follow-up: `ScanView.screenLine` `detail` is `@escaping` so the unsigned iOS CI compile succeeds (ForEach captures it).
...
- **[Socratic.Trade][CURSOR] iOS release-readiness leftovers (#2560) — IN PROGRESS 2026-08-17 (branch `cursor/ios-release-readiness-2560-b532`).** Close-only/Wind-down + APNs code already on main. This pass: PrivacyInfo.xcprivacy, Safari console handoffs (connections/strategy), honest price-alert copy. Do not invent APNS_* secrets — owner must add the Apple .p8 set in Infisical.
- **[Socratic.Trade][CURSOR] iOS release-readiness leftovers (#2560) — IN PR #2794 2026-08-17 (branch `cursor/ios-release-readiness-2560-b532`).** Close-only/Wind-down + APNs code already on main. This pass: PrivacyInfo.xcprivacy, Safari console handoffs (connections/strategy), honest price-alert copy. Do not invent APNS_* secrets — owner must add the Apple .p8 set in Infisical.
- **[Socratic.Trade][CURSOR] P3 curl-only diagnostics UI entry — IN PROGRESS 2026-08-17 (branch `cursor/p3-curl-only-ui-2563-814a`, closes #2563).** Strategy Weight Tuning Preview, Lessons Learning Ledger, Admin Factor Backtest, Activity Audit tab. Server routes unchanged.
- **[Socratic.Trade][CURSOR] P3 curl-only diagnostics UI entry — IN PR #2793 2026-08-17 (branch `cursor/p3-curl-only-ui-2563-814a`, closes #2563).** Strategy Weight Tuning Preview, Lessons Learning Ledger, Admin Factor Backtest, Activity Audit tab. Server routes unchanged.
- **[Socratic.Trade][CURSOR] Console a11y batch — IN PROGRESS 2026-08-17 (branch `cursor/console-a11y-batch-08ac`, closes #2561).** Light-theme chip AA on soft fills; Sheet/TabsSheet/Columns stack-aware Escape; tooltip keyboard + aria-describedby; Meter accessible name; dark faint headroom; Toggle label required. Presentation only.
- **[Socratic.Trade][CURSOR] Console a11y batch — IN PR #2795 2026-08-17 (branch `cursor/console-a11y-batch-08ac`, closes #2561).** Light-theme chip AA on soft fills; Sheet/TabsSheet/Columns stack-aware Escape; tooltip keyboard + aria-describedby; Meter accessible name; dark faint headroom; Toggle label required. Presentation only. CI verify green.
- **[Socratic.Trade][CURSOR] Retire FilingAPI.dev — use ROIC.ai only — IN PROGRESS 2026-08-17 (branch `cursor/retire-filingapi-roic-de61`, closes #2778).** No live HTTP to filingapi.dev. Health lane and cascade registration removed. ROIC + SEC EDGAR unchanged. Do not buy Plus / do not charge Stripe.
...
- **2026-08-16 — GROK — IN PROGRESS — Review UX: fast approve, live vs proposed price, Retry Red Team, clearer agent controls.** Issue #2752. Worktree `~/apps/trading-grok-review-ux` (remote branch gone; local residue).
- **2026-08-15 — GROK — IN PR #2785 2026-08-17 — Website favicon: cropped offset candlestick ST, transparent.** Issue #2731. Landed on `cursor/favicon-crop-st-e6ee`. Source is the iOS App Icon offset ST; App Icon file untouched.
- **[Socratic.Trade][GROK] Fix ST Litestream wedge and prefer Pushover over Resend — OPEN 2026-08-14.** Issue #2697.
- **[Socratic.Trade][MONET] Durable litestream remote-inventory cache (PR #2665 leftover) — IN PROGRESS.** Issue #2694.
...
- **[P2][Infra][S] Provider-knob sync: API-Usage-Monitor -> Infisical (CLAUDE (opus subagent), — COMPLETED (CURSOR board hygiene 2026-07-24).
  ...
  from Infisical prod over the proven SSH+universal-auth CLI path (box `<HETZNER_OLD_IP_RETIRED>`), and
...
- **Hetzner server migration: prod box 91.98.44.8 (4GB fsn1) -> <HETZNER_OLD_IP_RETIRED> (8GB hel1) — COMPLETED (CURSOR board hygiene 2026-07-24).
...
| 2026-08-19 | MONET | **`per-account-visibility` (P1, tranche-1 cluster, issue #2880).** Multi-account screens showed the ACTIVE account's data under all-accounts labels. Each site classified wrong-scope vs wrong-label first: broker rows now read real per-account policy + a real pending count (was dead code returning 0, and every non-loaded row said "Inactive" even while trading); decisions index keeps its by-design all-accounts fetch and gains an account chip; `mobileCommandBacklog` was global across USERS and is now user-scoped. 4 new test files + 2 updated; failing-first proven per change. Full gate green: lint 0 errors, tsc clean, 7071 tests, build 0. Rollout: `docs/rollouts/2026-08-19-per-account-visibility.md`. | **In review (PR)** |
| 2026-08-19 | MONET | **Two-space rule: documented HOW to emit it so it is visible.** Owner-verified: chat replies must use the `&nbsp;` ENTITY after the period (`Sentence one.&nbsp; Sentence two.`) because GFM collapses literal double spaces AND a raw U+00A0 in the rendered view; files (docs, commits, PR bodies, Slack, Notes) keep two literal spaces. Recorded what does not work (no app setting exists; do not patch the signed client) and the process lesson: diagnose the rendering layer before re-promising compliance. AGENTS.md + FLEET-UI-COPY.md. | **Completed (PR #2893 MERGED)** |
| 2026-08-19 | MONET | **All review clusters no other seat is handling (issue #2943).** Cross-checked the 45 clusters against every open PR + #2866's in-flight list. Claiming 16: web-ios-contract-drift, realized-pnl-ledger, prompt-trust-boundary, pnl-basis-labels, signin-return-first-run, visual-tokens-theme-contrast, admin-honesty, console-ia-forms-blotters, ios-network-decode, llm-catalog-routing-cost, ops-liveness-durability, a11y-structure-naming, copy-consistency-rules, ios-build-compliance, phone-layout-density, public-site-funnel-seo. Plus the 4 owner decisions from #2881 (Autopilot off-switch, 5% headroom as a policy field, Stopped queues approvals, session revocation). Isolated worktree + failing-first test + full gate + skeptic pass per item; landing in waves. | **In Progress** |
| 2026-08-20 | MONET | **`web-ios-contract-drift` (issue #2912).** Committed contract fixture pinning `GET /api/policy` to the Swift `FullPolicy` decoder, asserted from BOTH sides, so a rename fails CI instead of blanking a phone list. api-01 was already fixed on main by #2863 while this was in flight -> dropped the competing decode, kept main's. **qa-04 stays OPEN.** Found + reported that main's iOS TEST TARGET does not compile (MobileModelsTests.swift:403 encodes a non-Encodable type; 5 errors on clean main, arrived via #2863) - qa-01/qa-02 in the wild; Swift assertion ships present-but-unrunnable, stated honestly. Gate: lint 0, tsc clean, 7109 tests, build 0, xcodebuild build 0. Rollout: `docs/rollouts/2026-08-20-web-ios-contract-drift.md`. | **Completed (PR #2944 MERGED, verify green)** |
| 2026-08-20 | MONET | **`pnl-basis-labels` (issue #2914).** Performance numbers now state the basis and window they were computed on, across web AND iOS: "Unrealized P&L" no longer means three different things on Results/Home/iOS, an empty sample reads as no-data instead of a confident 0%, and mark-to-market sums shorts on |basis| instead of netting them against longs (costBasis changed meaning net->gross; identical for an all-long book). "-" vs "n/a" distinction preserved. **perf-17 NOT closed** (SPY benchmark total-return vs price-return by provider, intraday-vs-EOD misalignment) - confirmed, not papered over. Gate: lint 0, tsc clean, 7140 tests, build 0, xcodebuild build 0. Rollout: `docs/rollouts/2026-08-20-pnl-basis-labels.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`web-ios-contract-drift` (issue #2912).** Committed contract fixture pinning `GET /api/policy` to the Swift `FullPolicy` decoder, asserted from BOTH sides, so a rename fails CI instead of blanking a phone list. api-01 was already fixed on main by #2863 while this was in flight -> dropped the competing decode, kept main's. **qa-04 stays OPEN.** Found + reported that main's iOS TEST TARGET does not compile (MobileModelsTests.swift:403 encodes a non-Encodable type; 5 errors on clean main, arrived via #2863) - qa-01/qa-02 in the wild; Swift assertion ships present-but-unrunnable, stated honestly. Gate: lint 0, tsc clean, 7109 tests, build 0, xcodebuild build 0. Rollout: `docs/rollouts/2026-08-20-web-ios-contract-drift.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`prompt-trust-boundary` (issue #2915).** Closes coach-06/llm-04/llm-13/coach-05/llm-18. Untrusted content (fetched pages, LLM paraphrase) could reach the always-trusted strategy prompt verbatim and unlabelled. Fix is containment + provenance, NOT friction: trust boundary moved INTO the sink (`mergeStrategyDirectiveBlock` now REQUIRES `source`, contains internally, returns {prompt,contained}) so no caller can skip it; coach URL lessons contained at ingest and dropped+audited on a real hijack idiom; forgeable unauthenticated POST /api/chat-history deleted; MockLLM semantic-gate fallback now audited instead of silent; revalidation rationale contained + data-not-command clause. Owner text passes byte-for-byte (containment keys on provenance, not text); decide-mode auto-apply untouched per the owner ruling. Gate: lint 0, tsc clean, 7146 tests, build 0; failing-first 7/8. Rollout: `docs/rollouts/2026-08-20-prompt-trust-boundary.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`realized-pnl-ledger` (issue #2916).** Ledger reads no longer truncate: `listFillEvents` defaults to the WHOLE ledger; a numeric limit still exists for display windows and returns the NEWEST N. Rejected the plan's newest-DESC-everywhere fix - for a stateful FIFO replay, cutting either end is wrong (tail freezes P&L, head strands exits whose opening lot was never loaded). **Two members bite TODAY, no 500-row assumption:** the tuner's "recent" fills window was a LEADING slice of an oldest-first array, so the paid LLM review argued weight nudges from the account's FIRST 20 trades; and decision grading resolved on the first partial exit, scoring a -$100 trade as a +$40 win (failing-first: expected 40 to be close to -100). Also: unmatched closing fills disclosed not dropped, covered shorts realized short-term (IRC 1233). **perf-11 deliberately NOT closed** - regrouping win rate per round trip moves a number feeding every scorecard, the Kelly split and the tuner gate; needs an owner decision. Rebased onto merged #2950 with 3 overlaps hand-reconciled. Gate: lint 0, tsc clean, 7205 tests (up from 7140), build 0. Rollout: `docs/rollouts/2026-08-20-realized-pnl-ledger.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **iOS test target repair (owner-directed).** The Swift test target did NOT COMPILE on main (5 errors: MobileModelsTests encoded a Decodable-only MobileSnapshot), so no iOS test could run and three stale DeskModelsTests assertions were invisible - qa-01/qa-02 in the wild. Fixed test-side: the app caches RAW response bytes and never re-encodes, so the tests now seed the cache the same way (adding Encodable would bend the model to fit a test). Stale assertions corrected to the real contract: "mock" was deliberately removed by #2887 (keyless offline = the mock/demo path the rules forbid), so provider(for:) falls through to openai and firstAvailable returns nil with no keys; added missing mistral/deepseek/moonshot coverage. Self-contradictory scan test resolved to the implementation joined-warnings form (the second warning is what tells the owner it is a stale fallback scan). Verified: xcodebuild test exit 0, zero failures, 59s. Rollout: `docs/rollouts/2026-08-20-ios-test-target-repair.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **Wave 2 claim - llm-catalog-routing-cost (P2, M).** Live OpenRouter catalog + response-billed cost become the source of truth for rotation, receipts and /console/usage; compact Red Team payload to the documented subset. Files: src/lib/model-rotation.ts, src/lib/llm-usage.ts, src/lib/chat/llm.ts, src/lib/strategy.ts, app/console/components/approval-card.tsx. Verified clean of all 22 open PRs. | **In Progress** |
| 2026-08-20 | MONET | **Wave 2 claim - ios-network-decode (P2, M).** Element-tolerant decoding (drop+count bad rows, not whole-payload failure) + bounded retry/backoff for idempotent GETs and SSE, so a deploy swap or one bad row degrades a section not the app. Files: ios/SocraticTrade/MobileModels.swift, MobileStore.swift, MobileAPIClient.swift. Distinct from peer #2794 (DeepLink/HomeView/MarketsView/project.yml). | **In Progress** |
| 2026-08-20 | MONET | **Wave 2 claim - copy-consistency-rules (P2, M).** Owner copy rules are unenforced on the web: SENTENCE_GAP used in 9 files vs ~600 strings, typed double spaces collapse at render, public pages have zero NBSP gaps, compact money uppercase, one legal timestamp missing Central tz. Adding the copy-rule lint the iOS suite already has + one pass. Avoiding chrome.tsx (peer #2795). | **In Progress** |
| 2026-08-20 | MONET | **Wave 2 claim - admin-honesty SECURITY HALF only (P2).** Taking the parts no peer PR touches: no page under app/admin/** has ANY server-side gate; /api/chat-history has no requireAdmin yet nav calls it 'every chat turn'; server-knobs audits every write as user 'local' regardless of which admin flipped it. NOT taking the CPU-caveat/identity-chrome half - peer #2795 owns app/admin/page.tsx + server-metrics-client.tsx and #2793 owns app/admin/layout.tsx. | **In Progress** |
| 2026-08-20 | MONET | **Wave 2 claim - console keystroke-PATCH slice of console-ia-forms-blotters (P2).** Two numeric fields PATCH on EVERY keystroke against the blur-commit pattern every sibling uses, and a mid-edit empty field saves the fallback 2. Files: app/console/settings/page.tsx, app/console/strategy/overlays-panel.tsx. Rest of that cluster (nav/activity/guardrails/blotters) left to peers #2793/#2795/#2828. | **In Progress** |
| 2026-08-20 | MONET | **Wave 2 HELD - ops-liveness-durability + a11y-structure-naming.** Not started: peer #2816 owns app/api/health/route.ts + trading-liveness.ts + r2-cold-snapshot.ts, peer #2817 already adds the split app/api/live/route.ts + Dockerfile probe, and peer #2795 owns app/console/ui/primitives.tsx (Meter/Card/Tooltip) - i.e. the exact 'fix the primitives once' change. Re-claim only if those PRs close unmerged. | **Held (peer-owned)** |
| 2026-08-20 | MONET | **B2 restore proven -> historic R2 replica is prunable (docs-only).** Answers "why is R2 bigger than a 1-week snapshot": `socratic-trade-bucket` holds TWO sets - `cold-snapshots/` (LIVE weekly DR, retain 1, DB ~4.2GB) and `trading-live/**`, the ENTIRE pre-cutover litestream replica (~90k-object L1 backlog) kept since the 2026-08-07 B2 cutover. Owner confirmed the B2 restore is proven, lifting the config own "do not delete R2 until proven" blocker. **FOOTGUN recorded:** both replicas use the IDENTICAL path `trading-live/app.db` - only bucket+endpoint differ (R2 socratic-trade-bucket = dead; B2 jays-socratic-trade-eu = LIVE) - so a prune against the wrong endpoint destroys the active backup. Did NOT execute the prune: no object-level R2 delete tooling and deleting backups needs an explicit human decision. Rollout: `docs/rollouts/2026-08-20-r2-historic-prune-unblocked.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`admin-honesty` SECURITY HALF (P2, issue #2943).** Confirmed the review's worst claim: `grep -rn "requireAdmin\|isAdmin\|getSession\|redirect(" app/admin/` returned ONLY COMMENTS across all 24 files -- any authenticated allowlisted user could load the whole operator tree (client components, so only the numbers 403'd).  One edge gate in middleware.ts covers `/admin` + `/admin/**` without touching app/admin/page.tsx or layout.tsx (peer #2795/#2793).  Shared edge-safe `isAdminEmail` extracted to new src/lib/auth/admin-emails.ts so the page gate and requireAdmin cannot drift.  CORRECTS the review on defect 2: /api/chat-history is the SHARED Coach history, not an ungated admin endpoint -- adding requireAdmin would have broken chat on web AND iOS; instead added a genuinely cross-user requireAdmin-gated /api/admin/transcript so the nav's "every chat turn" becomes true.  Knob writes now record the acting admin instead of the literal string 'local'.  Hand-resolved a middleware.ts conflict with newly-landed #2957 (both functions kept; gate placement re-verified after/before the fail-closed and forwarding steps).  Honest scoping: exploitability is narrow today (empty ALLOWED_EMAILS means only the primary can authenticate, and primary is always admin) -- it matters once a second entry exists.  Rollout: `docs/rollouts/2026-08-20-admin-security-gate.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`llm-catalog-routing-cost` (P2, all 5 defects, issue #2943).** REFUTED the review's prescribed fix rather than implementing it: OpenRouter's `usage:{include:true}` is DEPRECATED AND INERT (usage is always returned now), and adding it would be dead weight plus risky, since `require_parameters:true` filters endpoints by advertised request fields -- the same class that caused the documented 404 storm.  The real fix was read-side: `usage.cost` was already arriving on every response and being discarded.  Headline: on 100k in / 20k out of claude-opus-5 the hand-maintained price table says $1.00 where the transport BILLED $0.25 -- a 4x overstatement on one call, and /console/usage was derived entirely from that table.  Billed vs estimated now separate columns/sums, never presented as one billed total.  Also: late post-soft-timeout replies are metered not just audited; the approval receipt stops asserting 'not a failover' when a fallback answered; rotation reads the live catalog instead of a hardcoded dead list; Red Team payload compacted to the documented subset (>90% cut).  Found 2 things the review missed: migrations 2 and 14 use `PRAGMA table_info` as an existence check, which returns EMPTY for a missing table (filed #2964, and it bit this change's own migration 86), and a test fixture matching on a substring of the whole payload caused a real 3-test cross-contamination that was bisected and repaired without weakening assertions.  Rollout: `docs/rollouts/2026-08-20-llm-catalog-routing-cost.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **Claim - `ios-parity-gaps` (P2, L, issue #2937) + iOS<->web COPY parity (owner-directed 2026-08-20).** Owner: improve iOS-web parity, and **be wise about which text or variation to adopt - do NOT assume one side is always superior**. So each divergence gets judged on its merits, adopting web wording where web is clearer and iOS wording where iOS is clearer, rather than a blanket sync in one direction. Known concrete divergence to settle: web `field-defs.ts` says "Max per order" vs `ios/SocraticTrade/GuardrailsView.swift:69` "Max Order". Feature-parity side: Coach prints raw markdown, Scan header advertises 3 columns that stack, Activity claims 'everything the agent did' but shows only fills+phone commands, Positions omit per-position P&L, Results has no equity curve, symbol sheet has no actions. | **In Progress** |
| 2026-08-20 | MONET | **RETRACTED claim - `ios-state-outcome-truth` (P1, issue #2904) is ALREADY FIXED.** I claimed this then verified before starting: PR #2863 (`fix(ios): clear session snapshot, show edit errors, decode nested stop-loss`) already landed every part of it. `clearLocalSession` removes BOTH `cacheKey` and `cacheTimestampKey` (MobileStore.swift:699-700); `testColdLaunchAfterSignOutDoesNotRestoreCachedSnapshot` pins it (MobileModelsTests.swift:461); a real `cacheTimestampKey` + `loadCachedSnapshotSavedAt()` replaced the `lastUpdatedAt = Date()` stamp; and `store.error` AND `store.successMessage` both render as `.alert`s (AppComponents.swift:630-650), which is exactly the fix the review prescribed. Caught by CURSOR-BUGBOT flagging it in #agent-sync before I spent anything on it. The 2026-08-18 review is stale here - treat its iOS line numbers and claims as needing re-verification against HEAD. | **Not needed (fixed by #2863)** |
| 2026-08-20 | MONET | **HELD - `ios-build-compliance` (P2, issue #2936).** Not claiming: peer PR #2794 already owns `ios/project.yml`, `PrivacyInfo.xcprivacy`, `DeepLink.swift`, `HomeView.swift`, `MarketsView.swift`, which is where the privacy-manifest, base-URL and strict-concurrency work all lands. Re-claim only if #2794 closes unmerged. Note for whoever holds it: the hardcoded production origin (LoginView.swift:265) means every simulator run hits PRODUCTION despite `MobileAPIClient.productionBaseURL` existing to prevent exactly that. | **Held (peer-owned)** |
| 2026-08-20 | MONET | **iOS<->web parity, judged divergence by divergence (owner-directed, issue #2937).** Owner: be wise about which text to adopt, do NOT assume one platform is superior.  Of 21 divergences: 9 adopt-web, 6 KEEP-BOTH, 4 ADOPT-iOS, 2 both-wrong.  A blanket sync would have been wrong on 12 of 21.  WEB was the drifted side on Title Case -- FLEET-UI-COPY.md names 'Run Once', 'Price Alerts', 'Current Policy', 'Win Rate' as canonical and every one is the iOS spelling.  Preserved on purpose: the 4-tab cap makes 'Assets' consolidation correct; web's Results blurb promises an equity curve and scorecards iOS does not have.  CORRECTED MY OWN INSTRUCTION: I claimed unhyphenated 'Exit Only' existed nowhere else -- it exists in 3 places and is a consistent COMMAND name parallel to 'Wind Down', since the app runs two deliberate vocabularies (Title Case commands vs sentence-case states).  The implementer fixed the real bug instead (a state title wearing command casing) AND found the identical defect the audit missed ('Winding Down' at AgentControlPlan:75).  Also caught that renaming only the read-only Guardrails rows would have left PolicyTightening's EDIT controls calling one field a different name.  Coach markdown renderer added with web's two security properties (no raw HTML, no auto-loaded remote images -- Coach output is untrusted model text carrying RAG content).  Scan table header DELETED rather than ported (a phone card list is not a table).  Verified: BUILD SUCCEEDED + 190 iOS tests, failing-first proven (9 failures on revert).  Rollout: `docs/rollouts/2026-08-20-ios-web-parity.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`copy-consistency-rules` (P2, issue #2943) - the two-space rule now RENDERS, not just reads right in source.** The real defect was that two literal ASCII spaces typed into JSX COLLAPSE to one at render, so source looked compliant while the screen was not.  Measured 698 violations across 73 files (review estimated ~600); fixed 652 (93.4%).  The 46 left are 100% in peer-locked chrome.tsx(29)/guardrails(16)/admin(1).  Byte-proof on the exact file the review cited: 0x20 0x20 -> 0xa0 0x20.  New `scripts/copy-rules-lint.mjs` + 17 tests incl. a renderCollapse() proof that asserts on RENDERED output, so this cannot silently rot again.  Building the fixer surfaced THREE bugs in the fixer itself, all caught by running the suite rather than trusting the regex - two of them altered real code (`params.id ?? ""` became `??  ""`), one corrupted numbered headings on the public legal pages, one mangled an IRS citation.  All now defended by regression guards.  Corrects the review twice: the 'three names for one run state' claim is STALE (fixed by #2842), and 13 of 14 flagged toLocaleString calls were plain number formatting needing no timezone.  Rollout: `docs/rollouts/2026-08-20-copy-rules-lint.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **`console-ia-forms-blotters` keystroke-PATCH slice (P2).** Two console numeric fields PATCHed the server on EVERY keystroke against the blur-commit pattern every sibling uses, and an emptied field silently wrote a fallback as a real setting -- these are risk/strategy knobs, so the harm is the silent wrong write.  Now local draft + commit on blur; blank or unparseable commits NOTHING and reverts.  Corrects the review: the fallback is literally `2` only in overlays-panel; in settings/page.tsx it is each row's own defaultValue.  Honest limitation recorded -- the commit DECISION is behaviorally tested (pure functions, real inputs), the WIRING is pinned by source-text assertions because this repo has no DOM test tooling at all (no jsdom/testing-library/environment), and adding one is a dependency decision that should not ride along on a two-file fix.  Third site with the identical bug filed as #2958, not fixed (peer-owned area).  Rollout: `docs/rollouts/2026-08-20-console-keystroke-commit.md`. | **In review (PR)** |
| 2026-08-20 | MONET | **Cancel what you time out + stop launching duplicates on live orders (from the stall investigation).** `withDeadline` was a bare Promise.race -- on timeout the caller moved on while the underlying broker call kept running with its socket open, for the LIFE OF THE PROCESS.  Now takes an optional AbortController, aborting on the timeout branch only.  `awaitWithFirstCallRetry` was worse: it issued a SECOND start() at 16s and never aborted the first.  Fix 2: both scheduler in-flight guards attached `.finally` to the RACED promise, so any broker lane slower than the 15s deadline got a duplicate launched on top every 60s tick -- on the stale-exit lane that is DUPLICATE REMEDIATION AGAINST LIVE ORDERS.  Fix 3: tick re-entrancy guard.  MONEY-PATH DISCIPLINE: only 4 idempotent GETs are abortable; createOrder/cancelOrder/replaceOrder are un-abortable STRUCTURALLY (no signal param exists on those paths) because an aborted placement may still have reached the broker and #2960's 409-reconcile depends on the call completing.  REFUTED the investigation's premise: there is no fetch in the Alpaca gateway (axios inside a bundled SDK whose module instance an interceptor cannot reach -- ESM vs CJS, verified `same default object: false`), so that workaround was deliberately NOT shipped; a money-path change that silently does nothing in prod is worse than no change.  Filed as #2970.  Also narrows the hunt: Tradier and Robinhood never leaked -- both already pass AbortSignal.timeout.  And an EXISTING test encoded the bug as the contract and was rewritten.  Rollout: `docs/rollouts/2026-08-20-abort-on-timeout.md`. | **In review (PR)** |
| 2026-08-21 | CLAUDE | **Numeric settings committed on blur (branch `claude/settings-commit-on-blur`).**  SILENT DATA LOSS, iPhone included: `SourceSettingNumberRow` saved only from `.onSubmit` and its field is a `.decimalPad` — a keyboard with NO RETURN KEY — so the only save path could never fire and every typed number was discarded on tap-away, no PATCH, no message.  Also fixed two more defects in the same row: `text` was `@State` seeded in `init` while the ForEach keys on `row.id`, so the row is reused and the field kept showing stale local edits after a server change or a rejected PATCH; and unparseable input hit `guard … else { return }` and vanished silently instead of reverting.  Decision extracted to `NumberFieldEditor` (one rule, 12 tests) because the two call sites disagreed.  **Two things found that were not in the report:** `Double("1,5")` is nil and `.decimalPad` prints the DEVICE locale's separator, so parsing with `Double` alone would have made the fix discard comma-locale input — added a locale-aware fallback; and `apply()` overwrote BOTH LLM budget text fields, so committing the token cap would have wiped an unsaved cost cap out from under the cursor — now skipped for the focused field.  `LlmBudgetSection` given the same treatment (nothing was lost there — it has an explicit Save button — but the `.onSubmit` was equally dead).  ONE keyboard `Done` on the Form, not per row; no `.frame(maxWidth:)` on the field.  Also corrected a STALE test expectation that had main red on clean `origin/main` (`statusTitle` is a SectionHeading subtitle -> sentence case per FLEET-UI-COPY; left the neighbouring Title-Case states alone as an owner copy call).  Gate: **221 tests 0 failures** on the Mac Catalyst destination (this Mac's iOS simulator is wedged — root-owned `simdiskimaged`); mutation-checked, reverting the fix fails 6 of 12.  NOT visually verified on a device. | **Completed (PR #3007 MERGED)** |
| 2026-08-21 | MONET | **P0: every deploy failed for ~5h on a top-level await; prod served STALE code.** Found while acting on an owner request. Coolify reported '4 failed deployment(s) in last 5' + 'Running container predates the last (failed) deployment'. EVERY deploy from 2026-08-21T05:07Z failed with `Top-level await is currently not supported with the "cjs" output format` in scripts/assert-rth-deploy-latch.ts, so the day's merged money-path fixes never shipped while the container kept serving healthy. ROOT CAUSE: package.json declares `"type":"module"` but the Dockerfile runs the latch at line 53 and COPYs package.json at line 54 -- deliberately, so a no-op dies in seconds instead of after a 30-min npm ci. tsx therefore sees no package.json, cannot read the module type, and falls back to CJS where top-level await is fatal. Runs fine locally, fails only in the image -- which is why it shipped. FIX: await moved into an async main() with a .catch() that exits 1, so a crash can never silently ALLOW a build. Deliberately did NOT move the COPY earlier -- that surrenders the fast-fail ordering the script exists for. Regression test PARSES the Dockerfile, finds every `RUN tsx *.ts` before the package.json COPY, and transpiles each under CJS -- generalizes to any future build-time script. Failing-first proven against the exact prod error string. Latch is peer #2817's and the design is sound; fixed here because it blocked ALL deploys for every seat. Rollout: `docs/rollouts/2026-08-21-deploy-latch-top-level-await.md`. | **In review (PR)** |
| 2026-08-21 | CLAUDE | **Login wordmark unfrozen + justified legal copy + two Catalyst-only defects (branch `claude/login-wordmark-catalyst`).**  The candlestick wordmark was FROZEN: `TimelineView(.periodic(from: .now,…))` anchors its phase to the `.now` captured when the schedule is built, and SwiftUI rebuilds that schedule on every body evaluation — inside a ScrollView the next tick kept being pushed a second into the future.  The web ticker drives the same integer tick off rAF and has no anchor, which is why the site moved and the app did not.  **The identical defect is in `SnapshotScaffold`'s 30s tick** (the "updated N seconds ago" line on every tab) and survived #2987, which rewrote that scaffold's layout but not its schedule.  Also: wordmark -20%, "strategic" dropped (iOS + web bullets), lock line + disclaimer merged into ONE justified paragraph (new `JustifiedText` — SwiftUI has no justified case).  Two defects found by building and capturing the Catalyst window: the Mac login screen was wearing the SIGNED-IN tab bar and title (opacity cannot reach chrome the system promotes into the window), and my own justified paragraph had nine-space rivers from `"""` continuation indentation.  Carried two small fixes: `SwipeRevealAction` advertised a disabled rotor action that silently did nothing plus a false gesture hint, and `SymbolInfoSheet` gets `.presentationSizing(.page)`.  **SCOPE: the tab-bar and iPad-layout halves of the owner's ask were DROPPED, not merged** — #2987 landed the same verbatim prompt first and went further in places; re-applying mine would have reverted it.  Gate: 216 tests, sole failure is the `"Winding Down"` expectation that is already red on clean main (fixed in #3007).  Mac Catalyst destination (iOS simulator wedged, root-owned `simdiskimaged`).  Mac verified visually; iPhone/iPad screenshots still owed.  Rollout: `docs/rollouts/2026-08-21-login-wordmark-and-catalyst-chrome.md`. | **Completed (PR #3008 MERGED)** |
| 2026-08-21 | MONET | **Web adopts iOS's Title Case - the WEB half of iOS/web parity (#2937).** Of 21 divergences, 4 were cases where iOS was correct and WEB had drifted: FLEET-UI-COPY.md names 'Run Once', 'Price Alerts', 'Current Policy', 'Win Rate' as canonical and every one is the iOS spelling.  67 strings across 10 files - buttons, card/section headings, stat-tile labels.  THE DISTINCTION THAT MATTERED: run STATE words are values and stay sentence case on BOTH platforms ('Winding down', 'Exit-only') - `grep '"Winding Down"'` returns zero in chrome.tsx, and derive.ts's RunStateWord union is untouched.  Title-Casing the state word because the adjacent button is Title Case is the one change here that would have done real damage.  REVERSED one implementer call on review: 'Wind Down - This Sells' back to 'Wind Down - this sells', because the appended clause is a WARNING PHRASE, not part of the control's name.  Left two 'Run once' sites sentence-case deliberately - every sibling entry in those arrays is a sentence-case action phrase, so consistency WITHIN a surface outranked consistency across.  Hand-resolved a results/page.tsx conflict with the just-landed #2971 where ours had the NBSP gaps and theirs had the Title Case - taking either side wholesale would have dropped one fix.  Rollout: `docs/rollouts/2026-08-21-web-title-case-parity.md`. | **In review (PR)** |
| 2026-08-21 | MONET | **HANDOFF (design only, no code): replace the daytime deploy ban with a quiescent-cutover swap.** Owner-directed 2026-08-21: stop the RTH build ban, keep the old container serving, and cut over only when no strategy run or embed is in flight. Note leads with the constraint that shapes everything: the app is a SINGLE-WRITER SQLITE process (`DB_PATH=$DATA_DIR/app.db`), so literal blue-green (both containers alive, flip traffic) is NOT achievable and must not be designed toward -- two writers plus litestream is how a real-money ledger gets corrupted. The achievable version is a brief restart taken at a quiescent moment; the difference that matters is never cutting a run in half, not seconds of downtime. Also corrects the premise that the build stops the container (it does not -- Coolify builds first, then swaps; the pain is that the BUILD is refused at Dockerfile:53, exit 2). Documents what already exists so it is not rebuilt (SIGTERM already forwarded at coolify-prod-start.sh:140-141 and mapped to 143/130 by exit-guard.ts:29-31). Names five traps incl. that `hasInFlightStrategyWork()` is unscoped/unbounded with no age floor, so ONE stuck 'running' row would make the app never quiescent -- do not use it as the gate as-is. Leaves the money-path decision explicitly to the owner: what happens when the drain cap expires with a run in flight. Credits #2817 as sound design being superseded by policy, not fixed. Handoff: `docs/rollouts/2026-08-21-handoff-quiescent-cutover-deploy.md`. | **Handoff written (unclaimed)** |
| 2026-08-31 | CLAUDE | **Owner-directed audit: self-hosted Qdrant eval + RAG optimization + alerted/silent error sweep + iOS parity + web/iOS UX (board 601d581c).**  10-agent audit, every P0/P1 adversarially re-verified.  APPLIED LIVE: 14 Qdrant payload indexes (16 total — every production filter field), mem cap 6g->10g, snapshots onto the volume, the 3 runbook UptimeRobot keyword monitors (803872370-72; liveness + litestream fired immediately, correctly — trading had been halted 10.4 days with zero pages).  HEADLINE: Pinecone read units EXHAUSTED, decisions silently run without RAG — cutover is the unblocking move (adapter + frozen golden set d2ac60c9 + delta copy w/ sentinel backfill 9e19673a).  New rows: 8620cad8 (CT share dead-auth 914x401) a9676caf (restart-loop no-alert/no-forensics) ab03d8c9 (alpaca-insights Sentry); evidence comments on 06df80cf + 1e3df744 (blind wedge detector, B2 403 root cause).  Hetzner: cx53 pays for 320GB, primary_disk_size=160GB — same-type rescale WITH disk claims it (owner window needed).  Report: https://claude.ai/code/artifact/4577e78f-88d8-45c2-a346-40dc43263213  Rollout: `docs/rollouts/2026-08-31-qdrant-rag-error-ux-audit.md`. | **Completed (infra live; docs PR)** |
| 2026-08-31 | CLAUDE | **Qdrant READ-path cutover, stage 1 (boards 9e19673a + d2ac60c9; owner-directed "now").**  Agent team + adversarial review (verdict SHIP-WITH-FIXES-APPLIED — review caught 3 real defects pre-ship: sentinel metadata leak that would drop every backfilled legacy vector post-fetch, voyage embed-space hole feeding cross-model garbage past the 0.30 floor, and total-Qdrant-outage reading as successful empty retrieval).  New src/lib/vector-store/ (translator throws on unimplemented ops — never drops a clause), runtime knob RAG_VECTOR_READ_QDRANT (Admin > Operations), six-tier dispatcher with pinecone branch call-for-call equivalent, qdrant metering.  71-case frozen golden set (72/72 refs live-verified), sentinel-backfill.py (live dry-run: 0 missing), DELTA-RUNBOOK.md + copy-script true-delta fix.  Gate: tsc clean, 262 tests green (37 new), eslint 0 errors.  Prod flips via Infisical env at deploy; revert = Admin knob or env unset.  Local golden LIVE run blocked on SILICONFLOW_API_KEY 401 (owner refresh needed); prod telemetry is the live validation.  Rollout: `docs/rollouts/2026-08-31-qdrant-read-cutover-stage1.md`. | **In review (PR)** |
