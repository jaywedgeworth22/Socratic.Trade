- **[Socratic.Trade][GROK] FMP/Quiver intentional OFF on Connections health + FMP policy defaults false — IN PROGRESS 2026-08-05 (branch `grok/data-sources-overhaul`).** Admin health: FMP variants + Quiver show muted OFF not red STOPPED; DEFAULT_POLICY fmp* toggles false; FMP removed from API key catalog; Settings FMP card retired. Source of truth: retired-direct-vendors.ts.
