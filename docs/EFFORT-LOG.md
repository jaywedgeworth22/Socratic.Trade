...
| 2026-07-23 | **Admin Panel Server RAM Fix & CI ESLint Ignores** — Extracted true physical memory capacity (in GB) from the Hetzner metadata payload to render correct memory percentages, and added `**/.worktrees/**` to `eslint.config.mjs` to prevent linting other agents' worktrees. | ANTIGRAVITY | **Completed** — Fixes verified via CI and PR ready to merge. | fix/cleanup-caches-runner |
