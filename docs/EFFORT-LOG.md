- **[Socratic.Trade][CURSOR] Activity layout redesign — IN PR 2026-08-21 (PR #3031, branch `cursor/activity-layout-redesign-085b`, rematched onto `origin/main` `ffb4ea1b`).**  Website + iOS Activity: Alerts Center, Notifications, Strategy Runs, Order Fills, Audit Log (former All).  Plain-English run failures.  APNs-first + admin system alerts.  Alpaca session-close not Delayed Quote.  Additive `?tab=` deep links only.  Cloud-proxy missed `pull_request`; dispatching `verify` + `ios-build`.  Rollout: `docs/rollouts/2026-08-21-activity-layout-redesign.md`.
