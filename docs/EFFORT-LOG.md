- **[Socratic.Trade][GROK] Rotation fail-closed + Red timeout + Alpaca penny 422 + RAG ingest unstick — IN PROGRESS 2026-08-16 (branch `grok/48h-money-path-ingest`, issue #2749).**  Drop known-dead slugs on /models/user timeout.  Red uses Green capturing timeout.  Alpaca >=$1 rounds to $0.01.  ROIC single-flight + requeue budget-misclassified SEC dead letters.  Sibling #2748 owns the daily WU fuse.
