...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, MERGED PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, READY PR #1429, branch `codex/strategy-lease-correctness`) — CURRENT-MAIN GREEN / HOSTED CHECKS RUNNING 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Approval executions now use invocation-unique lock owners; a shared heartbeat guard records refused/thrown renewals and re-proves ownership before broker placement; obsolete wrong-signature teardown calls are removed. `SCHEDULER_SINGLE_LEADER` remains default-on even when the env value is unset/empty. Adversarial review fixed a pre-cleanup setup leak when the run receipt insert throws, added a typed approval `busy` result with no broker call after ownership loss, and preserves already-completed autonomous proposal results on a later loss. Current `main@e395e65a` merged cleanly. Final Node24 gate: focused 7 files/36 tests, lint 0 errors/404 warnings, tsc, full 334 files/3,764 tests, build. The first build caught a client-trace `node:crypto` bundle edge; Web Crypto replacement plus focused 3/11, tsc, scoped lint, full test, and build reruns are green. Head `f70e9043` is pushed; no provider-boundary or production-config work is included. Hosted checks/review and production verification remain.
