...
- **[Socratic.Trade][CURSOR] iOS Scan empty table is a quote miss — IN PROGRESS 2026-08-18 (branch `cursor/scan-empty-screener-a128`).** Owner 12:03pm CT: "0 names · 2 watched" / "No Candidates" on a 200 empty `topCandidates`.  Empty screener + expired seed now 503s.  iOS surfaces scanned/quotes/warnings; watchlist is not the universe.  Yahoo-fallback covers the whole allowed set.  Not stacked on #2829 / #2800 / reserved #2792 #2798 #2794.  Rollout: `docs/rollouts/2026-08-18-scan-empty-screener.md`.
- **[Socratic.Trade][CURSOR] iOS Scan empty table is a quote miss — IN PR #2830 2026-08-18 (branch `cursor/scan-empty-screener-a128`, rebased onto `6429d984`).** Live cause: stub `"Mozilla/5.0"` UA + bare 8s abort on `fetchNasdaqScreener` (prod aborting since 2026-08-13T22:30Z).  Now `BROWSER_UA` + `fetchWithRetry`.  Yahoo whole-set fallback if Nasdaq still fails.  503 last resort only.  Live S&P 500 skip-enrichment on this VM: 498 quotes / 38 candidates.  Rollout: `docs/rollouts/2026-08-18-scan-empty-screener.md`.
