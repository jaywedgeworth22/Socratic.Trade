- **[Socratic.Trade][MONET] iOS OAuth return-to-app + workspace decode fixes — IN PR 2026-08-27 (branch `monet/ios-auth-redirect-workspace-decode`, board 0b58f822).**  Owner re-report after #3116/#3117; both root-caused with live evidence (internal-origin callback clamp; equity-curve timestamp-vs-date decode break).  Server compat fixes installed builds on deploy; Swift side hardened for future builds.  Grok dispatch died twice (owner-directed grok-first); MONET landed per fallback.  Rollout: `docs/rollouts/2026-08-27-ios-auth-redirect-workspace-decode.md`.
