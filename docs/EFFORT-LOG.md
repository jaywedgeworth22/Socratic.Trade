- **[Socratic.Trade][CURSOR] CI autofix: ios-fleet stale-snapshot test — IN PROGRESS 2026-08-26 (branch `cursor/ci-autofix-automation-27b6`, unblocks #3091).**  `verify-hosted` failed because the publisher node:test pinned live `ios-app-versions.json` (`!net.dealdex`).  #3102 on `main` added that key, so the PR merge commit broke the pin.  Test now uses a constructed 2026-08-21 stale `--base-json`.  Rollout: `docs/rollouts/2026-08-26-ios-versions-stale-fixture-test.md`.
