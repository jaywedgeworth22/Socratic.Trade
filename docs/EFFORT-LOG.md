...
- **Admin authorization fail-closed hardening (CODEX, branch `codex/admin-fail-closed`, MERGED PR #1410, worktree
  ...
  mutation.
- **Strategy owner-token+heartbeat lease & scheduler single-leader default (AG draft -> CODEX implementation, DRAFT PR #1429, branch `codex/strategy-lease-correctness`) — FINAL RECONCILIATION SCOPED GREEN / ROOT FULL GATE PENDING 2026-07-11.** Owner-ruled P0 collision fix for strategy vs scheduler concurrency. Current `main@67e1536d` is merged; the run is account-snapshotted, long awaited phases re-prove ownership, durable proposal/broker truth survives ancillary loss, signal shutdown retains the scheduler lease until TTL, and completed-run-only auto-tuning is scheduled-account scoped under its own renewed account lease and LLM reservation. The final independent review also bound the prompt and walk-forward evidence to the account, honored a lost pending-to-blocked transition, and moved the tuning clock to follow-up start. Node24 combined rerun: 11 files/129 tests, touched lint 0 errors/36 inherited warnings, tsc and diff-check clean. PR remains draft; root owns the serialized full suite/build, trusted push, hosted checks, readiness, merge, and production verification. No provider-boundary, production-config, auto-merge, merge, or deploy mutation is included.
