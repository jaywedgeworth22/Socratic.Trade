...
- **[Socratic.Trade][CODEX] FMP coverage, market-scan reliability, and non-scan ticker-sheet enrichment (ready PR #1618, branch `codex/fmp-market-data-reliability`, worktree `/Users/jay/apps/trading-codex-fmp`, claimed 2026-07-15) — MAIN ADVANCE RECONCILED / OVERLAP GREEN / FINAL LAND GATE PENDING.** Production RCA found three July 14 interactive 25-second timeouts caused by a cold 150-symbol all-provider cascade; Finnhub can enqueue 750 paced requests while the old timeout leaked work. Interactive scans now keep deep ingestion off-request, accept only 24-hour slow facts, single-flight refreshes, bound Nasdaq, and show an explicitly stale strategy fallback. FMP now maps broader stable profile/ratio/insider data with durable endpoint attribution; out-of-scan ticker sheets have a current identity/quote floor, timestamp arbitration, coalescing, and dynamic company-name header. Exact LRCX browser QA passed with zero console errors. The first land gate passed TypeScript, 380 files / 4,375 tests, and build, then opened #1618. PR #1616 advanced main during that run; exact `d3efc9a6` is merged locally, its broader capability adapters are preserved and upgraded to verified header auth plus durable quota/outcome accounting, and scoped lint/TypeScript plus 5 files / 163 tests pass. Final `scripts/land.sh`, refreshed hosted checks, protected merge, and exact production verification remain.
