- **[Socratic.Trade][GROK] Durable symbol_field_latest store (per-field as_of/fetched_at) — IN PROGRESS 2026-08-05 (branch `grok/symbol-field-store`).** Shared SQLite latest-value store for every market field on every symbol ever seen; cascade + scan write; interactive/strategy seed from store so bounded strategy_run audits no longer blank the Scan table. Rollout: `docs/rollouts/2026-08-05-symbol-field-latest-store.md`.
...
- **[Socratic.Trade][GROK] Activity-audit P2.7+P2.8 (stale-exit cancel settle + synthetic-stop fail alert) — IN PROGRESS 2026-08-05 (branch `grok/activity-audit-p2-7-8`).** #1320 multi-poll cancel settle (~10s) + #1321 persistent protective-exit-failing notification (cooldown already on main). Issues #1320/#1321.
