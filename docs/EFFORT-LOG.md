...
## In Progress
- **Decision-detail dissent content deduplication (CODEX, ready PR #1593, branch `codex/decision-dissent-dedup`, 2026-07-14) — REVIEW FIX ACCEPTED / FINAL-HEAD HOSTED RERUN PENDING.** The decision trace keeps one canonical structured Red Team verdict, removes only its exact generic echo and known generated policy wrappers, and retains distinct policy/override objections, including title-only override context. Display-only; no stored-case, broker, provider, production, or deploy mutation. Browser QA passed at desktop and 390 px mobile. Exact-tree Node 24 gate: focused 4/4, lint, TypeScript, 369 files / 4,132 tests, production build with 32 static pages, and diff-check all green; `scripts/land.sh` repeated TypeScript, all 4,132 tests, and the build. Required hosted CI, Playwright smoke, and gitleaks passed on the pre-autofix head. The accepted review fix passes focused 5/5, scoped ESLint, TypeScript, and diff-check; final-head hosted reruns remain. Auto-merge is disabled and production is unchanged.
