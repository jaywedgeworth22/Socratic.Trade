...
- **[Socratic.Trade][KIMI] Backtest-integrity §6 slice 3: qlib walk-forward window report + in-sample disclosure — IMPLEMENTED 2026-07-30 (branch `kimi/walk-forward-window`, PR #2305, auto-merge armed).** Audit: split already sound (chronological, always-on embargo, opt-in purge); leak is candidate weights proposed from ALL-history evidence spanning the held-out fold — partially in-sample. `splitWalkForward` returns fold-boundary indices; `OOSResult.window` (required) + `formatOosWindow`; manual + autonomous OOS readouts name the held-out window and carry the partially-in-sample caveat (ledger/provenance evidence). 8 new/updated tests + 3 fixtures; tsc/lint/full-suite (5450, 3 shards)/build all green. Rollout: `docs/rollouts/2026-07-30-walk-forward-window.md`.
- **[Socratic.Trade] Time-bounded proposal evidence for the auto-tuner — PLANNED / UNASSIGNED.** Definitive fix for the §6 slice-3 finding: cut `proposeStrategyTuning`'s evidence queries (factor/source scorecards, fills, skipped-candidate counterfactuals, performance summary) off at the OOS test-fold start so candidate weights are generated without seeing evaluation-period outcomes; retires the "partially in-sample" caveat for the weight path. Context: `docs/rollouts/2026-07-30-walk-forward-window.md`.
