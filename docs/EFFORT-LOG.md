...
## Completed
- **[Socratic.Trade][CLAUDE-sub] PR #1669 (SEC/RAG Phase 3) Codex-thread pickup — IN PROGRESS 2026-07-17 (owner-directed cap-reset pickup of the stalled AG lane).** Fixed the 6 stale unresolved Codex threads: form-aware Item-title canonicalization (`parseFilingHtml` `formType` option; 10-K map only when proven, raw titles otherwise), standalone SEC section-heading recognition (bounded anchored set, slug codes), valid GFM for td-only split tables (synthesized empty header, no data-row promotion), and an explicit in-code decision note that the accession skip is deliberately NOT parser-revision-versioned (v1 filings keep v1 chunks). +3 regression tests. Gates green (tsc / 4,679 tests / lint 0 errors / build). Correction note (in place, per board rules): the AG row below marks PR #1669 COMPLETED, but the PR is NOT merged — auto-merge is armed and blocked on unresolved review threads; Codex added ~20 more unresolved threads 2026-07-17 00:24–03:16 UTC (incl. 2 P1s on the embedding-provider work) that remain open outside this pickup's scope. Rollout: `docs/rollouts/2026-07-17-pr1669-parser-thread-pickup.md`.
