- **[Socratic.Trade][CURSOR] Three-column LLM catalog (display / OpenRouter wire / native) — IN PROGRESS 2026-08-21 (branch `agent/cursor/llm-model-slugs`, worktree `~/apps/trading-cursor`).**  Owner table is the catalog.  Settings store display slugs; live OpenRouter calls send column 2; `nativeSlugFor` exports column 3.  Aliases map gpt-5.4-mini / claude-sonnet-5 / grok-4.5 / deepseek-v4-* / gemini-3.5-flash-lite onto the new display slugs.
