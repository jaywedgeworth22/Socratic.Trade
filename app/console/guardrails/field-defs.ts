...
export const PROTECTIVE_STOPS: FieldDef[] = [
  ...
  { path: "riskRules.trailingStopPct", label: "Trailing stop", kind: "pct", optional: true, hint: "Runs IN ADDITION to the stop above: exits when price falls this far from its high-water mark since entry. Blank/0 = off. When on, it also becomes broker-held where supported (see Broker-held trailing below)." },
  ...
  { path: "brokerTrailingStops", label: "Broker-held trailing stops", kind: "bool", looserWhen: "off", hint: `With a trailing % set: native trailing_stop orders on Alpaca (the broker moves the trigger itself); on live Robinhood a resting stop the app ratchets upward each cycle (needs Robinhood resting stops ON). Turning this OFF keeps trailing app-managed only. ${ADVISORY_NOTE}` },
