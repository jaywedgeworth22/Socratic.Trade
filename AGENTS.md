...
- **No "Test mode" / local simulator.** The local-simulation execution path (`usesLocalSimulation`,
  ...
  add it back or reintroduce any fake-fill path. The app trades through a connected broker (paper or
  live); with no connected account it simply can't place orders. (The app still needs a *database* —
  `DATABASE_URL` / `data/app.db` — that's infrastructure, not a fake execution mode.)
