...
- Centered all column header titles (`th`), left-aligned logo/ticker (`Symbol`), right-aligned `Price`, and centered all other column data points (`Score`, `Chg`, `Vol`, `P/E`, `EPS gr`, `Div`, `News`, `Insiders`, `Rating`, `Congress`, `Sector`).
- Split sentiment into two distinct columns: **News** (`q.sentiment`, titled "News sentiment score") and **Insiders** (`q.insiderSentiment`, titled "Insider sentiment score").
