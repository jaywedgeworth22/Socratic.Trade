## 2026-07-20 — RapidAPI and FilingAPI / ROIC.ai Enrichment Providers Integration (ANTIGRAVITY, branch `agent/antigravity`)
...
- Implemented 5 new provider classes (`FilingApiEnrichmentProvider`, `RoicEnrichmentProvider`, `FmpRapidApiEnrichmentProvider`, `InsidersRapidApiEnrichmentProvider`, `TwelveDataRapidApiEnrichmentProvider`) in `src/lib/data-providers.ts` and added them to `getEnrichmentProvider()` so they run automatically in the cascade. Added `quotaScarce` sorting so that non-scarce providers run first, minimizing consumption of scarce quotas.
- Added the three new RapidAPI provider keys to `RapidApiProviderKey` in `src/lib/rapidapi-quota.ts` to ensure they abide by the global `rapidApiCombinedDailyCap()` ceiling of 900 calls/day.
- Added full test coverage for all new providers in `test/rapidapi-providers.test.ts`, and verified that the combined quota tracks consumption appropriately across all registered RapidAPI providers.
