...
## 2026-07-09 — Enrichment starvation fix (MONET, branch `monet/bold-lamport-20a8f9`, PR #1272)
Fixed the prod-verified starvation (30/42 candidates enriched, 2026-07-09T19:41Z run): providers
now enrich the full candidate set they're passed — `maxSymbols()` is just the 50-symbol quota cap
with `FMP_MAX_SYMBOLS` as the absolute operator override (a fixed 30, and then an env-derived
shape, both under-covered real scans); the `enrich()` symbol list is ordered by
`orderCandidatesForEnrichment` (ALL held → event outliers → rest by rank, stable) so a budget
shortfall can never starve owned/force-included names; and `withProvenance`/`cellTitle` no longer
stamp "Received <time>" on fields no provider returned. See
...
both kept), focused tests 132/132 then full gate green, PR #1272 opened via `land.sh` with
auto-merge armed.
ROUND 2 2026-07-10 (MONET): PR #1272 sat BLOCKED on two real codex-connector findings — held
