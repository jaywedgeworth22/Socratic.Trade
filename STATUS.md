...
## 2026-08-19 CURSOR — Account write guards — MERGED/DEPLOYED #2876 `44e9ef06`

Expert review tranche-1 cluster `account-write-guards`: `strategyAuthority` stays pinned on profile activate/copy/import; `setActiveConnectedAccount` rejects draining rows; `upsertConnectedAccount` clears `is_draining` on reconnect.  Squash-merged to `main` 2026-08-20 (auto-merge on green `verify`).  Rollout: `docs/rollouts/2026-08-19-account-write-guards.md`.
