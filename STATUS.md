...
- 2026-06-21 (`agent/claude`): **Walk-forward OOS validation + cost+tax-adjusted equity curve vs SPY (P1-1b).** Extended `src/lib/backtest.ts` with `splitWalkForward` (chronological 70/30 train/test split), `adjustReturns` (20bps round-trip cost + 24% tax drag), `computeCompositeIC` (composite score ICIR — the key OOS signal-quality measure), `buildEquityCurve` (top-K strategy compounded vs SPY benchmark), and `runWalkForwardOOS` (full IO orchestrator). `GET /api/admin/backtest-ic` now returns an `oos: {...}` block by default (oosIC, oosICIR, oosICDefault, equityCurve, annualizedReturn, activeReturn, sharpeRatio, maxDrawdownPct). +25 tests (541 total), tsc clean. Commit: `34ec168`. Remaining P1: congress/insider windowing, sample-floor sizing, independent critique stage. See `docs/rollouts/2026-06-21-oos-validation.md`.
