 ...
 * setting). "disregard" (default) and "block" persist; anything else 400s with a clear message.
 */
...
describe("/api/policy — taxSettings.iraWashSaleHandling validation", () => {
  it("defaults to 'disregard' on a fresh policy", async () => {
  ...
  it("rejects any other value with a clear 400", async () => {
    const { PUT } = await import("../app/api/policy/route");
    for (const bad of ["ignore", "ask", "auto", "off", 1, true]) {
      const response = await PUT(putTaxSettings({ iraWashSaleHandling: bad }));
      expect(response.status).toBe(400);
      expect(await response.text()).toContain("taxSettings.iraWashSaleHandling must be block or disregard.");
    }
  });
