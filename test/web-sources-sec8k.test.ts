...
import { parseCurrent8KFeed, parseCikTickerMap, getEightKSignals, mergeEightK, refreshEightK } from "../src/lib/web-sources/sec8k";
...
describe("8-K parsers", () => {
  it("parses CIKs + accessions from the current-8-K feed (deduped)", () => {
    ...
    expect(rows[0]).toMatchObject({ cik: "320193", accession: "0000320193-26-000001", filedAt: "2026-06-17" });
  });
...
describe("getEightKSignals + refresh", () => {
  it("surfaces a catalyst bulletin for symbols with a recent 8-K", async () => {
    ...
    setInternalSetting("webSource:sec8k:dataset", { events: [{ symbol: "AAPL", filedAt: today, accession: "a1" }], fetchedAt: new Date().toISOString(), recordCount: 1 });
