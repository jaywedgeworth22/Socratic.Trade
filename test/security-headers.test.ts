...
//
// middleware.ts now sets X-Frame-Options + Referrer-Policy on EVERY returned response, and a CSP that
// is DEFAULT-OFF (only emitted when CSP_ENABLED is truthy) and report-only unless CSP_REPORT_ONLY is
// explicitly off. Middleware is awkward to unit-test end-to-end (edge runtime, session verification),
// so we assert the exported `withSecurityHeaders` helper that builds the header set for each response.
...
describe("G4: withSecurityHeaders", () => {
  it("always sets X-Frame-Options: DENY and Referrer-Policy, with CSP OFF by default", () => {
