...
describe("sideVerb — tense-matched to lifecycle status", () => {
  ...
  it("passes an unknown side through unchanged, tense notwithstanding", () => {
    expect(sideVerb("frobnicate", "placed")).toBe("frobnicate");
  ...
  it("renders a pure observation (no side) as 'Observed'", () => {
    ...
    expect(sideVerb(undefined, "placed")).toBe("Observed");
...
describe("isExecutedStatus — only 'placed'/'filled'/'executed' mean an order reached the broker", () => {
  it("is true only for placed/filled/executed", () => {
    for (const s of ["placed", "filled", "executed", "PLACED", "Filled"]) {
      expect(isExecutedStatus(s)).toBe(true);
    }
  });

  it("is false for every non-executed status", () => {
    for (const s of ["proposed", "planned", "blocked", "rejected", "error", "pending", "approved", "skipped", "observed"]) {
...
describe("isNotPlacedStatus — terminal 'nothing reached the broker' states", () => {
  it("is true for blocked/rejected/error/failed", () => {
    for (const s of ["blocked", "rejected", "error", "failed", "Blocked", "REJECTED"]) {
      expect(isNotPlacedStatus(s)).toBe(true);
    }
  });

  it("is false for executed and in-flight states", () => {
    for (const s of ["placed", "filled", "executed", "proposed", "planned", "pending", "observed"]) {
