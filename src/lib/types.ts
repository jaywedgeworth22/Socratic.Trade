...
export interface AccountCapabilities {
  ...
  /**
   * Minimum account equity (dollars) required to open or maintain margin status.
   * Reg T federal minimum is $2,000; many brokers set higher house minimums.
   * e.g. 2000 for standard Reg T.
   */
  marginMinimumEquity?: number;
  /**
   * Maintenance margin percentage: the minimum equity-to-position-value ratio the
   * broker requires before issuing a margin call. Reg T / FINRA minimum is 25%;
   * brokers often require 30–40% ("house requirement").
