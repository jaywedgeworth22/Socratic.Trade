...
export async function runStrategyOnce(userId: string = "local"): Promise<StrategyResult> {
  ...
  try {
    ...
    const tradeNoun = executionState.usesLocalSimulation ? "Mock/Local Trade" : "Trade";
    const summary = [
...
async function proposeTrades(input: {
  ...
  const systemPrompt = [
    ...
    "Do not call mock/local mode Paper mode. Broker Paper, including Alpaca Paper, is a separate broker-hosted sandbox account concept.",
    "",
    "Investment Strategy:",
    input.prompt,
    "",
