...
export async function runStrategyOnce(
  ...
  try {
    ...
    const sizedProposals = llmProposals
      ...
      .map((p) => {
        const sized = applyDeterministicSizing(p, policy, workingPortfolio, learningSource, userId, workingPositions, marketScan, calibrationForSizing, realizedVolPctBySymbol, bookHeat);
