...
// Import from sub-modules so their symbols are available within db.ts
import { getUserSetting, setUserSetting } from "./db-settings";
import { getActiveStrategyProfile } from "./db-profiles";
...
// ── Settings, consent, and market-data demand — see db-settings.ts ──────────
...
// ── Audit log helpers + counterfactual learning — see db-learning.ts ─────────
export * from "./db-learning";


// ── Strategy profiles — see db-profiles.ts ───────────────────────────────────
export * from "./db-profiles";


// ── Execution timing, run lock, strategy runs — see db-execution.ts ─────────
...
// ── Trade proposals — see db-proposals.ts ────────────────────────────────────
export * from "./db-proposals";


// ── Fill events and portfolio snapshots — see db-fills.ts ────────────────────
export * from "./db-fills";


// ── Notification events — see db-notifications.ts ────────────────────────────
...
export function syncActiveProfile(patch: { policy?: TradingPolicy; prompt?: string; scoringWeights?: ScoringWeights }, userId: string = "local"): void {
  const active = getActiveStrategyProfile(userId);
  if (!active) return;
...
export function setSettingDirect(userId: string, key: string, value: unknown, updatedAt: string): void {
  getDb()
    .prepare(
...
// ── API keys, connected accounts, stops, watchlist, alerts, notify prefs, chat, memory — see db-api-keys.ts ─
