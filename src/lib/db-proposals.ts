...
export function updateProposalStatus(id: string, status: string, orderId?: string, review?: ReviewedOrder, estimatedNotional?: number, userId: string = "local", refId?: string, errorMessage?: string, decision?: PolicyDecision): void {
  getDb()
    .prepare(
    ...
    .run(status, orderId ?? null, review ? JSON.stringify(review) : null, estimatedNotional ?? null, refId ?? null, errorMessage ?? null, decision ? JSON.stringify(decision) : null, id, userId);
}
