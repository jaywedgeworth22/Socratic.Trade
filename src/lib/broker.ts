 ...
 * SINGLE choke point every real-order path flows through — the strategy loop, approval path,
 * synthetic stops, broker protective stops, order replacement, and any future caller — so real
 * capital is never touched unless `ALLOW_LIVE_TRADING` is set AND the run is genuinely out of paper
 * mode. No-op in Test/paper. The execution state is derived lazily (only when an order is actually
 * placed) so read-only gateway uses pay no extra DB cost. A Proxy (not mutation) keeps the underlying
