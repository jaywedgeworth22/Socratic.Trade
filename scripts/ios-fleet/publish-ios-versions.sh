...
#
# SAFETY: the in-repo ios-app-versions.json is a dated snapshot (2026-08-21)
# that is missing net.dealdex 1.0.2 and still lists Usage Client 1.0.8,
# Usage Local 1.0.7, and Autorotate 1.0.1.  Starting from that file and
# PUTting it would wipe / roll back sibling apps on the shared fleet
# manifest the in-app update prompt reads.  Always seed from the live
# remote (or an explicit --base-json).  Refuse to push a document with
# no apps.  Do not fall back to the stale in-repo snapshot when the
# remote cannot be fetched.
...
if [[ -z "$BASE_JSON" ]]; then
  ...
  if fetch_remote_json "$TMP_REMOTE"; then
    ...
    echo "error: no remote ${REPO}/${REMOTE_PATH}" >&2
    echo "error: refusing to publish the stale in-repo snapshot (would wipe the fleet manifest)" >&2
    exit 1
  fi
fi
