...
test("in-repo snapshot used as base would drop net.dealdex and roll back siblings", () => {
  const local = JSON.parse(
    readFileSync(join(ROOT, "scripts/ios-fleet/ios-app-versions.json"), "utf8"),
  );
  assert.equal(local.apps["net.dealdex"], undefined);
  assert.equal(local.apps["services.jays.usage.client.monitor"].marketingVersion, "1.0.8");
  assert.equal(local.apps["services.jays.usage.local.monitor"].marketingVersion, "1.0.7");
  assert.equal(local.apps["codes.autorotate"].marketingVersion, "1.0.1");
});

test("publish script fetches remote and refuses the stale in-repo snapshot", () => {
  const src = readFileSync(SCRIPT, "utf8");
  assert.match(src, /fetch_remote_json/);
  assert.match(src, /refusing to publish the stale in-repo snapshot/);
